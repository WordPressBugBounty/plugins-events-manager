				...
				if ( ! empty( $data ) && is_array( $data ) && ( ! defined( 'EM_USE_DATA_ATTS' ) || EM_USE_DATA_ATTS ) ) {
					...
					foreach ( $data as $key => $value ) {
						if ( array_key_exists( $key, $query_arr ) ) {
							if ( $ajax || empty( $_REQUEST['action'] ) || in_array( $key, array( 'action', 'id' ) ) ) {
								unset( $query_arr[ $key ] );
							}
						}
...
function em_admin_paginate( $total, $limit, $page = 1, $vars = false, $base = false, $format = '' ) {
	...
	$return    .= sprintf(
		'<span class="displaying-num">' . __( 'Displaying %1$s&#8211;%2$s of %3$s', 'events-manager' ) . ' </span>%4$s',
...
function em_get_scopes() {
	...
	$scopes             = array(
		...
		'week'       => sprintf( __( 'Events this whole week (%1$s to %2$s)', 'events-manager' ), $wp_locale->get_weekday_abbrev( $start_of_week_name ), $wp_locale->get_weekday_abbrev( $end_of_week_name ) ),
...
function em_get_currency_formatted( $price, $currency = false, $format = false, $precision = 2 ) {
	$formatted_price = '';
	if ( ! $format ) {
		$format = em_get_option( 'dbem_bookings_currency_format', '@#' );
	}
	if ( ! $currency ) {
...
function em_verify_nonce( $action, $nonce_name = '_wpnonce' ) {
	if ( is_admin() ) {
		if ( ! wp_verify_nonce( $_REQUEST[ $nonce_name ] ?? '', $action ) ) {
			check_admin_referer( 'trigger_error' );
		}
	} else {
		if ( ! wp_verify_nonce( $_REQUEST[ $nonce_name ] ?? '', $action ) ) {
			exit( __( 'Trying to perform an illegal action.', 'events-manager' ) );
		}
	}
}
...
function em_get_attributes( $lattributes = false ) {
	...
	if ( ! $lattributes && ! em_get_option( 'dbem_attributes_enabled' ) ) {
		return $attributes;
	}
	if ( $lattributes && ! em_get_option( 'dbem_location_attributes_enabled' ) ) {
		return $attributes;
	}
...
function em_booking_add_registration( $EM_Booking ) {
	...
	if ( ( ( ! is_user_logged_in() && em_get_option( 'dbem_bookings_anonymous' ) ) || EM_Bookings::is_registration_forced() ) && ! em_get_option( 'dbem_bookings_registration_disable' ) ) {
		...
		$user_data              = array(
			...
			'dbem_phone' => $_REQUEST['dbem_phone'],
		);
...
function em_new_user_notification() {
	...
	if ( em_get_option( 'dbem_email_disable_registration' ) ) {
		return true;  }
...
function em_wp_is_super_admin( $user_id = false ) {
	...
	if ( is_multisite() ) {
		if ( $user->has_cap( 'manage_network_options' ) ) {
			return true;
		}
	} else {
...
function em_get_search_form_defaults( $base_args = array(), $context = 'events' ) {
	...
	//scope
	$search_args['scope']                    = array(
		'',
		'',
		'name' => em_get_option( 'dbem_events_default_scope' ),
	); //default scope term
	...
	if ( empty( $search_views[ $args['view'] ] ) ) {
		$args['view'] = 'list';
	}
	...
	if ( ! empty( $args['search_term'] ) ) {
		$args['main_classes'][] = 'has-search-term';
	}
	if ( ! empty( $args['search_geo'] ) ) {
		$args['main_classes'][] = 'has-search-geo';
	}
	...
	if ( isset( $_REQUEST['view_id'] ) ) {
		$args['id'] = absint( $_REQUEST['view_id'] ); // id used for element ids
	}
	if ( isset( $_REQUEST['id'] ) ) {
		$args['id'] = absint( $_REQUEST['id'] ); // id used for element ids
	}
	if ( isset( $_REQUEST['geo'] ) ) {
		$args['geo'] = sanitize_text_field( $_REQUEST['geo'] );
	}
	if ( isset( $_REQUEST['near'] ) ) {
		$args['near'] = sanitize_text_field( wp_unslash( $_REQUEST['near'] ) );
	}
	if ( isset( $_REQUEST['em_search'] ) ) {
	...
	if ( isset( $_REQUEST['country'] ) ) {
		$args['country'] = sanitize_text_field( wp_unslash( $_REQUEST['country'] ) );
	}
	if ( isset( $_REQUEST['region'] ) ) {
		$args['region'] = sanitize_text_field( wp_unslash( $_REQUEST['region'] ) );
	}
	if ( isset( $_REQUEST['state'] ) ) {
		$args['state'] = sanitize_text_field( wp_unslash( $_REQUEST['state'] ) );
	}
	if ( isset( $_REQUEST['town'] ) ) {
		$args['town'] = sanitize_text_field( wp_unslash( $_REQUEST['town'] ) );
	}
	if ( isset( $_REQUEST['near_unit'] ) ) {
		$args['near_unit'] = sanitize_text_field( $_REQUEST['near_unit'] );
	}
	if ( isset( $_REQUEST['near_distance'] ) ) {
	...
	if ( empty( $args['ajax'] ) ) {
		...
		// get URL without pno
		$url       = add_query_arg( array( 'pno' => null ) );
		$url_parts = explode( '?', $url );
	...
	switch ( $view ) {
		...
		case 'map':
			...
			em_locate_template(
				...
				array(
					'args'           => $args,
					'map_json_style' => ! empty( $args['map_style'] ) ? $args['map_style'] : '',
				)
			);
			break;
		case 'calendar':
...
function em_parse_map_args( $args ) {
	...
	if ( $height == 0 || $height == '0px' || $height == '0%' ) {
		$height = 0;
	}
	$args['width']  = $width;
	$args['height'] = $height;
	...
	<tr valign="top" id='<?php echo esc_attr( $name ); ?>_row'>
		...
		<td>
			<div class="em-default-option">
				...
				if ( $translate ) :
					?>
					<span class="em-translatable dashicons dashicons-admin-site"></span><?php endif; ?>
				<br />
				<?php
				if ( $translate ) {
					...
					<div class="em-archetype-options">
						<?php foreach ( $archetype_options['archetypes'] as $type => $atts ) : ?>
							<tbody class="em-archetype-option" data-archetype="<?php echo $type; ?>">
								<tr>
									<td>
										<input id="<?php echo esc_attr( $name ); ?>_default_<?php echo $type; ?>" name="em_archetype_options[<?php echo $type; ?>][<?php echo esc_attr( $name ); ?>]" type="radio" value="" 
										<?php
										if ( $undefined ) {
											echo 'checked';}
										?>
								...
								<?php foreach ( $options as $value => $text ) : ?>
									<tr>
										...
										?>
										<td>
											<input id="<?php echo esc_attr( $name ); ?>_<?php echo esc_attr( $value ); ?>_<?php echo $type; ?>" name="em_archetype_options[<?php echo $type; ?>][<?php echo esc_attr( $name ); ?>]" type="radio" value="<?php echo esc_attr( $value ); ?>" 
											<?php
											if ( $atts['value'] == $value ) {
												echo 'checked';}
											?>
...
function em_options_get_archetype_options( $option_name ) {
	...
	if ( Archetypes::$types ) {  // skip this if no archetypes exist
		if ( preg_match( '/^([^\[]+)(\[([a-z_A-Z0-9\-]+)\])?$/', $option_name, $matches ) && Archetypes_Admin::is_overrideable( $option_name ) ) {
			...
			$return['classes']    = array( 'has-archetype-option' );
			$return['archetypes'] = array();
			foreach ( Archetypes::$types as $type => $archetype ) {
				if ( Archetypes_Admin::is_overrideable( $option_name, $type ) ) {
					...
					if ( $translate ) {
						...
						$return['archetypes'][ $type ]['ml'] = array(
							'values' => $translated[ $option_name ] ?? array(),
						);
					}
				}
			}
...
function em_options_radio_binary( $title, $name, $description = '', $option_names = '', $trigger = '', $untrigger = false ) {
	if ( empty( $option_names ) ) {
		$option_names = array(
			0 => __( 'No', 'events-manager' ),
			1 => __( 'Yes', 'events-manager' ),
		);
	}
	...
	?>
		...
		<td class="<?php echo $class; ?>">
			<div class="em-default-option">
				<?php echo $option_names[1]; ?> <input id="<?php echo esc_attr( $id ); ?>_yes" name="<?php echo esc_attr( $name ); ?>" type="radio" value="1" 
				<?php
				if ( $value ) {
					echo 'checked';
				} echo $trigger_att;
				?>
				>&nbsp;&nbsp;&nbsp;
				<?php echo $option_names[0]; ?> <input id="<?php echo esc_attr( $id ); ?>_no" name="<?php echo esc_attr( $name ); ?>" type="radio" value="0" 
				<?php
				if ( ! $value ) {
					echo 'checked';
				} echo $trigger_att;
				...
				<div class="em-archetype-options">
					<?php foreach ( $archetype_options['archetypes'] as $type => $opts ) : ?>
						<div class="em-archetype-option" id='<?php echo $id; ?>_row_<?php echo $type; ?>' data-archetype="<?php echo $type; ?>">
							...
							$default   = $value ? 1 : 0
							?>
							<?php echo $option_names[1]; ?> <input id="<?php echo esc_attr( $id ); ?>_yes_<?php echo $type; ?>" name="<?php echo esc_attr( $opts['name'] ); ?>" type="radio" value="1" 
							<?php
							if ( $opts['value'] ) {
								echo 'checked';
							} echo $trigger_att;
							?>
							>&nbsp;&nbsp;&nbsp;
							<?php echo $option_names[0]; ?> <input id="<?php echo esc_attr( $id ); ?>_no_<?php echo $type; ?>" name="<?php echo esc_attr( $opts['name'] ); ?>" type="radio" value="0" 
							<?php
							...
							?>
							>&nbsp;&nbsp;&nbsp;
							<?php esc_html_e( 'Default', 'events-manager' ); ?> (<?php echo $option_names[ $default ]; ?>) <input id="<?php echo esc_attr( $id ); ?>_default_<?php echo $type; ?>" name="<?php echo esc_attr( $opts['name'] ); ?>" type="radio" value="" 
							<?php
							if ( $undefined ) {
								echo 'checked';
							} echo $trigger_att;
...
function em_options_select( $title, $name, $list, $description = '', $default = '', $triggers = array(), $options = array() ) {
	...
	if ( ! empty( $triggers ) ) {
		$select_classes[] = 'em-trigger';
	}
	if ( ! empty( $options['selectize'] ) ) {
		$select_classes[] = 'em-selectize';
	}
	...
	?>
		<tr valign="top" id='<?php echo esc_attr( $name ); ?>_row'>
			...
			<td>
			<div class="em-default-option">
				<select name="<?php echo esc_attr( $name ); ?>" class="<?php echo implode( ' ', $select_classes ); ?>" 
				<?php
				if ( ! empty( $options['multiple'] ) ) {
					echo 'multiple';}
				?>
			...
			<?php if ( ! empty( $archetype_options['archetypes'] ) ) : ?>
				<div class="em-archetype-options">
					...
					<div class="em-archetype-option" id='<?php echo $name; ?>_row_<?php echo $type; ?>' data-archetype="<?php echo $type; ?>">
						<select name="<?php echo esc_attr( $opts['name'] ); ?>" class="<?php echo implode( ' ', $select_classes ); ?>" 
						<?php
						if ( ! empty( $options['multiple'] ) ) {
							echo 'multiple';}
						?>
...
if ( ! function_exists( 'is_main_query' ) ) {
	...
	function is_main_query() {
		global $wp_query;
		return $wp_query->in_the_loop == true; }
}
