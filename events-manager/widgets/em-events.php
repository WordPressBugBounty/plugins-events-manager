...
class EM_Widget extends WP_Widget {
	...
	public $em_orderby_options = array(
	...
	function __construct() {
		...
		parent::__construct( false, 'Events', array( 'description' => 'Display a list of events on Events Manager.' ) );
	...
	function wp_loaded() {
		...
		$this->em_orderby_options = apply_filters(
	...
	function widget( $args, $instance ) {
		...
		if ( ! array_key_exists( $instance['orderby'], $this->em_orderby_options ?? array() ) ) {
			//replace old values
			$old_vals = array(
				'name'       => 'event_name',
		...
		foreach ( $this->defaults as $key => $value ) {
			...
			if ( in_array( $key, array( 'format', 'no_events_text', 'all_events_text' ) ) ) {
				if ( is_multisite() && ! em_wp_is_super_admin() ) {
					$new_instance[ $key ] = wp_kses_post( $new_instance[ $key ] ); //for multisite
				}
	...
	function form( $instance ) {
		...
		if ( ( $v6 === true || $v6 === 'undo' ) && empty( $instance['v6'] ) ) {
			...
			if ( ! preg_match( '/^<li/i', trim( $instance['no_events_text'] ) ) ) {
				$instance['no_events_text'] = '<li class="no-events">' . $instance['no_events_text'] . '</li>';
			}
		}
		?>
		<p>
