...
$args = array_merge(
	array(
		...
		'scope'   => em_get_option( 'dbem_ical_scope' ),
	),
	$args
);
	...
	foreach ( $EM_Events as $EM_Event ) {
		...
		if ( $show_timezone ) {
			...
			if ( empty( $timezones[ $timezone ] ) ) {
				...
				if ( $timezones[ $timezone ][0] > $EM_Event->start()->getTimestamp() ) {
					$timezones[ $timezone ][0] = $EM_Event->start()->getTimestamp();
				}
				if ( $timezones[ $timezone ][1] < $EM_Event->end()->getTimestamp() ) {
					$timezones[ $timezone ][1] = $EM_Event->end()->getTimestamp();
				}
			}
		}
		...
		if ( is_multisite() ) {
			$UID = absint( $EM_Event->blog_id ) . '-' . $UID;
		}
...
if ( $timezone_support && ! empty( $timezones ) ) {
	...
	foreach ( $timezones as $timezone => $timezone_range ) {
		...
		do {
			...
			if ( $current_transition['ts'] > $timezone_range[1] ) {
				break;
			}
