...
if ( is_user_logged_in() ) { //only show this to logged in users
	...
	if ( $EM_Event->event_active_status === 0 ) {
		...
		?>
		<span class="em-closed-button"><?php echo $notice_cancelled; ?></span>
		<?php
	} else {
		...
		if ( is_object( $EM_Booking ) && $EM_Booking->booking_status != 3 && em_get_option( 'dbem_bookings_user_cancellation' ) ) {
			...
			?>
			<a id="em-cancel-button_<?php echo $booking_id . '_' . $nonce; ?>" class="button em-cancel-button" href="#" data-booking_id="<?php echo $booking_id; ?>" data-_wpnonce="<?php echo $nonce; ?>" data-action="booking_cancel"><?php echo $button_cancel; ?></a>
			<?php
			...
			if ( ! is_object( $EM_Booking ) ) {
				...
				?>
				<a id="em-booking-button_<?php echo $event_id . '_' . $nonce; ?>" class="button em-booking-button" href="#"  data-event_id="<?php echo $event_id; ?>" data-_wpnonce="<?php echo $nonce; ?>" data-action="booking_add_one"><?php echo $button_text; ?></a>
				<?php
			} else {
				$status = 'booked';
				?>
				<span class="em-booked-button"><?php echo $button_already_booked; ?></span>
				<?php
			}
