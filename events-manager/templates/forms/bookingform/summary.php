...
?>
<div class="em-booking-summary 
<?php
if ( empty( $EM_Booking ) ) {
	echo 'no-booking';}
?>
" id="em-booking-summary-<?php echo esc_attr( $EM_Event->event_id ); ?>"
	...
	<?php if ( $EM_Booking->get_spaces() > 0 ) : ?>
		...
		$subtotal_raw = apply_filters(
			'em_booking_form_summary_subtotal_amount',
			$subtotal_raw,
			$EM_Booking,
			array(
				'base_tax'      => $base_tax,
				'amount_raw'    => $subtotal_raw,
				'itemize_taxes' => $itemize_taxes,
