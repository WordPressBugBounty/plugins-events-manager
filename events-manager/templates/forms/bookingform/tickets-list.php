			...
			<tr class="em-ticket em-ticket-<?php echo $EM_Ticket->ticket_id; ?>" data-ticket-id="<?php echo absint( $EM_Ticket->ticket_id ); ?>">
				<?php foreach ( $collumns as $type => $name ) : ?>
					...
					switch ( $type ) {
						case 'type':
							?>
							<td class="em-bookings-ticket-table-type"><?php echo wp_kses( $EM_Ticket->name, 'em_ticket_name' ); ?>
							<?php
							if ( ! empty( $EM_Ticket->description ) ) :
								?>
<br><span class="ticket-desc"><?php echo wp_kses( $EM_Ticket->description, $allowedposttags ); ?></span><?php endif; ?></td>
							<?php
							break;
						case 'price':
							?>
