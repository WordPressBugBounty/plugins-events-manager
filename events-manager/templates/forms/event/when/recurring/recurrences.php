...
if ( $EM_Event->event_id ) {
	echo 'data-event_id="' . $EM_Event->event_id . '"';}
?>
>
	<?php
	foreach ( $types as $type => $description ) :
		...
		<div class="em-recurrence-type em-recurrence-type-<?php echo $type; ?>
		<?php
		if ( $type_count === 0 ) {
			echo 'em-recurrence-type-new';}
		?>
		" data-type="<?php echo $type; ?>" data-count="<?php echo $type_count; ?>" data-index="<?php echo $i; ?>">
			...
			<div class="em-recurrence-type-sets">
				<?php foreach ( $EM_Event->get_recurrence_sets()->{$type} as $Recurrence_Set ) : ?>
					<div class="em-recurrence-set 
					<?php
					if ( ! $Recurrence_Set->recurrence_set_id ) {
						echo 'new-recurrence-set';}
					?>
					" data-type="<?php echo esc_attr( $type ); ?>" id="recurrence-set-<?php echo $type . '-' . $i . '-' . $id; ?>">
						<?php
	...
	<template class="em-recurrence-set-template">
		<?php
			...
			require em_locate_template( 'forms/event/when/recurring/recurrences/set.php' );
		?>
	</template>
</div>