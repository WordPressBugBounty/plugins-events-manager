	...
	<div id="event-rsvp-options" class="event-rsvp-options">
		...
		">
			...
			if ( $reschedule_warnings && empty( $_REQUEST['recreate_tickets'] ) ) {
				$container_classes[] = 'reschedule-hidden';
			}
			if ( em_get_option( 'dbem_bookings_tickets_ordering' ) ) {
				$container_classes[] = 'em-tickets-sortable';
			}
			?>
								...
								<tr class="em-tickets-row">
									...
									<td class="ticket-name">
										...
										if ( $EM_Ticket->members ) {
											echo '* ';}
										?>
										<?php echo wp_kses_post( $EM_Ticket->name ); ?></span>
										<div class="ticket_description">
										<?php
										if ( ! empty( $EM_Ticket->description ) ) {
											echo wp_kses_post( $EM_Ticket->description );}
										?>
										</div>
										<div class="ticket-actions">
											<a href="#" class="ticket-actions-edit"><?php esc_html_e( 'Edit', 'events-manager' ); ?></a>
										<?php if ( $EM_Ticket->get_bookings_count() == 0 ) : ?>
											| <a href="<?php bloginfo( 'wpurl' ); ?>/wp-load.php" class="ticket-actions-delete 
											<?php
											if ( $EM_Event->is_recurring( true ) && $EM_Ticket->multi_event == null ) {
												echo 'parent-ticket';}
											?>
											" ><?php esc_html_e( 'Delete', 'events-manager' ); ?></a>
											<?php else : ?>
			...
			<div class="recurrence-reschedule-buttons">
				<button class="button-secondary button em-reschedule-cancel
				<?php
				...
				<button class="em-reschedule-trigger button button-secondary
				<?php
				if ( ! empty( $_REQUEST['recreate_tickets'] ) ) {
					echo ' reschedule-hidden';}
				?>
		...
		<p>
			...
			<div class="inline-inputs">
				...
				<input type="text" name="event_rsvp_time" class="em-time-input" maxlength="8" size="8" value="
				<?php
				if ( ! empty( $EM_Event->event_rsvp_time ) ) {
					echo $EM_Event->rsvp_end()->format( em_get_hour_format() );}
				?>
				" />
			</div>
			<br />
