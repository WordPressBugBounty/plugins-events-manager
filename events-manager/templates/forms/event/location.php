...
">
	...
	<select name="location_type" class="em-location-types-select" data-active="<?php echo esc_attr( $EM_Event->event_location_type ); ?>">
		...
		<option value="<?php echo esc_attr( $location_type ); ?>" 
			<?php
			if ( ! empty( $location_type_option['selected'] ) ) {
				echo 'selected="selected"';}
			?>
		data-display-class="
			<?php
			if ( ! empty( $location_type_option['display-class'] ) ) {
						echo esc_attr( $location_type_option['display-class'] );}
			?>
">
...
<div id="em-location-data" class="em-location-data em-location-where em-location-type em-location-type-place 
	<?php
	if ( count( $location_types ) == 1 ) {
		echo 'em-location-type-single ';
	}  em_template_classes( 'event-editor' );
