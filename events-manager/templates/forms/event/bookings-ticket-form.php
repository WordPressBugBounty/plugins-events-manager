	...
	<div class="em-ticket-form-main">
		...
		<div class="ticket-status">
			...
			<select name="em_tickets[<?php echo $col_count; ?>][ticket_status]" class="ticket_status" data-default="<?php echo $default_value; ?>">
				...
				<option value="1" 
				<?php
				if ( $EM_Ticket->ticket_status ) {
					echo 'selected';}
				?>
				><?php esc_html_e( 'Enabled', 'events-manager' ); ?></option>
				<option value="0" 
				<?php
				if ( $EM_Ticket->ticket_status === 0 ) {
					echo 'selected';}
				?>
				><?php esc_html_e( 'Disabled', 'events-manager' ); ?></option>
			</select>
		</div>
	</div>
		...
		<div class="ticket-dates em-time-range">
			<div class="ticket-dates-from inline-inputs">
				...
				<div class="ticket-dates-from-recurring ">
					<input type="text" name="em_tickets[<?php echo $col_count; ?>][ticket_start_recurring_days]" size="3" value="
					<?php
					if ( ! empty( $EM_Ticket->ticket_meta['recurrences']['start_days'] ) && is_numeric( $EM_Ticket->ticket_meta['recurrences']['start_days'] ) ) {
						echo absint( $EM_Ticket->ticket_meta['recurrences']['start_days'] );}
					?>
					" >
					...
					<select name="em_tickets[<?php echo $col_count; ?>][ticket_start_recurring_when]" class="ticket-dates-from-recurring-when">
						<option value="before" 
						<?php
						if ( isset( $EM_Ticket->ticket_meta['recurrences']['start_days'] ) && $EM_Ticket->ticket_meta['recurrences']['start_days'] <= 0 ) {
							echo 'selected="selected"';}
						?>
						><?php echo esc_html( sprintf( _x( '%s the event starts', 'before or after', 'events-manager' ), __( 'Before', 'events-manager' ) ) ); ?></option>
						<option value="after" 
						<?php
						if ( ! empty( $EM_Ticket->ticket_meta['recurrences']['start_days'] ) && $EM_Ticket->ticket_meta['recurrences']['start_days'] > 0 ) {
							echo 'selected="selected"';}
						?>
						><?php echo esc_html( sprintf( _x( '%s the event starts', 'before or after', 'events-manager' ), __( 'After', 'events-manager' ) ) ); ?></option>
					</select>
				</div>
			...
			<div class="ticket-dates-to inline-inputs">
				...
				<div class="ticket-dates-to-recurring">
					<input type="text" name="em_tickets[<?php echo $col_count; ?>][ticket_end_recurring_days]" size="3" value="
					<?php
					if ( isset( $EM_Ticket->ticket_meta['recurrences']['end_days'] ) && $EM_Ticket->ticket_meta['recurrences']['end_days'] !== false ) {
						echo absint( $EM_Ticket->ticket_meta['recurrences']['end_days'] );}
					?>
					" >
					<?php esc_html_e( 'day(s)', 'events-manager' ); ?>
					<select name="em_tickets[<?php echo $col_count; ?>][ticket_end_recurring_when]" class="ticket-dates-to-recurring-when">
						<option value="before" 
						<?php
						if ( isset( $EM_Ticket->ticket_meta['recurrences']['end_days'] ) && $EM_Ticket->ticket_meta['recurrences']['end_days'] <= 0 ) {
							echo 'selected="selected"';}
						?>
						><?php echo esc_html( sprintf( _x( '%s the event starts', 'before or after', 'events-manager' ), __( 'Before', 'events-manager' ) ) ); ?></option>
						<option value="after" 
						<?php
						if ( ! empty( $EM_Ticket->ticket_meta['recurrences']['end_days'] ) && $EM_Ticket->ticket_meta['recurrences']['end_days'] > 0 ) {
							echo 'selected="selected"';}
						?>
						><?php echo esc_html( sprintf( _x( '%s the event starts', 'before or after', 'events-manager' ), __( 'After', 'events-manager' ) ) ); ?></option>
					</select>
		...
		<div class="ticket-type">
			...
			<select name="em_tickets[<?php echo $col_count; ?>][ticket_type]" class="ticket_type" data-default="<?php echo $default_value; ?>">
				...
				if ( $EM_Ticket->ticket_members ) {
					echo 'selected="selected"';}
				?>
				><?php esc_html_e( 'Logged In Users', 'events-manager' ); ?></option>
				<option value="guests" 
				<?php
				if ( $EM_Ticket->ticket_guests ) {
					echo 'selected="selected"';}
				?>
				><?php esc_html_e( 'Guest Users', 'events-manager' ); ?></option>
			</select>
		</div>
