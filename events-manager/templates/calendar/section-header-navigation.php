...
<section class="em-cal-nav em-cal-nav-<?php echo esc_attr( $args['calendar_header'] ); ?>">
	...
	<div class="month-nav input">
		...
		<?php
		if ( ! empty( $args['calendar_nav_nofollow'] ) ) {
			echo 'rel="nofollow"';}
		?>
		>
		...
		<?php if ( $args['calendar_header'] === 'centered' ) : ?>
			<?php echo $header; ?>
		<?php else : ?>
			<a href="<?php echo esc_url( $calendar['links']['today_url'] ); ?>" class="em-calnav-today button button-secondary size-large size-medium 
			<?php
			if ( date( 'Y-m' ) === $EM_DateTime->format( 'Y-m' ) ) {
				echo 'is-today';}
			?>
			" 
			<?php
