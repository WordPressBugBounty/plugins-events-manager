...
<form id='locations-filter' method='post' action='' class="input">
	...
	if ( ! empty( $show_add_new ) && current_user_can( 'edit_locations' ) ) {
		echo '<a class="em-button button add-new-h2" href="' . add_query_arg( 'action', 'edit', $url ) . '">' . __( 'Add New', 'events-manager' ) . '</a>';
	}
	?>
	...
	<div class="subsubsub">
		...
		?>
		' <?php echo ( empty( $_REQUEST['view'] ) ) ? 'class="current"' : ''; ?>><?php echo sprintf( __( 'My %s', 'events-manager' ), __( 'Locations', 'events-manager' ) ); ?> <span class="count">(<?php echo $locations_mine_count; ?>)</span></a>
		...
		<a href='
			...
			?>
					' <?php echo ( ! empty( $_REQUEST['view'] ) && $_REQUEST['view'] == 'others' ) ? 'class="current"' : ''; ?>><?php echo sprintf( __( 'All %s', 'events-manager' ), __( 'Locations', 'events-manager' ) ); ?><span class="count">(<?php echo $locations_all_count; ?>)</span></a>
		<?php endif; ?>
	</div>
	...
	<div class='tablenav'>					
		...
		<div class="alignleft actions">
			...
			$hide_checkboxes = true; /* @todo this and the first condition of this if statement will need to change when other bulk actions are added */
			?>
		<?php endif; ?>
		<?php
