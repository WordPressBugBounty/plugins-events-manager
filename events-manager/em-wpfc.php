			...
			if ( ! in_array( $EM_Event->event_id, $event_ids ) ) {
				...
				if ( $event_day_counts[ $date ] <= $limit ) {
					...
					$event_array = array(
						...
						'allDay'      => $allDay,
						'nonce'       => wp_create_nonce( 'wpfc_event_nonce_' . $EM_Event->event_id ),
					);
					if ( $args['long_events'] == 0 ) {
						$event_array['end'] = $event_array['start']; //if long events aren't wanted, make the end date same as start so it shows this way on the calendar
					}
