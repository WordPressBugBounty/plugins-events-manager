...
class EM_Location extends EM_Object {
	...
	function __construct( $id = false, $search_by = 'location_id' ) {
		...
		$this->required_fields = array(
			'location_address' => __( 'The location address', 'events-manager' ),
			'location_town'    => __( 'The location town', 'events-manager' ),
			'location_country' => __( 'The country', 'events-manager' ),
		);
	...
	function get_location_meta( $blog_id = false ) {
		if ( ! empty( $this->blog_id ) ) {
			$blog_id = $this->blog_id; //if there's a blog id already, there's no doubt where to look for
		}
	...
	function save_meta() {
		...
		if ( $this->can_manage( 'edit_locations', 'edit_others_locations' ) || ( em_get_option( 'dbem_events_anonymous_submissions' ) && empty( $this->location_id ) ) ) {
			...
			//language default
			if ( ! $this->location_language ) {
				$this->location_language = EM_ML::$current_language;
			}
			...
			foreach ( array_keys( $this->fields ) as $key ) {
				if ( ! EM_ML::$is_ml && ( $key == 'location_language' || $key == 'location_translation' ) ) {
					continue;
				}
		...
		if ( $this->can_manage( 'edit_locations', 'edit_others_locations' ) ) {
			...
			if ( $EM_Location->save() ) {
				...
				foreach ( $location_meta as $location_meta_key => $location_meta_vals ) {
					if ( $location_meta_key == '_wpas_' ) {
						continue; //allow JetPack Publicize to detect this as a new post when published
					}
				...
				if ( array_key_exists( '_location_approvals_count', $location_meta ) ) {
					update_post_meta( $EM_Location->post_id, '_location_approvals_count', 0 );
				}
	...
	function duplicate_url( $raw = false ) {
		$url = add_query_arg(
			array(
				'action'      => 'location_duplicate',
				'location_id' => $this->location_id,
				'_wpnonce'    => wp_create_nonce( 'location_duplicate_' . $this->location_id ),
			)
		);
		...
		if ( $this->can_manage( 'edit_locations', 'edit_others_locations' ) ) {
			if ( EM_MS_GLOBAL ) {
				...
				if ( get_site_option( 'dbem_ms_mainblog_locations' ) ) {
					...
					if ( get_blog_option( $this->blog_id, 'dbem_edit_locations_page' ) && ! is_admin() ) {
						$link = em_add_get_params(
							get_blog_permalink( $this->blog_id, get_blog_option( $this->blog_id, 'dbem_edit_locations_page' ) ),
							array(
								'action'      => 'edit',
								'location_id' => $this->location_id,
							),
					...
					if ( ( is_main_site() || empty( $link ) ) && get_blog_option( $current_site->blog_id, 'dbem_edit_locations_page' ) && ! is_admin() ) { //if editing on main site and edit page exists, stay on same site
						$link = em_add_get_params(
							get_blog_permalink( em_get_option( 'dbem_edit_locations_page' ) ),
							array(
								'action'      => 'edit',
								'location_id' => $this->location_id,
							),
					...
					if ( ( is_main_site() || empty( $link ) ) && get_blog_option( $current_site->blog_id, 'dbem_edit_locations_page' ) && ! is_admin() ) { //if editing on main site and edit page exists, stay on same site
						$link = em_add_get_params(
							get_blog_permalink( $current_site->blog_id, get_blog_option( $current_site->blog_id, 'dbem_edit_locations_page' ) ),
							array(
								'action'      => 'edit',
								'location_id' => $this->location_id,
							),
				...
				if ( em_get_option( 'dbem_edit_locations_page' ) && ! is_admin() ) {
					$link = em_add_get_params(
						get_permalink( em_get_option( 'dbem_edit_locations_page' ) ),
						array(
							'action'      => 'edit',
							'location_id' => $this->location_id,
						),
				...
				case '#_LOCATIONEXCERPTCUT':
					if ( ! empty( $this->post_excerpt ) && $result != '#_LOCATIONEXCERPTCUT' ) {
						...
						if ( ! empty( $placeholders[3][ $key ] ) ) {
							...
							if ( is_numeric( $ph_args[0] ) || empty( $ph_args[0] ) ) {
	...
	function get_full_address( $glue = ', ', $include_country = false ) {
		...
		if ( ! empty( $this->location_address ) ) {
			$location_array[] = $this->location_address;
		}
		if ( ! empty( $this->location_town ) ) {
			$location_array[] = $this->location_town;
		}
		if ( ! empty( $this->location_state ) ) {
			$location_array[] = $this->location_state;
		}
		if ( ! empty( $this->location_postcode ) ) {
			$location_array[] = $this->location_postcode;
		}
		if ( ! empty( $this->location_region ) ) {
			$location_array[] = $this->location_region;
		}
		if ( $include_country ) {
			$location_array[] = $this->get_country();
		}
	...
	function get_google_maps_embed_url() {
		...
		$args   = apply_filters(
			'em_location_google_maps_embed_args',
			array(
				'maptype' => 'roadmap',
				'zoom'    => 15,
