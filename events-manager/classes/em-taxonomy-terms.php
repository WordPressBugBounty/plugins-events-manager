...
class EM_Taxonomy_Terms extends EM_Object implements Iterator, Countable {
		...
		if ( is_object( $data ) && $data instanceof EM_Event && ! empty( $data->post_id ) ) { //Creates a blank taxonomies object if needed
			...
			if ( EM_MS_GLOBAL && $this->is_ms_global && ! is_main_site( $data->blog_id ) ) {
				...
				if ( EM_MS_GLOBAL && ( get_current_blog_id() !== $data->blog_id || ( ! $data->blog_id && ! is_main_site() ) ) ) {
					if ( ! $this->blog_id ) {
						$this->blog_id = get_current_site()->blog_id;
					}
	...
	public function save_index() {
		...
		foreach ( $this->terms as $EM_Taxonomy_Term ) {
			$wpdb->insert(
				EM_META_TABLE,
				array(
					'meta_value' => $EM_Taxonomy_Term->term_id,
	...
	public function has( $search ) {
		if ( is_numeric( $search ) ) {
			foreach ( $this->terms as $EM_Taxonomy_Term ) {
				if ( $EM_Taxonomy_Term->term_id == $search ) {
					return apply_filters( 'em_' . $this->terms_name . '_has', true, $search, $this );
				}
			}
		} else {
			foreach ( $this->terms as $EM_Taxonomy_Term ) {
				if ( $EM_Taxonomy_Term->slug == $search || $EM_Taxonomy_Term->name == $search ) {
					return apply_filters( 'em_' . $this->terms_name . '_has', true, $search, $this );
				}
			}
		}
	...
	public static function output( $args = array() ) {
		...
		if ( ! empty( $EM_Taxonomy_Term_old ) ) {
			$GLOBALS[ self::$instance->term_class ] = $EM_Taxonomy_Term_old;
		}
	...
	public static function get_pagination_links( $args, $count, $search_action = false, $default_args = array() ) {
		...
		if ( $search_action === false ) {
			$search_action = self::$instance->ajax_search_action;
		}
	...
	public static function get_default_search( $array_or_defaults = array(), $array = array() ) {
		$defaults = array(
			...
			'orderby'      => em_get_option( 'dbem_' . self::$instance->terms_name . '_default_orderby' ),
			'order'        => em_get_option( 'dbem_' . self::$instance->terms_name . '_default_order' ),
			'hide_empty'   => false,
			'exclude'      => array(),
			'exclude_tree' => array(),
			'include'      => array(),
		...
		if ( is_array( $return['orderby'] ) ) {
			$return['orderby'] = implode( ',', $return['orderby'] ); //clean up for WP functions
		}
