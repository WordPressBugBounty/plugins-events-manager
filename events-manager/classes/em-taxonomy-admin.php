...
class EM_Taxonomy_Admin {
	...
	public static function form_edit( $tag ) {
		...
		<tr class="form-field term-color-wrap">
			...
			<td>
				...
				<p class="description"><?php echo sprintf( __( 'Choose a color for your %1$s. You can access this using the %2$s placeholder.', 'events-manager' ), __( self::$name_singular, 'events-manager' ), '<code>' . self::$placeholder_color . '</code>' ); ?></p>
		...
		<tr class="form-field term-image-wrap">
			...
			<td>
				...
				<p class="hide-if-no-js">
					...
					<input id="delete_image_button" type="button" value="<?php _e( 'Remove Image', 'events-manager' ); ?>" class="delete-img-button button-secondary" 
					<?php
				...
				<br >
				<p class="description"><?php echo sprintf( __( 'Choose an image for your %1$s, which can be displayed using the %2$s placeholder.', 'events-manager' ), __( self::$name_singular, 'events-manager' ), '<code>' . self::$placeholder_image . '</code>' ); ?></p>
			</td>
		</tr>
		<?php
	}
	...
	public static function form_add() {
		...
		<div class="term-color-wrap">
			...
			<input type="text" name="term_color" id="term-color" class="term-color" value="<?php echo esc_attr( em_get_option( 'dbem_' . static::$option_name . '_default_color', '#80b538' ) ); ?>" ><br>
			<p class="description"><?php echo sprintf( __( 'Choose a color for your %1$s. You can access this using the %2$s placeholder.', 'events-manager' ), __( self::$name_singular, 'events-manager' ), '<code>' . self::$placeholder_color . '</code>' ); ?></p>
		</div>
			...
			if ( $color ) {
				...
				if ( count( $prev_settings ) > 0 ) {
					$wpdb->update(
						...
						array(
							'object_id' => $term_id,
							'meta_key'  => self::$option_name . '-bgcolor',
						)
					);
				} else {
					$wpdb->insert(
		...
		if ( ! empty( $_POST['term_image'] ) ) {
			...
			if ( count( $prev_settings ) > 0 ) {
				$wpdb->update(
					...
					array(
						'object_id'  => $term_id,
						'meta_value' => $term_image,
					),
					array(
						'object_id' => $term_id,
						'meta_key'  => self::$option_name . '-image',
					)
				);
			} else {
				$wpdb->insert(
			...
			if ( ! empty( $_POST['term_image_id'] ) && is_numeric( $_POST['term_image_id'] ) ) {
				...
				if ( count( $prev_settings ) > 0 ) {
					$wpdb->update(
						...
						array(
							'object_id'  => $term_id,
							'meta_value' => $term_image_id,
						),
						array(
							'object_id' => $term_id,
							'meta_key'  => self::$option_name . '-image-id',
						)
					);
				} else {
					$wpdb->insert(
