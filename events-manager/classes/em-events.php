...
class EM_Events extends EM_Object {
	...
	public static function get( $args = array(), $count = false ) {
		...
		if ( ! empty( $args['groupby'] ) || ( defined( 'EM_DISABLE_OPTIONAL_JOINS' ) && EM_DISABLE_OPTIONAL_JOINS ) ) {
			...
			if ( ! $join_locations ) {
				foreach ( array( 'groupby', 'orderby', 'groupby_orderby' ) as $arg ) {
					if ( ! is_array( $args[ $arg ] ) ) {
						continue; //ignore this argument if set to false
					}
			...
			$join_locations = true; }//end temporary if( !empty($args['groupby']).... wrapper
...
$orderby_sql";
	...
	public static function build_sql_conditions( $args = array() ) {
		...
		if ( em_get_option( 'dbem_locations_enabled' ) ) {
			...
			if ( ! empty( $args['has_location'] ) ) {
				...
				foreach ( $location_specific_args as $location_arg ) {
					if ( ! empty( $args[ $location_arg ] ) ) {
						$skip_location_null_condition = true;
					}
				}
		...
		if ( ! empty( $args['search'] ) ) {
			...
			foreach ( $like_search as $v ) {
				$like_search_strings[] = $like_search_string;
			}
	...
	public static function build_sql_orderby( $args, $accepted_fields, $default_order = 'ASC' ) {
		...
		if ( $args['timeslots'] ) {
			foreach ( $orderby as $key => $value ) {
				$orderby[ $key ] = str_replace( array( 'event_start_date', 'event_start_time', 'event_start' ), 'event_start, timeslot_start', $value );
				$orderby[ $key ] = str_replace( array( 'event_end_date', 'event_end_time', 'event_end' ), 'event_end, timeslot_end', $orderby[ $key ] );
