...
class EM_Admin_Notices {
	...
	public static function add( $EM_Admin_Notice, $network = false ) {
		...
		if ( ! is_array( $notices_data ) ) {
			$notices_data = array(); //we store the data regarldess of whether a message will require a hook, since it contains location and caps considtions
		}
		//start building data
	 ...
	 */
	public static function network_add( $EM_Admin_Notice ) {
		return self::add( $EM_Admin_Notice, true ); }
	 ...
	 */
	public static function network_remove( $notice_key ) {
		return self::remove( $notice_key, true ); }
	...
	public static function dismiss_admin_notice() {
		...
		if ( empty( $_REQUEST['nonce'] ) || ! wp_verify_nonce( $_REQUEST['nonce'], $_REQUEST['action'] . $_REQUEST['notice'] . get_current_user_id() ) ) {
			die( 'Invalid Nonce' );
		}
		$key     = $_REQUEST['notice'];
