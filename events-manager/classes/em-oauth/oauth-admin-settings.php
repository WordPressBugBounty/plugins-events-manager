...
class OAuth_API_Admin_Settings {
	...
	public static function em_settings_save_apps() {
		...
		if ( $api::get_option_dataset() == 'dbem_oauth' ) {
			return;
		}
	...
	public static function em_settings_apps() {
		$desc             = esc_html__( "You'll need to create an App with %1\$s and obtain the App credentials by going to %2\$s.", 'events-manager' );
		$desc_url         = esc_html__( 'You will also need supply an OAuth redirect url to %1$s. Your url for this site is : %2$s', 'events-manager' );
		...
		try {
			...
			if ( ! is_wp_error( $api_client ) ) { //oauth is not configured correctly...
				...
				foreach ( $access_tokens as $account_id => $oauth_account ) {
					...
					$disconnect_url_args         = array(
						'action'   => 'em_oauth_' . $api::get_option_name(),
						'callback' => 'disconnect',
						'account'  => $account_id,
						'nonce'    => wp_create_nonce( 'em-oauth-' . $option_name . '-disconnect-' . $account_id ),
					);
				...
				if ( $connected ) {
					$button_url   = add_query_arg(
						array(
							'action'   => 'em_oauth_' . $option_name,
							'callback' => 'disconnect',
							'nonce'    => wp_create_nonce( 'em-oauth-' . $option_name . '-disconnect' ),
						),
						...
						<?php foreach ( $oauth_accounts as $oauth_account ) : ?>
							<li class="em-oauth-service-account em-oauth-account-<?php echo empty( $oauth_account['reconnect'] ) ? 'connected' : 'disconnected'; ?>">
								...
								<span class="em-oauth-account-label">
									<?php
									if ( ! empty( $oauth_account['reconnect'] ) ) :
										?>
