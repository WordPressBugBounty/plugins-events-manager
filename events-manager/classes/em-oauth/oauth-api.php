...
class OAuth_API {
	...
	public static function get_client_class() {
		if ( static::$client_class !== null && class_exists( static::$client_class ) ) {
	...
	public static function get_token_class() {
		if ( static::$token_class !== null && class_exists( static::$token_class ) ) {
	...
	public static function get_site_tokens() {
		if ( static::$authorization_scope !== 'site' ) {
			return array();
		}
		...
		if ( ! empty( $_REQUEST['code'] ) ) {
			try {
				...
				if ( $client->oauth_state && ( empty( $_REQUEST['state'] ) || ! wp_verify_nonce( $_REQUEST['state'], static::$option_name . '_authorize' ) ) ) {
					$EM_Notices->add_error( sprintf( esc_html__( 'There was an error connecting to %1$s: %2$s', 'events-manager' ), static::$service_name, '<code>No State Provided</code>' ), true );
				} else {
					try {
						...
						$EM_Notices->add_error( sprintf( esc_html__( 'There was an error connecting to %1$s: %2$s', 'events-manager' ), static::$service_name, '<code>' . $e->getMessage() . '</code>' ), true );
					}
				}
			...
			$EM_Notices->add_error( sprintf( esc_html__( 'There was an error connecting to %1$s: %2$s', 'events-manager' ), static::$service_name, '<code>No Authorization Code Provided</code>' ), true );
		}
...
//include dependents
require 'oauth-api-token.php';
require 'oauth-api-client.php';
if ( is_admin() ) {
