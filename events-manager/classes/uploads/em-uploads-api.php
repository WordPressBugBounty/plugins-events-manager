...
class API {
	...
	public static function init() {
		...
		add_action( 'init', array( static::class, 'schedule_cleanup' ) );
		add_action( 'em_uploads_api_cleanup', array( static::class, 'run_cleanup' ) );
	}
	...
	public static function register_routes() {
		...
		register_rest_route(
			...
			array(
				'methods'             => 'DELETE',
				'callback'            => array( static::class, 'handle_revert' ),
				'permission_callback' => '__return_true',
			)
		);
		...
		register_rest_route(
			...
			array(
				'methods'             => 'GET',
				'callback'            => array( static::class, 'handle_load' ),
	...
	public static function find_nested_upload_array( array $array ) {
		$expected_keys = array( 'tmp_name', 'name', 'error', 'size', 'type' );
	...
	public static function handle_upload( $request ) {
		...
		if ( is_array( $_FILES[ $upload_key ]['tmp_name'] ) ) {
			...
			foreach ( $_FILES[ $upload_key ] as $key => $value ) {
				...
				$_FILES[ $upload_key ][ $key ] = array( $file[ $key ] ); // for later, 'flattened' as a simple field uploaded
			}
		} else {
			$file = $_FILES[ $upload_key ];
			foreach ( $_FILES[ $upload_key ] as $key => $value ) {
				$_FILES[ $upload_key ][ $key ] = array( $value );
			}
		}
		...
		$validate_options = apply_filters( 'em_uploads_api_upload_validate_options_' . $path, array( 'type' => array( 'image' ) ), $upload_key, $file );
		...
		if ( ! $valid ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'error'   => 'Failed to validate the uploaded file.',
				),
				422
			);
		...
		if ( ! move_uploaded_file( $file['tmp_name'], $file['tmp_name'] . Uploader::$temp_suffix ) ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'error'   => 'Failed to temporarily store the uploaded file.',
				),
				422
			);
		}
		...
		if ( ! wp_verify_nonce( $nonce, 'em_uploads_api_file/' . $nonce_action ) ) {
			return new WP_REST_Response(
				array(
					'success' => false,
					'success' => false,
					'error'   => 'File not deleted, failed nonce.',
				),
				200
			);
		}
