...
class Uploader {
	...
	public static $supported_file_types = array(
		...
		'doc'  => array(
			'mime' => array( 'application/msword', 'application/x-msword' ),
			'type' => 'document',
		),
		'docx' => array(
			'mime' => array( 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ),
			'type' => 'document',
		),
		'rtf'  => array(
			'mime' => array( 'text/rtf', 'application/rtf', 'application/x-rtf' ),
			'type' => 'document',
		),
		'odt'  => array(
		...
		// Spreadsheets
		'xls'  => array(
			'mime' => array( 'application/vnd.ms-excel', 'application/xls' ),
			'type' => 'spreadsheet',
		),
		'xlsx' => array(
		...
		'csv'  => array(
			'mime' => array( 'text/csv', 'application/csv', 'text/plain' ),
			'type' => 'spreadsheet',
		),
		'ods'  => array(
		...
		// Presentations
		'ppt'  => array(
			'mime' => array( 'application/vnd.ms-powerpoint', 'application/mspowerpoint' ),
			'type' => 'presentation',
		),
		'pptx' => array(
	...
	public static function init() {
		...
		static::$default_options['extensions']     = ! empty( $extensions ) ? explode( ',', $extensions ) : array();
	...
	public static function get_options( array $options = array() ) {
		...
		$preserve_keys = array( 'max_file_size', 'image_max_width', 'image_max_height', 'image_min_width', 'image_min_height' );
		...
		$merged_options['extensions'] = $merged_options['extensions'] ?: array();
		if ( ! is_array( $merged_options['extensions'] ) ) {
			$merged_options['extensions'] = explode( ',', str_replace( ' ', '', $merged_options['extensions'] ) );
		}
		$merged_options['extensions'] = array_map( 'strtolower', $merged_options['extensions'] );
		$merged_options['type']       = $merged_options['type'] ?: array();
		if ( ! is_array( $merged_options['type'] ) ) {
			$merged_options['type'] = explode( ',', str_replace( ' ', '', $merged_options['type'] ) );
		}
	 ...
	 */
	public static function get_accepted_mime_types( $options = array() ) {
	 ...
	 */
	public static function prepare( $file_key, $data = array() ) {
		...
		if ( ! empty( $_FILES[ $files_key ] ) ) {
			...
			if ( ! is_array( $_FILES[ $files_key ]['name'] ) ) {
				foreach ( $_FILES[ $files_key ] as $key => $val ) {
					$_FILES[ $files_key ][ $key ] = array( $val );
				}
			}
			return true;
		}
		...
		if ( ! empty( $REQUEST[ $file_key ] ) ) {
			if ( ! is_array( $REQUEST[ $file_key ] ) ) {
				$REQUEST[ $file_key ] = array( $REQUEST[ $file_key ] );
			}

			$_FILES[ $files_key ] = array(
				'name'     => array(),
				'type'     => array(),
				'tmp_name' => array(),
				'error'    => array(),
			...
			foreach ( $REQUEST[ $file_key ] as $file_id ) {
				...
				if ( preg_match( '/^https?:\/\//', $file_id ) ) {
					continue; // ignore URLs as they are already uploaded items
				}
	 ...
	 */
	public static function validate( $file_key, $options = array(), $data = array() ) {
		// prepare the files
			...
			if ( empty( $single_file['uploaded'] ) ) {
				...
				if ( $extension_str && array_key_exists( $extension_str, $permitted_extensions ) ) {
					...
					if ( $extension['type'] === 'image' ) {
						...
						$file_errors = apply_filters(
							'em_uploads_uploader_validate_extension_' . $extension_str,
							array(),
							$single_file,
							array(
								'file_key'  => $file_key,
	...
	public static function upload( $file_key, $destination = false, $file_name = false ) {
		...
		if ( ! empty( $destination ) ) {
			...
			add_filter( 'upload_dir', array( static::class, 'upload_dir' ), 100 );
		}

		// upload files
		$uploaded_files = $uploads = array();
		foreach ( $files as $fn => $file ) {
			$result = wp_handle_upload(
				$file,
				array(
					'test_form' => false,
					'action'    => 'em_handle_upload',
				)
			);
			/* Attach file to item */
		...
		if ( ! empty( $destination ) ) {
			...
			remove_filter( 'upload_dir', array( static::class, 'upload_dir' ), 100 );
		}
	...
	public static function post_image_uploader( $EM_Object, $path, $name ) {
		$path     = esc_attr( $path );
		$name     = esc_attr( $name );
		$accepted = implode( ', ', Uploader::get_accepted_mime_types( array( 'type' => 'image' ) ) );
		?>
...
add_filter( 'events_manager_loaded', array( 'EM\Uploads\Uploader', 'init' ), 1 );