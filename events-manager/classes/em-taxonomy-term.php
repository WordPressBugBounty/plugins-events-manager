...
class EM_Taxonomy_Term extends EM_Object {
	...
	function has_events( $status = 1, $args = array() ) {
		$events_count = EM_Events::count(
			array_merge(
				array(
					$this->option_name => $this->term_id,
					'scope'            => 'future',
					'status'           => $status,
			...
			foreach ( $conditionals[1] as $key => $condition ) {
				...
				foreach ( EM\Archetypes::get_cpts( array(), array( 'event', 'types' ) ) as $cpt ) {
					if ( $condition == 'has_events_' . $cpt ) {
						//does this tax have any upcoming events
						$show_condition = $this->has_events( array( 'event_archetype' => $cpt ) );
					} elseif ( $condition == 'no_events_' . $cpt ) {
						//does this tax NOT have any upcoming events?
						$show_condition = $this->has_events( array( 'event_archetype' => $cpt ) ) == false;
					}
				}
		...
		foreach ( $placeholders[1] as $key => $result ) {
			...
			switch ( $result ) {
				...
				case '#_' . $ph . 'ALLEVENTS':
					...
					if ( $result == '#_' . $ph . 'PASTEVENTS' ) {
						$scope = 'past'; } elseif ( $result == '#_' . $ph . 'NEXTEVENTS' ) {
						$scope = 'future'; } else {
												$scope = 'all'; }
						$args                   = array(
							$this->option_name => $this->term_id,
