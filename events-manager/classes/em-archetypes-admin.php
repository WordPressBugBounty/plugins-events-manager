...
class Archetypes_Admin {
	...
	public static function init() {
		add_action( 'em_options_save', array( static::class, 'em_options_save' ) );
		add_action( 'admin_footer', array( static::class, 'admin_footer' ) );
	}
		...
		try {
			if ( defined( 'EM_SETTINGS_ARCHETYPES_MERGED' ) && EM_SETTINGS_ARCHETYPES_MERGED || Archetypes::get_current() === Archetypes::$event['cpt'] ) {
				...
				if ( is_multisite() && ! is_network_admin() && Archetypes::get_ms_mode() === 'choose' ) {
					$available = array_keys( (array) get_site_option( 'em_ms_event_archetypes', array() ) );
					// Include base event CPT as a valid choice as well
					$available[] = Archetypes::$event['cpt'];
					$available   = array_unique( $available );
					$selected    = isset( $_POST['em_archetypes_selected'] ) && is_array( $_POST['em_archetypes_selected'] ) ? array_map( 'sanitize_key', $_POST['em_archetypes_selected'] ) : array( EM_POST_TYPE_EVENT );
			...
			if ( ! empty( $_POST['em_archetype_options'] ) ) {
				if ( defined( 'EM_SETTINGS_ARCHETYPES_MERGED' ) && EM_SETTINGS_ARCHETYPES_MERGED ) {
					...
					$archetypes_options = array();
				} else {
					// just saving current archetype
					$the_archetype                        = Archetypes::get_current();
					$archetypes_options                   = get_option( 'em_event_archetypes_options', array() );
					$archetypes_options[ $the_archetype ] = array();
				}
				foreach ( $_POST['em_archetype_options'] as $type => $post_options ) {
					if ( ! empty( $the_archetype ) && $type !== $the_archetype ) {
						continue; // skip in single mode
					}
	...
	public static function save_main_archetype( $base = 'events' ) {
		...
		foreach ( array( "em_cp_{$base}_cpt", "em_cp_{$base}_cpts" ) as $wp_option_key ) {
	...
	public static function save_archetype( $type, $data ) {
		$archetype = Archetypes::$types[ $type ] ?? array();
		$new       = empty( $archetype );
		if ( $data['cpt'] ) {
			if ( $new || ( $data['cpt'] !== $archetype['cpt'] && array( 'cpt_nonce' ) && wp_verify_nonce( $data['cpt_nonce'], 'edit_cpt_' . $type ) ) ) {
	...
	public static function validate_archetype( $data ) {
		$required = array( 'cpt', 'cpts', 'slug', 'label', 'label_single' );
	...
	public static function change_cpt( $current, $new, $base = 'events' ) {
		...
		if ( ! in_array( $new, $post_types ) ) {
			...
			if ( $post_type === $new ) {
				...
				global $wpdb;
				$wpdb->update( $wpdb->posts, array( 'post_type' => $post_type ), array( 'post_type' => $current ) );
				...
				if ( $base === 'events' ) {
					$wpdb->update( EM_EVENTS_TABLE, array( 'event_archetype' => $post_type ), array( 'event_archetype' => $current ) );
				}
			} else {
			...
			throw new Exception( sprintf( __( 'You cannot change the CPT type of %1$s to %2$s. This custom post type already exists.', 'events-manager' ), "<code>{$new}</code>", "<code>{$_POST['em_cp_events_cpts']}</code>" ), true );
		}
	}
	...
	public static function get_overrideable_options() {
		if ( empty( static::$options ) ) {
			$options = apply_filters(
				'em_archetypes_overrideable_options',
				array(
					// General
