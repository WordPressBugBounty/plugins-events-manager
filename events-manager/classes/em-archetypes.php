...
class Archetypes {
	...
	public static function init() {
		...
		add_filter( 'map_meta_cap', array( static::class, 'map_meta_cap' ), 10, 4 );
		// add thumbnail support to theme
		add_action( 'after_setup_theme', array( static::class, 'after_setup_theme' ), 100 );
		...
		if ( get_option( 'dbem_locations_enabled', true ) ) {
			$location_cpt_supports = array( 'title', 'editor', 'excerpt', 'thumbnail', 'author' );
			if ( get_option( 'dbem_cp_locations_custom_fields' ) ) {
				$location_cpt_supports[] = 'custom-fields';
			}
			if ( get_option( 'dbem_cp_locations_comments' ) ) {
				$location_cpt_supports[] = 'comments';
			}
		...
		if ( ! defined( 'EM_POST_TYPE_LOCATION' ) ) {
			define( 'EM_POST_TYPE_LOCATION', Archetypes::$location['cpt'] ?? 'location' );
		}
		define( 'EM_POST_TYPE_LOCATION_SLUG', Archetypes::$location['slug'] ?? 'locations' );

		// Deal with Gutenberg
		if ( defined( 'EM_GUTENBERG' ) && EM_GUTENBERG ) {
			add_filter( 'gutenberg_can_edit_post_type', array( static::class, 'gutenberg_can_edit_post_type' ), 10, 2 ); //Gutenberg
		}
		...
		if ( static::$enabled ) {
			...
			if ( $source === 'network' ) {
				$archetypes = (array) get_site_option( 'em_ms_event_archetypes', array() );
			} else {
				$archetypes = (array) get_option( 'em_event_archetypes', array() );
			}
		...
		if ( ! defined( 'EM_POST_TYPE_EVENT' ) ) {
			define( 'EM_POST_TYPE_EVENT', $event_cpt );
		}
		$event_slug = defined( 'EM_POST_TYPE_EVENT_SLUG' ) ? EM_POST_TYPE_EVENT_SLUG : static::$event['slug'];
		if ( ! defined( 'EM_POST_TYPE_EVENT_SLUG' ) ) {
			define( 'EM_POST_TYPE_EVENT_SLUG', $event_slug );
		}
		...
		add_action( 'em_taxonomies_init', array( static::class, 'em_taxonomies_init' ) );

		if ( is_admin() && strstr( $_REQUEST['page'] ?? '', 'events-manager-' ) ) {
			include EM_DIR . '/classes/em-archetypes-admin.php';
		}

		add_action( 'init', array( static::class, 'register_post_types' ), 1 );

		// as JS for editor
		add_filter( 'em_enqueue_assets', array( static::class, 'em_enqueue_assets' ) );
	}
	...
	public static function em_enqueue_assets( $assets ) {
		...
		$assets['#em-opt-archetypes'] = array(
			'js' => array(
	...
	public static function get_options() {
		if ( empty( static::$options ) ) {
			static::$options = get_option( 'em_event_archetypes_options', array() );
		}
	...
	public static function get_selected_archetypes() {
		...
		$selected = get_option( 'em_archetypes_selected', array() );
	...
	public static function get_default_post_type() {
		if ( ! static::$base ) {
			$event_cpt_supports = array( 'title', 'editor', 'excerpt', 'thumbnail', 'author' );
			if ( get_option( 'dbem_cp_events_custom_fields' ) ) {
				$event_cpt_supports[] = 'custom-fields';
			}
			if ( get_option( 'dbem_cp_events_comments' ) ) {
				$event_cpt_supports[] = 'comments';
			}

			// merge $type with defaults which is the $event
			static::$base = apply_filters(
				'em_ct__default',
				array( // __ not a typo, made to avoid conflicts
					'public'              => true,
					'hierarchical'        => false,
					'show_ui'             => true,
					'show_in_menu'        => true,
	...
	public static function get_default_post_type_repeating( $archetype ) {
		...
		$repeating_base = array( // specifically keyed event_recurring to trigger legacy filter em_cpt_event_recurring
	...
	public static function create_archetype_cpt( $type ) {
		...
		if ( empty( $type['capability_type'] ) ) {
			$archetype['capability_type'] = array( 'event', 'events' ); // default to event for now so we use general caps;
	...
	public static function register_post_types() {
		...
		$post_types = array( static::$event['cpt'] => static::$event );
		if ( static::$location ) {
	...
	public static function generate_capabilities( $archetype ) {
		if ( empty( $archetype['capability_type'] ) || ! is_array( $archetype['capability_type'] ) ) {
			$cap = array( $archetype['cpt'], $archetype['cpts'] ?? $archetype['cpt'] );
		} else {
	...
	public static function map_meta_cap_repeating( $c, $archetype ) {
		if ( ! empty( $archetype['repeating'] ) ) {
			...
			if ( ! is_array( $repeating ) ) {
				$suffix    = $archetype['cpt'] === EM_POST_TYPE_EVENT ? '-recurring' : '-repeating';
				$repeating = array( // specifically keyed event_recurring to trigger legacy filter em_cpt_event_recurring
					'cpt'             => $archetype['cpt'] . $suffix,
					'capability_type' => array( 'event', 'events' ),
				);
			}
	...
	public static function get_cpts( $exclude = array(), $include = array() ) {
		$valid_cpts = array();
	...
	public static function is_valid_cpt( $cpt, $exclude = array(), $include = array() ) {
	...
	public static function is_event( $cpt, $include_repeating = true ) {
		...
		if ( $include_repeating ) {
			// check repeating too
			return $cpt && in_array( $cpt, static::get_cpts( array( 'location' ) ) );
		} else {
			return $cpt && in_array( $cpt, array( static::$event['cpt'], ...array_keys( static::$types ) ) );
		}
	}
	...
	public static function is_repeating( $cpt ) {
		$cpt = static::get_post_type( $cpt );
		return $cpt && in_array( $cpt, static::get_cpts( array(), array( 'repeating' ) ) );
	}
