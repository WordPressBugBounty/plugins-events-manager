...
class Timerange extends EM_Object {
	...
	public $fields = array(
		...
		'timerange_group_id' => array(
			'type' => '%s',
			'null' => false,
		),
		'timerange_start'    => array(
			'type' => '%s',
			'null' => false,
		),
		'timerange_end'      => array(
		...
		'timerange_all_day'  => array(
			'type' => '%d',
			'null' => false,
		),
		'timeslot_frequency' => array(
			'type' => '%s',
			'null' => true,
		),
		'timeslot_buffer'    => array(
			'type' => '%s',
			'null' => true,
		),
	...
	public function __construct( $data = false ) {
		...
		// Type casting
		if ( $this->timerange_id ) {
			$this->timerange_id = absint( $this->timerange_id );
		}
	...
	public function get_post( $post ) {
		...
		if ( isset( $post['all_day'] ) ) {
			...
			foreach ( array( 'start', 'end' ) as $timeName ) {
				$match = array();
		...
		if ( ! empty( $post['timeslots'] ) ) {
			foreach ( array( 'frequency', 'buffer', 'duration' ) as $field ) {
				if ( isset( $post[ $field ] ) ) {
	...
	public function validate() {
		...
		foreach ( array( 'timeslot_frequency', 'timeslot_buffer', 'timeslot_duration' ) as $field ) {
	...
	public function has_timeslots() {
		...
		return count( $timeslots ?? array() ) > 1;
	}
	...
	public function generate_timeslots_data() {
		...
		if ( $endMinutes >= $startMinutes ) {
			...
			if ( $currentStart == $endMinutes ) {
				...
				if ( $interval <= 0 ) {
					$interval = $endMinutes; // prevent infinite loop
				}
	...
	protected function intervalTimestamp( $interval ) {
		...
		if ( is_numeric( $interval ) ) {
			return intval( $interval ); // for simple numbers
		}
		$totalSeconds = 0;
		...
		foreach ( $matches as $m ) {
			...
			switch ( $unit ) {
				...
				case 'S':
					...
					break;
				default:
					$totalSeconds += $value * 60; // fallback to minutes
			}
		}
