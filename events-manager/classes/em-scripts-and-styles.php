...
class Scripts_and_Styles {
	...
	public static function inline_enqueue() {
		...
		if ( em_get_option( 'dbem_css_theme' ) ) {
			$css = array();
			if ( em_get_option( 'dbem_css_theme_font_family' ) == 1 ) {
				$css[] = '--font-family : inherit;';
			}
			if ( em_get_option( 'dbem_css_theme_font_weight' ) == 1 ) {
				$css[] = '--font-weight : inherit;';
			}
			if ( em_get_option( 'dbem_css_theme_font_size' ) == 1 ) {
				$css[] = '--font-size : 1em;';
			}
			if ( em_get_option( 'dbem_css_theme_line_height' ) == 1 ) {
	...
	public static function admin_enqueue( $hook_suffix = false ) {
		if ( $hook_suffix == 'post.php' || $hook_suffix === true || ( ! empty( $_GET['page'] ) && substr( $_GET['page'], 0, 14 ) == 'events-manager' ) || ( ! empty( $_GET['post_type'] ) && in_array( $_GET['post_type'], Archetypes::get_cpts() ) ) ) {
			if ( $hook_suffix == 'post.php' && empty( $_GET['post_type'] ) && ! empty( $_GET['post'] ) ) {
				...
				if ( ! in_array( $post->post_type, Archetypes::get_cpts() ) ) {
					return;
				}
			}
			static::register();
	...
	public static function get_minified_extension_css( $minified = true ) {
		if ( ! em_get_option( 'dbem_css_minified', false ) ) {
			return ''; // force non-minified file for now, because AVAST AVG is giving a false positive and wreaking havoc
		}
		return static::get_minified_extension( $minified );
	}

	public static function get_minified_extension_js( $minified = true ) {
		if ( ! em_get_option( 'dbem_js_minified', false ) ) {
			return ''; // force non-minified file for now, because AVAST AVG is giving a false positive and wreaking havoc
		}
		return static::get_minified_extension( $minified );
	}
	...
	public static function localize_script( $script = 'events-manager' ) {
		...
		*/
		...
		if ( em_get_option( 'dbem_uploads_ui' ) ) {
			$em_localized_js['assets']['input.em-uploader'] = array(
				...
				'js'  => array(
					...
					'filepond'                            => array(
						'url'    => 'filepond/filepond' . $js,
						'locale' => preg_match( '/^en/i', EM_ML::$wplang ) ? '' : strtolower( str_replace( '_', '-', EM_ML::$wplang ) ),
					),
		...
		$em_localized_js['assets']['.em-event-editor'] = array(
			'js'  => array(
				'event-editor' => array(
					'url'   => $js_url . 'events-manager-event-editor' . $js,
					'event' => 'em_event_editor_ready',
				),
			),
			'css' => array(
		...
		$em_localized_js['assets']['.em-recurrence-sets, .em-timezone'] = array(
			'js' => array(
				'luxon' => array(
					'url'   => 'luxon/luxon' . $js,
		...
		// booking form loader
		$em_localized_js['assets']['.em-booking-form, #em-booking-form, .em-booking-recurring, .em-event-booking-form'] = apply_filters(
			'em_booking_form_assets',
			array(
				'js' => array(
					'em-bookings' => array(
						'url'   => $js_url . 'bookingsform' . $js,
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			$em_localized_js['ui_css'] = plugins_url( 'includes/css/jquery-ui/build.css', EM_FILE );
		}
		...
		if ( is_admin() ) {
			$em_localized_js['event_post_types']   = Archetypes::get_cpts( array( 'location' ) ); // all but location
	...
	public static function localize_script_footer() {
		if ( ! wp_script_is( 'events-manager', 'done' ) ) {
			return;
		}
