...
class Consent {
	...
	public static function init() {
		if ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX && ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] != 'booking_add_one' ) ) {
			add_action( 'init', array( static::class, 'hooks' ) );
		}
	...
	public static function hooks() {
		...
		if ( em_get_option( 'dbem_data_' . static::$prefix . '_bookings' ) == 1 || ( em_get_option( 'dbem_data_' . static::$prefix . '_bookings' ) == 2 && ! is_user_logged_in() ) ) {
			add_action( 'em_booking_form_footer', array( static::class, 'checkbox' ), 9, 0 ); //supply 0 args since arg is $EM_Event and callback will think it's an event submission form
			add_action( 'em_booking_form_after_user_details', array( static::class, 'checkbox' ), 9, 0 ); //supply 0 args since arg is $EM_Event and callback will think it's an event submission form
		...
		if ( em_get_option( 'dbem_data_' . static::$prefix . '_events' ) == 1 || ( em_get_option( 'dbem_data_' . static::$prefix . '_events' ) == 2 && ! is_user_logged_in() ) ) {
			add_action( 'em_front_event_form_footer', array( static::class, 'event_checkbox' ), 9, 1 );
			add_action( 'em_event_get_post_meta', array( static::class, 'cpt_get_post' ), 10, 2 );
			add_action( 'em_event_validate', array( static::class, 'cpt_validate' ), 10, 2 );
			add_action( 'em_event_save', array( static::class, 'cpt_save' ), 10, 2 );
		}
		//LOCATIONS
		if ( em_get_option( 'dbem_data_' . static::$prefix . '_locations' ) == 1 || ( em_get_option( 'dbem_data_' . static::$prefix . '_events' ) == 2 && ! is_user_logged_in() ) ) {
			add_action( 'em_front_location_form_footer', array( static::class, 'location_checkbox' ), 9, 1 );
			add_action( 'em_location_get_post_meta', array( static::class, 'cpt_get_post' ), 10, 2 );
			add_action( 'em_location_validate', array( static::class, 'cpt_validate' ), 10, 2 );
			add_action( 'em_location_save', array( static::class, 'cpt_save' ), 10, 2 );
		}
	}
	...
	public static function has_consented( $item, $type = false ) {
		...
		return apply_filters(
			'em_consent_has_consented',
			$consented == true,
			static::class,
			array(
				'item'      => $item,
				'type'      => $type,
				'EM_Person' => $EM_Person ?? null,
			)
		);
	}
	...
	public static function checkbox( $EM_Object = false ) {
		...
		if ( ! doing_action( 'em_booking_form_after_user_details' ) && did_action( 'em_booking_form_after_user_details' ) ) {
			return; // backcompat
		}
		...
		if ( is_user_logged_in() ) {
			...
			if ( ! empty( $consent_given_already ) && em_get_option( static::$options['remember'] ) == 1 ) {
				return; //ignore if consent given as per settings
			}
			if ( ! empty( $consent_given_already ) && em_get_option( static::$options['remember'] ) == 2 ) {
				$checked = true;
			}
		}
		if ( empty( $checked ) && ! empty( $_REQUEST[ static::$options['param'] ] ) ) {
			$checked = true;
		}
		// output checkbox
		?>
		<p class="input-group input-checkbox em-consent-checkbox input-field-data_<?php echo esc_attr( static::$prefix ); ?>">
			<label>
				<input type="checkbox" name="<?php echo esc_attr( static::$options['param'] ); ?>" value="1" 
				<?php
				if ( ! empty( $checked ) ) {
	...
	public static function cpt_validate( $result, $EM_Object ) {
		...
		if ( is_user_logged_in() ) {
			...
			$consent_given_already = get_user_meta( get_current_user_id(), static::$options['meta_key'], true );
			if ( ! empty( $consent_given_already ) && em_get_option( static::$options['remember'] ) == 1 ) {
				return $result; //ignore if consent given as per settings
			}
		}
