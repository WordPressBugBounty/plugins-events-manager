...
class Comms extends Consent {
	...
	public static function init() {
		...
		add_action( 'em_person_get_summary', array( static::class, 'em_person_get_summary' ) ); // add to summary for exporting etc.
		// add field to wp dashboard profile in contact information section
		add_action( 'show_user_profile', array( static::class, 'show_profile_fields' ), 10 );
		add_action( 'edit_user_profile', array( static::class, 'show_profile_fields' ), 10 );
		add_action( 'em_user_profile_fields', array( static::class, 'show_profile_field' ), 10 );
		...
		add_action( 'em_person_display_summary_bottom', array( static::class, 'em_person_display_summary_bottom' ) ); //$this
		add_action( 'em_booking_get_person_editor_bottom', array( static::class, 'em_booking_get_person_editor_bottom' ) ); //$this
		if ( ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] == 'booking_modify_person' ) { //only hook in if we're editing a no-user booking
			add_filter( 'em_booking_get_person_post', array( static::class, 'em_booking_get_post' ), 100, 2 );
			add_filter( 'em_nouser_booking_details_modified', array( static::class, 'em_nouser_booking_details_modified' ), 100, 2 );
		}
	}
		...
		<tr>
			...
			<td>
				...
				if ( ! $consented && $already_revoked ) {
					...
					echo '<p style="color:#600000"><em>' . sprintf( esc_html__( 'Consent %1$s on %2$s', 'events-manager' ), strtolower( esc_html__( 'Revoked', 'event-manager' ) ), $EM_DateTime->formatDefault() ) . '</em></p>';
				}
				?>
			</td>
		</tr>
	...
	public static function em_person_get_summary( $summary, $EM_Person ) {
		$consents = array(
			'comms_consent_given'   => array(
				'name'  => __( 'Communications Consent', 'events-manager' ),
				'value' => $EM_Person->{ static::$options['meta_key'] },
			),
			'comms_consent_revoked' => array(
				'name'  => __( 'Communications Consent', 'events-manager' ) . ' ' . __( 'Revoked', 'events-manager' ),
				'value' => $EM_Person->{ static::$options['meta_key'] . '_revoked' },
			),
		);
	...
	public static function show_profile_fields( $user ) {
		if ( did_action( 'em_user_profile_fields' ) ) {
			return;
		}
		?>
	...
	public static function show_checkbox( $already_consented = false, $already_revoked = false ) {
		...
		if ( $already_consented ) {
			...
			echo sprintf( esc_html__( 'Consent %1$s on %2$s', 'events-manager' ), strtolower( esc_html__( 'Received', 'event-manager' ) ), $EM_DateTime->formatDefault() );
			if ( $already_revoked ) {
				$EM_DateTime = EM_DateTime::create( $already_revoked, 'UTC' )->setTimezone();
				echo '<br>' . sprintf( esc_html__( 'Consent previously %1$s on %2$s', 'events-manager' ), strtolower( esc_html__( 'Revoked', 'event-manager' ) ), $EM_DateTime->formatDefault() );
			}
			echo '<em></p>';
	...
	public static function em_bookings_table_get_sortable_columns( $sortable_columns, $EM_Bookings_Table ) {
		$sortable_columns[ static::$prefix ] = array( static::$prefix, false );
