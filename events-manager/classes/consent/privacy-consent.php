...
class Privacy extends Consent {
	...
	public static function get_label() {
		...
		if ( has_filter( 'the_privacy_policy_link', 'bp_core_change_privacy_policy_link_on_private_network' ) ) {
			$bb_fix = remove_filter( 'the_privacy_policy_link', 'bp_core_change_privacy_policy_link_on_private_network', 999999 );
		}
		// replace privacy policy text %s with link to policy page
		if ( function_exists( 'get_the_privacy_policy_link' ) ) {
			$label = sprintf( $label, get_the_privacy_policy_link() );
		}
