...
class EM_Locations extends EM_Object {
	...
	public static function get( $args = array(), $count = false ) {
		...
		if ( $count ) {
			...
			if ( $calc_found_rows ) {
				$selectors = 'SQL_CALC_FOUND_ROWS ' . $selectors; //for storing total rows found
			}
...
$limit $offset
	...
	public static function check_events_table_join( $args, $event_fields ) {
		...
		if ( ! empty( $args['groupby'] ) || ( defined( 'EM_DISABLE_OPTIONAL_JOINS' ) && EM_DISABLE_OPTIONAL_JOINS ) ) {
			...
			foreach ( $event_specific_args as $arg ) {
				if ( ! empty( $args[ $arg ] ) ) {
					$join_events_table = true;
				}
			}
			//if set to false the following would provide a false negative in the line above
			if ( $args['recurrences'] !== null ) {
				$join_events_table = true;
			}
			if ( $args['recurring'] !== null ) {
				$join_events_table = true;
			}
			if ( $args['event_status'] !== false ) {
				$join_events_table = true; }
			...
			if ( $join_events_table && $args['recurring'] === null ) {
				$args['recurring'] = false;
			}
		} else {
			$join_events_table = true; }//end temporary if( !empty($args['groupby']).... wrapper
	...
	public static function build_sql_conditions( $args = array(), $count = false ) {
		...
		if ( true == $args['eventful'] ) {
			...
			if ( ! empty( $conditions['scope'] ) ) {
				unset( $conditions['scope'] ); //scope condition would render all queries return no results
			}
		}
		//owner lookup
