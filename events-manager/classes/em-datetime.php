...
class EM_DateTime extends DateTime {
	...
	public function format( $format = 'Y-m-d H:i:s', $timezone = null ) {
		...
		// format normally
		if ( ! $this->valid && ( $format == 'Y-m-d' || $format == em_get_date_format() ) ) {
			return '';
		}
		if ( $format !== 'Y-m-d H:i:s' ) {
			$format = $this->formatTimezones( $format ); // format UTC timezones
		}
	...
	public function i18n( $format = 'Y-m-d H:i:s' ) {
		if ( ! $this->valid && $format == em_get_date_format() ) {
			return '';
		}
	...
	public function setTimezone( $timezone = false ) {
		if ( $timezone == $this->getTimezone()->getName() ) {
			return $this;
		}
		$timezone            = EM_DateTimeZone::create( $timezone );
	...
	public function getTime( $utc = false ) {
		...
		if ( $utc ) {
			$this->setTimezone( $current_timezone );
		}
		return $return;
	}
