...
class EM_Bookings_Table extends EM\List_Table {
	public static $cols_allowed_html = array(
		...
		'ticket_total'          => 0,
		'ticket_spaces'         => 0,
		'ticket_booking_spaces' => 0,
		'ticket_id'             => 0,
	);
	public static $has_filters       = true;
	public static $filter_vars       = array(
		'search' => array(
			'param'   => 'em_search',
			'default' => '',
		),
	...
	function __construct( $args = array() ) {
		...
		$this->statuses = array(
			...
			'confirmed'       => array(
				'label'  => __( 'Confirmed', 'events-manager' ),
				'search' => array( 1 ),
			),
			'cancelled'       => array(
				'label'  => __( 'Cancelled', 'events-manager' ),
				'search' => array( 3 ),
			),
			'rejected'        => array(
				'label'  => __( 'Rejected', 'events-manager' ),
				'search' => array( 2 ),
			),
			'needs-attention' => array(
				'label'  => __( 'Needs Attention', 'events-manager' ),
				'search' => array( 0 ),
			),
			'incomplete'      => array(
				'label'  => __( 'Incomplete Bookings', 'events-manager' ),
				'search' => array( 0 ),
			),
		);
	...
	public function load_columns() {
		...
		$this->cols_users_template      = apply_filters(
			'em_bookings_table_cols_users_template',
			array(
		...
		$this->cols_events_template     = apply_filters(
			'em_bookings_table_cols_events_template',
			array(
		...
		$this->cols_bookings_template   = apply_filters(
			'em_bookings_table_cols_bookings_template',
			array(
		...
		$this->cols_attendees_template  = apply_filters(
			'em_bookings_table_cols_attendees_template',
			array(
		...
		$this->cols_tickets_template    = apply_filters(
			'em_bookings_table_cols_tickets_template',
			array(
		...
		$this->cols_template_groups     = apply_filters(
			'em_bookings_table_cols_template_groups',
			array(
				'user'         => array(
	...
	public function get_current_context() {
		...
		if ( empty( $context_settings['cols'] ) ) {
			...
			$view_default    = $this->views[ $this->view ];
			unset( $view_default['contexts'] );
	...
	public function get_booking_allowed_actions( $EM_Object ) {
		...
		$actions = $this->get_action_data_extra( array( 'booking_id' => $EM_Booking->booking_id ) );
		// determine which keys to show
		switch ( $EM_Booking->booking_status ) {
			case 0: //pending
				if ( em_get_option( 'dbem_bookings_approval' ) ) {
					$allowed_actions = array( 'approve', 'reject', 'delete' );
					break;
				}//if approvals are off, treat as a 1
			case 1: //approved
				$allowed_actions = array( 'unapprove', 'reject', 'delete' );
				break;
			case 2: //rejected
				$allowed_actions = array( 'approve', 'delete' );
				break;
			case 3: //cancelled
		...
		$allowed_actions = apply_filters(
			'em_bookings_table_booking_allowed_actions',
			$allowed_actions,
			$EM_Booking,
			array(
				'table'   => $this,
		...
		// return the data
		return apply_filters(
			'em_bookings_table_get_booking_allowed_actions',
			$booking_actions,
			$this,
			array(
				'booking'         => $EM_Booking,
	...
	public function get_action_data_items() {
		$data_template = array(
			'action'  => static::$basename . '_row',
			'context' => 'bookings',
		);
		$actions       = array(
			'approve'   => array(
				'label' => __( 'Approve', 'events-manager' ),
				'data'  => array_merge( $data_template, array( 'row_action' => 'approve' ) ),
			),
			'reject'    => array(
				'label' => __( 'Reject', 'events-manager' ),
				'data'  => array_merge( $data_template, array( 'row_action' => 'reject' ) ),
			),
			'delete'    => array(
				'label' => __( 'Delete', 'events-manager' ),
				'data'  => array_merge( $data_template, array( 'row_action' => 'delete' ) ),
			),
			'unapprove' => array(
				'label' => __( 'Unapprove', 'events-manager' ),
				'data'  => array_merge( $data_template, array( 'row_action' => 'unapprove' ) ),
			),
		);
		$actions       = apply_filters( static::$basename . '_get_action_data_items', $actions, $this, array( 'data_template' => $data_template ) );
		...
		foreach ( $actions as $action => $action_data ) {
			if ( isset( $action_data['actions'] ) ) {
				foreach ( $action_data['actions'] as $the_action => $the_action_data ) {
					if ( in_array( $this->view, array( 'attendees', 'tickets' ) ) && $the_action_data['data']['context'] === 'bookings' ) {
						$actions[ $action ]['actions'][ $the_action ]['data']['upstream'] = 1;
					}
				}
			} else {
				if ( in_array( $this->view, array( 'attendees', 'tickets' ) ) && $action_data['data']['context'] === 'bookings' ) {
	...
	public function get_bulk_action_message( $action, $context ) {
		...
		if ( $parent_message && $action === 'delete' ) {
			$message = $parent_message . '&#10;&#10;' . $message;
		}
		// return message via custom filtersd
		return apply_filters( static::$basename . '_get_bulk_action_message', $message, $action, $context, $this, array( 'parent_message' => $parent_message ) );
	}
	...
	public function get_action_message( $action, $context ) {
		...
		return apply_filters( static::$basename . '_get_action_message', $message, $action, $context, $this, array( 'parent_message' => $parent_message ) );
	}
	...
	public function get_default_column_data( $item, $col ) {
		...
		if ( $col == 'user_email' ) {
			...
			if ( $EM_Booking->is_no_user() ) {
				...
				if ( in_array( $format, array( 'csv', 'xls', 'xlsx' ) ) ) {
					...
					) . '">' . esc_html( $EM_Booking->person->user_login ) . '</a>';
				}
			}
		} elseif ( $col == 'dbem_phone' ) {
			if ( ! in_array( $format, array( 'csv', 'xls', 'xlsx' ) ) && ! $EM_Booking->get_person()->phone_validity ) {
			...
			if ( in_array( $format, array( 'csv', 'xls', 'xlsx' ) ) ) {
				...
				) . '">' . esc_html( $EM_Booking->person->get_name() ) . '</a>';
			}
			...
			if ( in_array( $format, array( 'csv', 'xls', 'xlsx' ) ) ) {
				...
				if ( $EM_Booking->get_event()->is_recurrence() || $EM_Booking->get_event()->is_timeslot() ) {
					$val                               = $EM_Booking->get_event()->output( '#_EVENTDATES' );
					$val                               = '<a href="' . add_query_arg( array( 'event_id' => $EM_Booking->get_event()->get_event_id() ), $EM_Booking->get_event()->get_bookings_url() ) . '">' . esc_html( $val ) . '</a>';
					static::$cols_allowed_html[ $col ] = true;
				} else {
					$val = $EM_Booking->get_event()->output( '#_EVENTDATES' );
				}
		...
		} elseif ( $col == 'actions' && ! in_array( $format, array( 'csv', 'xls', 'xlsx' ) ) ) {
			// html only
	...
	public function get_tickets_multiple_col( $tickets_array, $col, $EM_Booking ) {
		...
		if ( ! in_array( $this->format, array( 'csv', 'xls', 'xlsx' ) ) ) {
	...
	public function get_attendees_multiple_col( $attendees_array, $col, $EM_Object, $html = false ) {
		...
		if ( ! in_array( $this->format, array( 'csv', 'xls', 'xlsx' ) ) ) {
			...
			// count the attendees
			$attendees_count = 0;
			foreach ( $attendees_array as $ticket_attendees ) {
				$attendees_count += count( $ticket_attendees['attendees'] );
			}
	...
	public function extra_tablenav( $which ) {
		...
		$id       = esc_attr( $this->id );
		?>
		<div class="alignleft actions filters em-list-table-filters <?php echo $id; ?>-filters 
		<?php
		if ( ! static::$show_filters ) {
			echo 'hidden';}
		?>
		">
				...
				<div class="em-bookings-table-views-options" id="em-bookings-table-views-options-<?php echo $id; ?>">
					<fieldset class="em-bookings-table-views-options-list" id="em-bookings-table-views-options-select-<?php echo $id; ?>">
						...
						<?php foreach ( $this->views as $view_id => $view ) : ?>
							<label class="em-bookings-table-view-option em-bookings-table-view-type-<?php echo esc_attr( $view_id ); ?>
							<?php
							if ( $view_id === $this->view ) {
								echo 'checked';}
							?>
							"  data-view="<?php echo esc_attr( $view_id ); ?>">
								<input type="radio" name="view" class="em-bookings-table-view-option em-bookings-table-view-type-<?php echo esc_attr( $view_id ); ?>" value="<?php echo esc_attr( $view_id ); ?>"  
								<?php
								if ( $view_id === $this->view ) {
									echo 'checked';}
								?>
