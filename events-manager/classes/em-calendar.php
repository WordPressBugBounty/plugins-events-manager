...
class EM_Calendar extends EM_Object {
	...
	public static function get( $args ) {
		...
		if ( em_get_option( 'dbem_full_calendar_abbreviated_weekdays' ) ) {
			$weekdays['large'] = array( 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat' );
		}
		if ( em_get_option( 'dbem_small_calendar_abbreviated_weekdays' ) ) {
			$weekdays['small'] = array( 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat' );
		}
		$day_initials_lengths = array(
			'large' => em_get_option( 'dbem_full_calendar_initials_length' ),
			'small' => em_get_option( 'dbem_small_calendar_initials_length' ),
		);
		...
		foreach ( $weeks as $week ) {
			foreach ( $week as $d ) {
				$date                             = date( 'Y-m-d', $d );
				$calendar_array['cells'][ $date ] = array(
					'date'         => $d,
					'events'       => array(),
					'events_count' => 0,
		...
		if ( ! empty( $day_link_args['limit'] ) ) {
			unset( $day_link_args['limit'] );
		}
		//get event link
			...
			if ( array_key_exists( $day_key, $calendar_array['cells'] ) ) {
				...
				if ( $eventful_days_count[ $day_key ] > 1 || ! em_get_option( 'dbem_calendar_direct_links' ) ) {
					if ( $wp_rewrite->using_permalinks() && ! defined( 'EM_DISABLE_PERMALINKS' ) ) {
						...
						if ( ! empty( $event_page_link_parts[1] ) ) {
							$calendar_array['cells'][ $day_key ]['link'] .= '?' . $event_page_link_parts[1];
						}
		...
		if ( empty( $args['empty_months'] ) ) {
			...
			if ( empty( $day_key ) || (int) $EM_DateTime->format( 'm' ) === (int) $month ) {
				$months_args = array_merge(
					$months_args,
					array(
						'scope'   => array( $EM_DateTime->setEndOfMonth()->add( 'P1D' )->getDate(), false ),
						'orderby' => 'event_start_date',
						'order'   => 'ASC',
					)
				);
			...
			if ( $scope !== 'future' || $EM_DateTime > $today ) {
				if ( empty( $eventful_days ) || (int) $EM_DateTime->format( 'm' ) === (int) $month ) {
					$months_args = array_merge(
						$months_args,
						array(
							'scope'   => array( false, $EM_DateTime->setStartOfMonth()->sub( 'P1D' )->getDate() ),
							'orderby' => 'event_start_date',
							'order'   => 'DESC',
						)
					);
		...
		if ( $today->format( 'Y-n' ) != $year . '-' . absint( $month ) ) {
			$calendar_array['links']['today_url'] = esc_url_raw(
				add_query_arg(
					array(
						'mo' => $today->format( 'm' ),
						'yr' => $today->format( 'Y' ),
		...
		if ( $month_last && ( $args['scope'] !== 'future' || $today->format( 'Y-n' ) !== $year . '-' . absint( $month ) ) ) { // don't show if future scope and this month
			$calendar_array['links']['previous_url'] = esc_url_raw(
				add_query_arg(
					array(
						'mo' => $month_last,
						'yr' => $year_last,
	...
	public static function output( $base_args = array(), $wrapper = true ) {
		...
		if ( ! empty( $args['has_search'] ) && empty( $args['show_search'] ) ) {
			...
			$args['search_scope_advanced'] = false;
		...
		if ( ! empty( $args['calendar_size'] ) ) {
			...
			// add extra args
			$allowed_heights = apply_filters(
				'em_calendar_output_dates_heights',
				array(
		...
		if ( ! empty( $args['has_advanced_trigger'] ) ) {
			$calendar_array['css']['calendar_classes'][] = 'with-advanced';
		}
			$EM_DateTime = new EM_DateTime( $calendar_array['month_start'], 'UTC' );
		if ( $EM_DateTime->format( 'Y-m' ) === date( 'Y-m' ) ) {
			$calendar_array['css']['calendar_classes'][] = 'this-month';
		}
		...
		<div class="<?php em_template_classes( 'view-container' ); ?>" id="em-view-<?php echo absint( $args['id'] ); ?>" data-view="calendar">
			...
			if ( ! em_locate_template( $template ) ) {
				$template = 'calendar/calendar.php'; // backcompat
			}
			em_locate_template(
				$template,
				true,
			...
			<div class="em-view-custom-data" id="em-view-custom-data-<?php echo absint( $args['id'] ); ?>">
				...
				foreach ( $args as $name => $value ) {
					...
					if ( $name === 'scope' && ! empty( $value['name'] ) ) {
						$value = $value['name'];
					}
					if ( is_array( $value ) ) {
						$value = implode( ',', $value );
					...
					if ( $value === true || $value === false ) {
						$value = $value ? 1 : 0; // make sure we get a 1 or 0
					}
	...
	public static function get_default_search( $array_or_defaults = array(), $array = array() ) {
		...
		if ( isset( $array['full'] ) ) {
			$atts['full'] = ( $array['full'] ) ? 1 : 0; //deprecated, we're changing this now to calendar_size for display purposes
		}
