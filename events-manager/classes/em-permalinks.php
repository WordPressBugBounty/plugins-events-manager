...
if ( ! class_exists( 'EM_Permalinks' ) ) {
	class EM_Permalinks {
		...
		public static function init() {
			...
			if ( ! defined( 'EM_LOCATION_SLUG' ) ) {
				define( 'EM_LOCATION_SLUG', 'location' );
			}
			if ( ! defined( 'EM_LOCATIONS_SLUG' ) ) {
				define( 'EM_LOCATIONS_SLUG', 'locations' );
			}
			if ( ! defined( 'EM_CATEGORY_SLUG' ) ) {
				define( 'EM_CATEGORY_SLUG', 'category' );
			}
			if ( ! defined( 'EM_CATEGORIES_SLUG' ) ) {
				define( 'EM_CATEGORIES_SLUG', 'categories' );
			}
			add_filter( 'post_type_archive_link', array( 'EM_Permalinks', 'post_type_archive_link' ), 10, 2 );

			add_action( 'template_redirect', array( static::class, 'handle_converted_repeating_404_redirects' ), 10 );
		}
		...
		public static function rewrite_rules_array( $rules ) {
				...
				if ( count( $conflicting_posts ) > 0 ) { //won't apply on homepage
					foreach ( $conflicting_posts as $conflicting_post ) {
						...
						$child_posts = get_posts(
							array(
								'post_type'   => 'any',
								'post_parent' => $conflicting_post->ID,
								'numberposts' => 0,
							)
						);
						foreach ( $child_posts as $child_post ) {
			...
			foreach ( EM\Archetypes::get_cpts( array(), array( 'event', 'types' ) ) as $cpt ) {
		...
		/**
		 * Handle 404s for deleted event posts by redirecting to unified page
		 */
		public static function handle_converted_repeating_404_redirects() {
			...
			// Check if this looks like it might be an event post
			// Assuming your events have URLs like /events/event-slug/
			$events_base = EM_POST_TYPE_EVENT_SLUG; // Change this if your events use a different base
			if ( count( $path_parts ) >= 2 && $path_parts[0] === $events_base ) {
				$potential_slug = end( $path_parts );

				// Check if this slug exists in postmeta or another custom table where you store old slugs
				$query    = $wpdb->prepare( 'SELECT event_id FROM ' . EM_EVENTS_TABLE . ' WHERE event_slug = %s', $potential_slug );
				$event_id = $wpdb->get_var( $query );
				if ( $event_id ) {
					// Get the unified page URL
					$EM_Event = em_get_event( $event_id );
					if ( $EM_Event->is_recurrence() ) {
						// Perform 302 redirect
						wp_redirect( $EM_Event->get_recurring_event()->get_permalink(), 302 );
						exit();
					}
				}
			}
