...
class EM_Object {
	...
	public static function get_default_search( $defaults = array(), $array = array() ) {
		...
		foreach ( self::get_taxonomies() as $item => $item_data ) {
			$super_defaults[ $item ] = false; }
		...
		if ( is_array( $array ) ) {
			...
			if ( array_key_exists( 'recurrence_id', $array ) && ! array_key_exists( 'recurring_event', $array ) ) {
				$array['recurring_event'] = $array['recurrence_id']; }
			if ( array_key_exists( 'recurrence', $array ) && ! array_key_exists( 'recurring_event', $array ) ) {
				$array['recurring_event'] = $array['recurrence']; }
			if ( array_key_exists( 'location_id', $array ) && ! array_key_exists( 'location', $array ) ) {
				$array['location'] = $array['location_id']; }
			if ( array_key_exists( 'category_id', $array ) && ! array_key_exists( 'category', $array ) ) {
				$array['category'] = $array['category_id']; }
			if ( array_key_exists( 'page', $array ) ) {
				$array['page'] = absint( $array['page'] ); }

			//Clean all id lists
			$clean_ids_array = array( 'location', 'event', 'active_status' );
			if ( ! empty( $array['owner'] ) && $array['owner'] != 'me' ) {
				$clean_ids_array[] = 'owner'; //clean owner attribute if not 'me'
			}
			// add post
			...
			foreach ( array_keys( $array ) as $key ) {
				if ( ! array_key_exists( $key, $defaults ) && ! array_key_exists( $key, $taxonomies ) ) {
					unset( $array[ $key ] );
				}
			}
			//Timezone
			...
			if ( empty( $array['event'] ) ) {
				...
				if ( ! empty( $array['event_type'] ) ) {
					...
					$allowed_event_types = array( 'recurring', 'repeating', 'recurrence', 'single' );
		...
		foreach ( array( 'orderby', 'groupby', 'groupby_orderby' ) as $orderby_arg ) {
			if ( ! is_array( $defaults[ $orderby_arg ] ) && preg_match( '/,/', $defaults[ $orderby_arg ] ) ) {
				$defaults[ $orderby_arg ] = str_replace( array( ' asc', ' desc' ), array( ' ASC', ' DESC' ), $defaults[ $orderby_arg ] ); // uppercase ordering so we can correct later
				$defaults[ $orderby_arg ] = str_replace( ' ', '', $defaults[ $orderby_arg ] );
				$defaults[ $orderby_arg ] = str_replace( array( 'ASC', 'DESC' ), array( ' ASC', ' DESC' ), $defaults[ $orderby_arg ] ); // correct removed spaces
			...
			if ( is_array( $defaults[ $orderby_arg ] ) ) {
				$defaults[ $orderby_arg ] = array_values( $defaults[ $orderby_arg ] ); //reset array keys because we want an index 0 present
			}
		}
	...
	public static function build_sql_conditions( $args = array() ) {
		...
		if ( ! empty( $args['timezone'] ) ) {
			...
			foreach ( $args['timezone'] as $tz ) {
				$timezones[] = $wpdb->prepare( '%s', $tz );
			}
		...
		if ( $args['timezone_scope'] ) {
			$timezone_scope  = in_array( $args['timezone_scope'], array( 1, '1', true ), true ) ? em_get_option( 'timezone_string', $args['event_archetype'] ) : $args['timezone_scope'];
			$cast            = 'DATETIME';
		...
		if ( ! empty( $args['near'] ) && self::array_is_numeric( $args['near'] ) ) {
			...
			if ( empty( $args['near_unit'] ) ) {
				$args['near_unit'] = em_get_option( 'dbem_search_form_geo_distance', 'mi', $args['event_archetype'] );
			}
			...
			if ( ! empty( $args[ $item ] ) && ! is_array( $args[ $item ] ) ) {
				if ( preg_match( '/[,&]/', $args[ $item ] ) !== false ) { //accepts numbers or words
					...
					foreach ( $args[ $item ] as $k => $v ) {
						...
						foreach ( $args[ $item ][ $k ] as $k_x => $v_x ) {
							$args[ $item ][ $k ][ $k_x ] = trim( $v_x );
						}
					}
				} else {
		...
		foreach ( $taxonomies as $tax_name => $tax_data ) {
			if ( ! empty( $args[ $tax_name ] ) && is_array( $args[ $tax_name ] ) ) {
				if ( ! empty( $tax_data['ms'] ) ) {
					self::ms_global_switch(); //if in ms global mode, switch here rather than on each EM_Category instance
				}
				$tax_conds = array();
				...
				foreach ( $args[ $tax_name ] as $k => $v ) {
					if ( is_array( $v ) ) {
						$contains_array = true;
					}
				}
				if ( empty( $contains_array ) ) {
					$args[ $tax_name ] = array( $args[ $tax_name ] );
				}
				...
				if ( ! empty( $tax_data['ms'] ) ) {
					self::ms_global_switch_back(); //if in ms global mode, switch back from previous switch
				}
			}
		}
		...
		if ( empty( self::$taxonomies_array ) ) {
			...
			$taxonomies_array = array(
				'category' => array(
					...
					'ms'        => 'event-category',
					'context'   => array(),
					'query_var' => EM_TAXONOMY_CATEGORY,
				),
				'tag'      => array(
					'name'      => EM_TAXONOMY_TAG,
					'slug'      => EM_TAXONOMY_TAG_SLUG,
					'context'   => array(),
					'query_var' => EM_TAXONOMY_TAG,
				),
			);
				...
				if ( $tax_name == EM_TAXONOMY_CATEGORY || $tax_name == EM_TAXONOMY_TAG ) {
					...
					if ( is_array( $tax->object_type ) && ! empty( $tax->rewrite ) ) {
						if ( $event_tax || $loc_tax ) {
							$taxonomies_array[ $prefix . $tax_name ] = array(
								'name'      => $tax_name,
								'context'   => array(),
	...
	public static function build_wpquery_conditions( $args, $wp_query ) {
		...
		if ( is_array( $scope ) || preg_match( '/^[0-9]{4}-[0-9]{2}-[0-9]{2},[0-9]{4}-[0-9]{2}-[0-9]{2}$/', $scope ) ) {
			...
			if ( em_get_option( 'dbem_events_current_are_past', $args['event_archetype'] ) && $wp_query->query_vars['post_type'] != 'event-recurring' ) {
				$query[] = array(
					'key'     => '_event_start_date',
					'value'   => array( $start_month, $end_month ),
					'type'    => 'DATE',
					'compare' => 'BETWEEN',
				);
			} else {
	...
	public static function get_post_search( $args = array(), $filter = false, $request = array(), $accepted_searches = array() ) {
		...
		if ( ! empty( $request['em_search'] ) && empty( $args['search'] ) ) {
			$request['search'] = $request['em_search']; //em_search is included to circumvent wp search GET/POST clashes
		}
	...
	function can_manage( $owner_capability = false, $admin_capability = false, $user_to_check = false ) {
		...
		if ( ! $admin_capability ) {
			$admin_capability = $owner_capability;
		}
	...
	function add_error( $errors ) {
		...
		if ( ! is_array( $this->errors ) ) {
			$this->errors = array(); } //create empty array if this isn't an array
	...
	function image_validate() {
		...
		try {
			...
			$result       = EM\Uploads\Uploader::validate(
				$type . '_image',
				array(
					'type'          => 'image',
					'max_file_size' => $max_filesize,
				)
			) !== false; // no false returned, error thrown if not true/null
		} catch ( EM_Exception $e ) {
			$this->add_error( $e );
			$result = false;
		}
	...
	function process_meta( $raw_meta ) {
		...
		foreach ( $raw_meta as $meta ) {
			...
			if ( preg_match( '/^_([a-zA-Z\-0-9 _]+)\|([^\|]+)?$/', $meta_key, $match ) || preg_match( '/^_([a-zA-Z\-0-9]+)_(.+)$/', $meta_key, $match ) ) {
				$key = $match[1];
				if ( empty( $processed_meta[ $key ] ) ) {
					$processed_meta[ $key ] = array();
				}
