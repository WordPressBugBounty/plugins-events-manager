...
class EM_Ticket extends EM_Object {
	...
	public $fields = array(
		...
		'multi_event_auto_enrol' => array(
			'type' => '%d',
			'null' => 1,
		),
		'multi_event_ticket'     => array(
			'type' => '%d',
			'null' => 1,
		),
	);
	...
	protected $reserved_spaces = array();
	...
	function __construct( $ticket_data = false ) {
		...
		if ( $ticket_data !== false ) {
			...
			if ( $this->ticket_parent === null && $this->ticket_price === null ) {
				$this->ticket_price = 0;
			}
	...
	function __get( $prop ) {
		...
		if ( ! empty( static::$field_shortcuts[ $prop ] ) && ! empty( $this->ticket_parent ) ) {
			...
			if ( $this->{$property} === null || ( $prop === 'members_roles' && $this->{$property} === array() ) ) {
	...
	public function __set( $prop, $val ) {
		if ( in_array( $prop, array( 'name', 'ticket_name' ) ) ) {
			$this->ticket_name = $val;
		} elseif ( in_array( $prop, array( 'description', 'ticket_description' ) ) ) {
			$this->ticket_description = $val;
		} elseif ( $prop === 'event' && $val instanceof EM_Event ) {
			$this->event    = $val;
			$this->event_id = $this->event->event_id;
		} elseif ( in_array( $prop, array( 'ticket_start', 'start' ) ) ) {
			$this->ticket_start = $val;
			$this->start        = false;
		} elseif ( in_array( $prop, array( 'ticket_end', 'end' ) ) ) {
			$this->ticket_end = $val;
			$this->end        = false;
		} elseif ( $prop == 'meta' ) {
			if ( is_array( $val ) ) {
	...
	public function is_overridden( $check_meta = false ) {
		$fields_to_check = array( 'name', 'description', 'price', 'spaces', 'start', 'end', 'min', 'max', 'status' );
			...
			if ( $this->get_event()->is_recurring( true ) ) {
				...
				if ( ! empty( $tickets ) ) {
					foreach ( $tickets as $ticket ) {
						...
						$ticket_properties = array( 'price', 'description', 'spaces', 'start', 'end', 'min', 'max', 'required', 'members', 'members_roles', 'guests' );
						...
						// update this ticket
						$wpdb->update( EM_TICKETS_TABLE, $ticket, array( 'ticket_id' => $ticket['ticket_id'] ) );
					}
				}
				// mark upgrade flag
				$this->ticket_meta['recurrences']       = $this->ticket_meta['recurrences'] ?? array();
	...
	function is_available( $ignore_member_restrictions = false, $ignore_guest_restrictions = false, $ignore_spaces = false, $user = null ) {
		...
		if ( isset( $this->is_available ) && ! $ignore_member_restrictions && ! $ignore_guest_restrictions && ! $ignore_spaces && $user === null ) {
			return apply_filters( 'em_ticket_is_available', $this->is_available, $this, false, false, false, null ); //save extra queries if doing a standard check
		}
		$is_available = false;
	...
	function get_available_spaces() {
		...
		return apply_filters(
			...
			$this,
			array(
				'event_spaces'  => $event_available_spaces,
				'ticket_spaces' => $ticket_available_spaces,
			)
		);
	}
	...
	public function get_recurrence_ticket_ids() {
		...
		foreach ( $ticket_ids as $k => $v ) {
			$ticket_ids[ $k ] = absint( $v ); //clean for SQL usage
		}
		...
		if ( $this->can_manage() ) {
			if ( count( $this->get_bookings()->bookings ) == 0 ) {
				...
				if ( $child_ticket_ids ) {
					...
					$wpdb->update( EM_TICKETS_TABLE, array( 'ticket_status' => 0 ), array( 'ticket_parent' => $this->ticket_id ), array( '%d' ), array( '%d' ) );
	...
	public function detach() {
		...
		if ( $this->ticket_parent ) {
			$sql = static::detach_parent_sql( $this->ticket_parent, array( $this->ticket_id ) );
	...
	function get_spaces_options( $zero_value = true, $default_value = 0 ) {
		...
		if ( EM_Bookings::$disable_restrictions && $max_spaces > $available_spaces ) {
		...
		if ( $this->is_available() ) {
			...
			<select name="em_tickets[<?php echo $this->id; ?>][spaces]" class="em-ticket-select" id="em-ticket-spaces-<?php echo $this->ticket_id; ?>" data-ticket-id="<?php echo esc_attr( $this->ticket_id ); ?>" aria-label="<?php echo $label; ?>">
				...
				if ( empty( $shown_default ) && $default_value > 0 ) :
					?>
					<option selected="selected"><?php echo absint( $default_value ); ?></option><?php endif; ?>
			</select>
			<?php
