...
class EM_Taxonomy_Frontend {
	...
	public static function template( $template = '' ) {
		...
		if ( is_tax( static::$taxonomy_name ) && ! locate_template( 'taxonomy-' . static::$taxonomy_name . '.php' ) && em_get_option( 'dbem_cp_' . static::$option_name_plural . '_formats', true ) ) {
			...
			if ( static::get_page_id() ) {
				...
				add_filter( 'pre_get_shortlink', array( static::class, 'pre_get_shortlink' ), 10, 2 );
			}
	...
	public static function the_content( $content ) {
		...
		if ( ! empty( $GLOBALS[ $em_taxonomy_property ] ) ) {
			$taxonomy_id = $GLOBALS[ $em_taxonomy_property ];
		}
		if ( ! empty( $wp_query->{$em_taxonomy_property} ) ) {
			$taxonomy_id = $wp_query->{$em_taxonomy_property};
		}
	...
	public static function parse_query( $wp_query ) {
		...
		if ( $wp_query->is_tax( static::$taxonomy_name ) ) {
			...
			if ( ! em_get_option( 'dbem_cp_events_search_results' ) ) {
				$wp_query->set( 'post_type', $post_types ); //in case events aren't publicly searchable due to 'bug' in WP - https://core.trac.wordpress.org/ticket/17592
			}
