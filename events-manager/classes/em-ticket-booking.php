...
class EM_Ticket_Booking extends EM_Object {
	...
	var $fields                = array(
		...
		'ticket_booking_price'  => array(
			'name' => 'price',
			'type' => '%f',
		),
		'ticket_booking_spaces' => array(
			'name' => 'spaces',
			'type' => '%d',
		),
	);
	...
	public function __get( $prop ) {
		...
		return parent::__get( $prop ); // TODO: Change the autogenerated stub
	}
	...
	function update_meta( $meta_key = null, $meta_value = null ) {
		...
		if ( is_array( $meta_value ) ) {
			...
			foreach ( $meta_value as $subkey => $subvalue ) {
				...
				if ( null !== $existing ) {
					$wpdb->update(
						EM_TICKETS_BOOKINGS_META_TABLE,
						array( 'meta_value' => $subvalue ),
						array(
							'ticket_booking_id' => $this->ticket_booking_id,
	...
	function calculate_price( $force_refresh = false ) {
		if ( $this->ticket_booking_price === null || $force_refresh ) {
			...
			$this->ticket_booking_price = $this->get_ticket()->get_price_without_tax() ?? 0;
