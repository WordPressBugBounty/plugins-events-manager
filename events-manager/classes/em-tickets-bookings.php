...
class EM_Tickets_Bookings extends EM_Object implements Iterator, Countable, ArrayAccess {
			...
			if ( ! empty( $args['orderby'] ) ) {
				...
				foreach ( $args['orderby'] as $orderby ) {
					...
					if ( in_array( $orderby_field, static::$sortable_columns ) ) {
						if ( in_array( $orderby_field, array( 'ticket_id', 'ticket_booking_id', 'ticket_booking_price' ) ) ) {
	...
	public static function get_built_count_sql( $bookings_sql, $extras = array(
		'joins'      => array(),
		'conditions' => array(),
	) ) {
		$joins     = $extras['joins'] ?? array();
	...
	public static function get_bookings_subquery( $sql_parts, $count = false ) {
		...
		if ( $count ) {
			...
			$selectors = array( EM_BOOKINGS_TABLE . '.booking_id' );
			$fields    = array();
			...
			$orderbys_clean = array();
			foreach ( $sql_parts['data']['orderbys'] as $orderby ) {
				$orderbys_clean[] = str_replace( array( ' ASC', ' DESC', ' asc', ' desc' ), '', $orderby );
			}
	...
	public function get_ticket_bookings( $ticket = false ) {
		...
		if ( ! $this->tickets_bookings_loaded && ! empty( $this->booking_id ) ) {
			...
			foreach ( $results as $ticket_booking ) {
				...
				if ( empty( $tickets_bookings[ $EM_Ticket_Booking->ticket_id ] ) ) {
					$tickets_bookings[ $EM_Ticket_Booking->ticket_id ] = array();
				}
		...
		if ( $ticket_id ) {
			if ( empty( $this->tickets_bookings[ $ticket_id ] ) ) {
				$this->tickets_bookings[ $ticket_id ] = new EM_Ticket_Bookings(
					array(
						'ticket_id' => $ticket_id,
