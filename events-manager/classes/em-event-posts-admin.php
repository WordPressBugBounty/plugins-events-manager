...
class EM_Event_Posts_Admin {
	public static function init() {
		...
		if ( $pagenow == 'edit.php' && ! empty( $_REQUEST['post_type'] ) && Archetypes::is_event( $_REQUEST['post_type'] ) ) { //only needed for events list
			...
			//admin warnings
			add_action( 'admin_notices', array( $class, 'admin_notices' ) );
			...
			if ( ( $_REQUEST['post_status'] ?? '' ) !== 'trash' ) {
				add_filter( $row_action_type, array( $class, 'row_actions' ), 10, 2 );
			}
			add_action( 'admin_head', array( $class, 'admin_head' ) );

		}
		foreach ( Archetypes::get_cpts( array( 'location' ) ) as $post_type ) {
			$class = Archetypes::is_repeating( $post_type ) ? 'EM_Event_Recurring_Posts_Admin' : 'EM_Event_Posts_Admin';
			//collumns
	...
	public static function parse_query() {
		...
		if ( ! empty( $wp_query->query_vars['post_type'] ) && Archetypes::is_event( $wp_query->query_vars['post_type'] ) && ( empty( $wp_query->query_vars['post_status'] ) || ! in_array( $wp_query->query_vars['post_status'], array( 'trash', 'pending', 'draft' ) ) ) ) {
	...
	public static function columns_add( $columns ) {
		if ( array_key_exists( 'cb', $columns ) ) {
			...
			$id_array = array(
				'cb'       => $cb,
				'event-id' => sprintf( __( '%s ID', 'events-manager' ), __( 'Event', 'events-manager' ) ),
			);
		} else {
