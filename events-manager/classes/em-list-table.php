...
namespace EM {
	...
	class List_Table extends WP_List_Table {
		...
		public function save_default_settings( $current_settings ) {
			...
			$settings = apply_filters( static::$basename . '_save_default_settings_pre', $settings, $current_settings, $this, array( 'context' => $context ) );
			update_user_meta( get_current_user_id(), static::$basename . '_settings', $settings );
			do_action( static::$basename . '_save_default_settings', $settings, $current_settings, $this, array( 'context' => $context ) );
			return $settings;
		}
		...
		public function get_action_data_items() {
			...
			return apply_filters( 'em_list_table_get_action_data_items', $actions, $this, array( 'data_template' => $data_template ) );
		}
		...
		public function get_action_data_extra( $extra_data ) {
			...
			return apply_filters( 'em_list_table_get_action_data_extra', $actions, $this, array( 'extra_data' => $extra_data ) );
		}
		...
		public function build_action_link( $data, $url = '' ) {
			...
			foreach ( $data as $action => $action_data ) {
				if ( ! empty( $action_data['actions'] ) ) {
					$action_links[ $action ] = array(
						'actions' => $this->build_action_link( $action_data['actions'], $url ),
						'label'   => $action['label'] ?? '',
					);
				} else {
					$data       = array_merge( $action_data['data'], array( 'nonce' => wp_create_nonce( $action_data['data']['action'] ?? $action ) ) );
					$attributes = array();
		...
		public function display() {
			...
			<?php
			em_template_classes( static::$template_component_name );
			echo ' ' . implode( ' ', $extra_classes );
			?>
			" id="<?php echo $uid; ?>" data-basename="<?php echo static::$basename; ?>" <?php echo implode( ' ', $atts ); ?>>
				<form class='<?php echo static::$form_class; ?> em-list-table-form' method='post' action='' id="<?php echo $uid; ?>-form" data-action-messages='<?php echo esc_attr( json_encode( $action_messages ) ); ?>'>
					...
					<?php
					if ( ! empty( $_REQUEST['cols'] ) ) {
						echo esc_attr( implode( ',', $this->cols ) );}
					?>
					">
				...
				<form id="<?php echo $uid; ?>-export-form" class="em-list-table-form em-list-table-export-form" action="" method="post" rel="#<?php echo $uid . '-form'; ?>">
					<div class="em-modal-popup">
						...
						<div class="em-modal-content input">
							...
							<?php
							static::output_overlay_columns_selection(
								array(
									'text' => array(
										'show'   => array(
		...
		public function output_overlay_columns_selection( $options = array() ) {
			...
			foreach ( $this->cols_template_groups as $group_key => $group_data ) {
				...
				$grouped_fields[ $group_key ] = array();
					...
					<select class="em-list-table-cols-inactive em-selectize always-open checkboxes" multiple>
						<?php foreach ( $grouped_fields as $group_key => $cols ) : ?>
							<optgroup label="<?php echo esc_attr( $this->cols_template_groups[ $group_key ]['label'] ); ?>" data-data='{"type":"<?php echo esc_attr( $group_key ); ?>"}'>
								<?php foreach ( $cols as $col_key => $col_data ) : ?>
									...
									?>
									<option value="<?php echo esc_attr( $col_key ); ?>" data-data='{"type":"<?php echo esc_attr( $col_label ); ?>", "header":"<?php echo esc_attr( $col_header ); ?>"}' 
									<?php
									if ( in_array( $col_key, $this->cols ) ) {
										echo 'selected';}
									?>
									>
						...
						<optgroup label="<?php esc_html_e( 'Other', 'events-manager' ); ?>" data-data='{"type":"other"}'>
							<?php foreach ( $ungrouped_fields as $col_key => $col_data ) : ?>
								...
								$col_label  = $col_data['label'] ?? $col_data;
								?>
								<option value="<?php echo esc_attr( $col_key ); ?>" data-data='{"type":"other", "header":"<?php echo esc_attr( $col_header ); ?>"}' 
								<?php
								if ( in_array( $col_key, $this->cols ) ) {
									echo 'selected';}
								?>
		...
		function get_headers( $csv = false ) {
			...
			foreach ( $this->cols as $col ) {
				$spreadsheet_format = in_array( $this->format, array( 'csv', 'xls', 'xlsx' ) );
				if ( $col == 'actions' ) {
		...
		public function bulk_actions_input( $which = '' ) {
			...
			<div class="alignleft actions bulkactions-input">
				...
				<select class="bulk-action-selector em-list-table-bulk-action" id="<?php echo $uid; ?>-bulk-action-selector
				<?php
				if ( $which ) {
					echo '-' . $which;}
				?>
		...
		public function extra_tablenav_trigger( $which = '' ) {
			...
			<div class="alignleft actions <?php echo $id; ?>-settings">
				<button class="button filters-trigger <?php echo esc_attr( $this->id ); ?>-filters-trigger em-tooltip 
				<?php
				if ( ! static::$show_filters ) {
					echo 'hidden';}
				?>
				" aria-label="<?php echo $tooltip_text; ?>"
