...
class EM_ML_Admin {
	...
	public static function init() {
		...
		if ( ! defined( 'EM_SETTINGS_TABS' ) && count( EM_ML::$langs ) > 3 ) {
			define( 'EM_SETTINGS_TABS', true );
		}
	}
			...
			foreach ( array( 'em-event-when', 'em-event-recurring', 'em-event-when-recurring', 'em-event-where', 'em-event-bookings', 'em-event-bookings-stats', 'em-event-group' ) as $box_name ) {
				foreach ( array( EM_POST_TYPE_EVENT, 'event-recurring' ) as $type ) {
					...
					foreach ( array( 'core', 'default', 'high', 'low' ) as $priority ) {
						if ( isset( $wp_meta_boxes[ $screen_name ]['side'][ $priority ][ $box_name ] ) ) {
							unset( $wp_meta_boxes[ $screen_name ]['side'][ $priority ][ $box_name ] );
						}
						if ( isset( $wp_meta_boxes[ $screen_name ]['normal'][ $priority ][ $box_name ] ) ) {
							unset( $wp_meta_boxes[ $screen_name ]['normal'][ $priority ][ $box_name ] );
						}
					}
				}
			}
