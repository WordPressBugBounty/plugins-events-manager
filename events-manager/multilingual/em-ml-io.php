...
class EM_ML_IO {
		...
		if ( ! EM_ML::is_original( $EM_Event ) ) {
			...
			foreach ( $EM_Event->get_tickets()->tickets as $EM_Ticket ) { /* @var $EM_Ticket EM_Ticket */
				...
				if ( ! empty( $_REQUEST['ticket_translations'][ $EM_Ticket->ticket_id ]['ticket_name'] ) ) {
					$ticket_translation['ticket_name'] = wp_kses_data( wp_unslash( $_REQUEST['ticket_translations'][ $EM_Ticket->ticket_id ]['ticket_name'] ) );
				}
				if ( ! empty( $_REQUEST['ticket_translations'][ $EM_Ticket->ticket_id ]['ticket_description'] ) ) {
					$ticket_translation['ticket_description'] = wp_kses_post( wp_unslash( $_REQUEST['ticket_translations'][ $EM_Ticket->ticket_id ]['ticket_description'] ) );
				}
		...
		if ( EM_ML::is_original( $EM_Event ) ) {
			...
			foreach ( $event_translations as $language => $event_translation ) {
				if ( $language != $EM_Event->event_language ) {
					if ( empty( $event ) ) {
						$event = $event_translation; // first available translation
					}
			...
			if ( ! empty( $event->event_id ) && $EM_Event->event_id != $event->event_id ) {
				...
				$wpdb->update(
					$wpdb->postmeta,
					array( 'meta_value' => $event->event_id ),
					array(
						'meta_key'   => '_event_parent',
						'meta_value' => $EM_Event->event_id,
					)
				);
		...
		if ( ! EM_ML::is_original( $EM_Event ) ) {
			...
			$url      = add_query_arg(
				array(
					...
					'_wpnonce' => wp_create_nonce( 'event_duplicate_' . $EM_Event->event_id ),
				)
			);
	...
	public static function event_duplicate( $result, $EM_Event ) {
		...
		if ( defined( 'EM_WPML_VERSION' ) && version_compare( EM_WPML_VERSION, '2.0.2.1', '<=' ) ) {
			return $result;
		}
		if ( $result !== false && empty( $EM_ML_DUPLICATING ) ) {
			if ( ! EM_ML::is_original( $EM_Event ) ) {
				return $result; // we only duplicate the original event
			}
		...
		if ( ! EM_ML::is_original( $EM_Location ) ) {
			...
			$url         = add_query_arg(
				array(
					...
					'_wpnonce'    => wp_create_nonce( 'location_duplicate_' . $EM_Location->location_id ),
				)
			);
			//this gets escaped later
		}
	...
	public static function location_duplicate( $result, $EM_Location ) {
		...
		if ( $result !== false && empty( $EM_ML_DUPLICATING ) ) {
			if ( ! EM_ML::is_original( $EM_Location ) ) {
				return $result; // we only duplicate the original location
			}
		...
		if ( $result ) {
			...
			if ( $EM_Event instanceof EM_Event && $EM_Event->is_recurring( true ) ) {
				...
				foreach ( $EM_Event->get_bookings()->get_tickets() as $EM_Ticket ) { /* @var EM_Ticket $EM_Ticket */
					...
					if ( isset( $ticket_meta['recurrences'] ) ) {
						unset( $ticket_meta['recurrences'] );
					}
