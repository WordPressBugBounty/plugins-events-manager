...
class EM_ML {
	 ...
	 * @var array
	 */
	public static $originals_cache = array();
	...
	public static function init() {
		...
		self::$init = true;
		if ( self::$is_ml ) {
			do_action( 'em_ml_init' ); //only initialize when this is a MultiLingual instance
		}
	...
	public static function restore_locale() {
		if ( ! self::$current_language_restore ) {
			return;
		}
	...
	public static function get_translation( $object, $language ) {
		...
		if ( $object->post_id != $translated_id ) {
			if ( EM_MS_GLOBAL ) {
				if ( em_is_event( $object ) ) {
					$translated_object = em_get_event( $translated_id, $object->blog_id );
				}
				if ( em_is_location( $object ) ) {
					$translated_object = em_get_location( $translated_id, $object->blog_id );
				}
			} else {
				if ( em_is_event( $object ) ) {
	...
	public static function set_language_by_post_ids( $locale, $post_ids, $post_type, $blog_id = null, $update = false ) {
		global $wpdb;
		if ( ! preg_match( '/^[a-zA-Z_]{2,14}+$/', $locale ) ) {
			return false; //valid locale must be supplied
		}
		if ( EM_MS_GLOBAL ) {
			if ( ! $blog_id ) {
		...
		foreach ( $post_ids as $k => $post_id ) {
			$post_ids[ $k ] = absint( $post_id ); //sanitize post ids
		}
		if ( em_is_event( $post_type ) || em_is_location( $post_type ) ) {
	...
	public static function attach_translations( $locale, $post_ids_map, $post_type, $blog_id = null ) {
		...
		if ( em_is_event( $post_type ) || em_is_location( $post_type ) ) {
			...
			if ( EM_MS_GLOBAL ) {
				$sql .= ' AND blog_id=' . absint( $blog_id );
			}
			...
			$wpdb->query(
				"INSERT INTO $table_name ({$prefix}_id, {$prefix}_parent, {$prefix}_language, {$prefix}_translation) VALUES " . implode( ',', $inserts ) . "
	...
	public static function toggle_languages_index( $force = null ) {
		global $wpdb;
		$tables_keys = array(
			EM_EVENTS_TABLE    => 'event_language',
			EM_LOCATIONS_TABLE => 'location_language',
		);
