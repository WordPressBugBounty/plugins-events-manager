...
define( 'EM_DIR', __DIR__ ); //an absolute path to this directory
...
if ( ! defined( 'EM_CONDITIONAL_RECURSIONS' ) ) {
	define( 'EM_CONDITIONAL_RECURSIONS', get_option( 'dbem_conditional_recursions', 2 ) ); //allows for conditional recursios to be nested, 2 recommended due to our default template formats
}
...
require EM_DIR . '/classes/requirements-check.php';
...
//Base classes
require EM_DIR . '/classes/em-exception.php';
require EM_DIR . '/classes/em-options.php';
require EM_DIR . '/classes/em-utils.php';
require EM_DIR . '/classes/em-object.php';
require EM_DIR . '/classes/em-datetimezone.php';
require EM_DIR . '/classes/em-datetime.php';
require EM_DIR . '/classes/em-taxonomy-term.php';
require EM_DIR . '/classes/em-taxonomy-terms.php';
require EM_DIR . '/classes/em-taxonomy-frontend.php';
require EM_DIR . '/classes/uploads/em-uploads-api.php';
require EM_DIR . '/classes/uploads/em-uploads-uploader.php';
//set up events as posts
require EM_DIR . '/classes/em-archetypes.php';
require EM_DIR . '/classes/em-taxonomies.php';

//include( EM_DIR . '/em-posts.php' );
//Template Tags & Template Logic
require EM_DIR . '/em-actions.php';
require EM_DIR . '/em-events.php';
require EM_DIR . '/em-emails.php';
require EM_DIR . '/em-functions.php';
require EM_DIR . '/em-ical.php';
require EM_DIR . '/em-shortcode.php';
require EM_DIR . '/em-template-tags.php';
require EM_DIR . '/classes/consent/consent.php';
require EM_DIR . '/classes/consent/privacy-consent.php';
require EM_DIR . '/classes/consent/comms-consent.php';
require EM_DIR . '/multilingual/em-ml.php';
//Widgets
require EM_DIR . '/widgets/em-events.php';
if ( get_option( 'dbem_locations_enabled' ) ) {
	include EM_DIR . '/widgets/em-locations.php';
}
require EM_DIR . '/widgets/em-calendar.php';
//Classes
require EM_DIR . '/classes/em-list-table.php';
require EM_DIR . '/classes/em-booking.php';
require EM_DIR . '/classes/em-bookings.php';
require EM_DIR . '/classes/em-bookings-table.php';
require EM_DIR . '/classes/em-list-table-events-bookings.php';
require EM_DIR . '/classes/em-calendar.php';
require EM_DIR . '/classes/em-category.php';
require EM_DIR . '/classes/em-categories.php';
require EM_DIR . '/classes/em-categories-frontend.php';
require EM_DIR . '/classes/timeslots/timeranges.php';
require EM_DIR . '/classes/timeslots/event-timeranges.php';
require EM_DIR . '/classes/recurrences/recurrence-set.php';
require EM_DIR . '/classes/recurrences/recurrence-sets.php';
require EM_DIR . '/classes/em-event.php';
require EM_DIR . '/classes/event-locations/em-event-locations.php';
require EM_DIR . '/classes/em-event-post.php';
require EM_DIR . '/classes/em-events.php';
require EM_DIR . '/classes/em-location.php';
require EM_DIR . '/classes/em-location-post.php';
require EM_DIR . '/classes/em-locations.php';
require EM_DIR . '/classes/em-mailer.php';
require EM_DIR . '/classes/em-notices.php';
require EM_DIR . '/classes/em-people.php';
require EM_DIR . '/classes/em-person.php';
require EM_DIR . '/classes/em-permalinks.php';
require EM_DIR . '/classes/em-tag.php';
require EM_DIR . '/classes/em-tags.php';
require EM_DIR . '/classes/em-tags-frontend.php';
require EM_DIR . '/classes/em-ticket-booking.php';
require EM_DIR . '/classes/em-ticket.php';
require EM_DIR . '/classes/em-tickets-bookings.php';
require EM_DIR . '/classes/em-ticket-bookings.php';
require EM_DIR . '/classes/em-tickets.php';
require EM_DIR . '/classes/em-phone.php';
...
require EM_DIR . '/classes/em-scripts-and-styles.php';
...
function em_enqueue_public() {
	EM_Scripts_and_Styles::public_enqueue(); } //In case ppl used this somewhere
...
function em_plugins_loaded() {
	...
	if ( defined( 'WPFC_VERSION' ) ) {
		function load_em_wpfc_plugin() {
			if ( ! function_exists( 'wpfc_em_init' ) ) {
				include EM_DIR . '/em-wpfc.php';
			}
		}
...
function em_init() {
	...
	$em_capabilities_array = apply_filters(
		'em_capabilities_array',
		array(
...
if ( is_admin() ) {
	add_action( 'init', 'em_load_event', 2 ); }
...
function em_get_template_components_classes( $component ) {
	...
	return array(
		...
		'use_theme'       => absint( $show_theme_class ),
		'use_theme_admin' => $show_theme_class_admin,
	);
}
...
function em_get_template_classes( $component, $subcomponents = array(), $just_subcomponent = false ) {
	...
	if ( ! empty( $subcomponents ) ) {
		if ( ! is_array( $subcomponents ) ) {
			$subcomponents = str_replace( ' ', '', explode( ',', $subcomponents ) );
		}
...
class EM_Formats {
	...
	public static function add_filters( $get_all = false ) {
		...
		if ( is_admin() && ! empty( $_REQUEST['page'] ) && $_REQUEST['page'] == 'events-manager-options' ) {
			return; // exit on setting pages to avoid content wiping
		}
...
function em_upgrade_plugin_check() {
	if ( version_compare( EM_VERSION, get_option( 'dbem_version', 0 ), '>' ) || ( is_multisite() && ! EM_MS_GLOBAL && get_option( 'em_ms_global_install' ) ) ) {
		require_once __DIR__ . '/em-install.php';
		em_install();
	}
}
