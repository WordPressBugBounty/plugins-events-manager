	...
	public static function init() {
		...
		if ( defined( 'DUPLICATE_POST_CURRENT_VERSION' ) ) {
			if ( isset( $_GET['page'] ) && $_GET['page'] === 'duplicatepost' && isset( $GLOBALS['pagenow'] ) && $GLOBALS['pagenow'] === 'options-general.php' ) {
				add_action( 'admin_notices', array( static::class, 'yoast_duplicate_posts_admin_notices' ) );
			}
		}
		// Duplicate Page plugin
		if ( defined( 'DUPLICATE_PAGE_PLUGIN_VERSION' ) ) {
			add_action( 'admin_init', array( static::class, 'disable_duplicate_page_plugin' ) );
		}
		// Copy and Duplicate Plugin
		if ( defined( 'CDP_VERSION' ) ) {
			add_filter( 'post_row_actions', array( static::class, 'disable_copy_and_duplicate_plugin_row_actions' ), 10, 2 );
			add_filter( 'page_row_actions', array( static::class, 'disable_copy_and_duplicate_plugin_row_actions' ), 10, 2 );
			add_filter( 'option__cdp_globals', array( static::class, 'disable_copy_and_duplicate_plugin' ), 10, 1 );
		}
	}
