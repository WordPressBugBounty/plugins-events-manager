	...
	switch ( $activity->type ) {
		case 'new_booking':
			if ( $activity->component == 'groups' ) {
				$action = sprintf( __( '%1$s is attending %2$s of the group %3$s.', 'events-manager' ), $member_link, $event_link, $group_link );
			} else {
				$action = sprintf( __( '%1$s is attending %2$s.', 'events-manager' ), $member_link, $event_link );
			}
			break;
		case 'cancelled_booking':
			if ( $activity->component == 'groups' ) {
				$action = sprintf( __( '%1$s will not be attending %2$s of group %3$s anymore.', 'events-manager' ), $user_link, $event_link, $group_link );
			} else {
				$action = sprintf( __( '%1$s will not be attending %2$s anymore.', 'events-manager' ), $user_link, $event_link );
			}
			break;
	}
...
function em_bp_events_format_activity_action_events( $action, $activity ) {
	...
	$action = sprintf( __( '%1$s added the event %2$s', 'events-manager' ), $member_link, $EM_Event->output( '#_EVENTLINK' ) );
	...
	if ( $result && $EM_Event->event_status == 1 && $EM_Event->get_previous_status() != 1 ) {
		...
		if ( empty( $EM_Event->group_id ) ) {
			bp_em_record_activity(
				array(
					'user_id'       => $user->ID,
					'action'        => sprintf( __( '%1$s added the event %2$s', 'events-manager' ), "<a href='" . $member_link . "'>" . $user->display_name . '</a>', $EM_Event->output( '#_EVENTLINK' ) ),
			...
			bp_em_record_activity(
				array(
					'user_id'       => $user->ID,
					'action'        => sprintf( __( '%1$s added the event %2$s to %3$s.', 'events-manager' ), "<a href='" . $member_link . "'>" . $user->display_name . '</a>', $EM_Event->output( '#_EVENTLINK' ), '<a href="' . bp_get_group_permalink( $group ) . '">' . bp_get_group_name( $group ) . '</a>' ),
					'component'     => 'groups',
					'type'          => 'new_event',
...
function bp_em_record_activity_booking_save( $result, $EM_Booking ) {
	...
	if ( ! empty( $EM_Booking->event_id ) && $result ) {
		...
		if ( ! empty( $EM_Booking->last_bp_activity ) && $EM_Booking->last_bp_activity == $action_type ) {
			return $result; //prevent duplicates
		}
		...
		if ( empty( $EM_Event->group_id ) ) {
			if ( $status == 1 || ( ! $EM_Event->get_option( 'dbem_bookings_approval' ) && $status < 2 ) ) {
				$action = sprintf( __( '%1$s is attending %2$s.', 'events-manager' ), $user_link, $event_link );
			} elseif ( ( $EM_Booking->previous_status == 1 || ( ! $EM_Event->get_option( 'dbem_bookings_approval' ) && $EM_Booking->previous_status < 2 ) ) && in_array( $status, $rejected_statuses ) ) {
				$action = sprintf( __( '%1$s will not be attending %2$s anymore.', 'events-manager' ), $user_link, $event_link );
			...
			if ( $status == 1 || ( ! $EM_Event->get_option( 'dbem_bookings_approval' ) && $status < 2 ) ) {
				$action = sprintf( __( '%1$s is attending %2$s of the group %3$s.', 'events-manager' ), $user_link, $event_link, $group_link );
			} elseif ( ( $EM_Booking->previous_status == 1 || ( ! $EM_Event->get_option( 'dbem_bookings_approval' ) && $EM_Booking->previous_status < 2 ) ) && in_array( $status, $rejected_statuses ) ) {
				$action = sprintf( __( '%1$s will not be attending %2$s of group %3$s anymore.', 'events-manager' ), $user_link, $event_link, $group_link );
				//$action_type = 'cancelled_booking';
			}
