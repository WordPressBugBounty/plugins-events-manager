...
function em_options_save() {
	...
	if ( current_user_can( 'manage_options' ) && ! empty( $_POST['em-submitted'] ) && check_admin_referer( 'events-manager-options', '_wpnonce' ) ) {
		...
		if ( defined( 'EM_SETTINGS_ARCHETYPES_MERGED' ) && EM_SETTINGS_ARCHETYPES_MERGED || Archetypes::get_current() === Archetypes::$event['cpt'] ) {
			...
			if ( ! isset( $_POST['dbem_phone_countries_include'] ) ) {
				$_POST['dbem_phone_countries_include'] = array();
			}
			if ( ! isset( $_POST['dbem_phone_countries_exclude'] ) ) {
				$_POST['dbem_phone_countries_exclude'] = array();
			}
			...
			foreach ( $_POST as $postKey => $postValue ) {
				if ( $postKey != 'dbem_data' && substr( $postKey, 0, 5 ) == 'dbem_' ) {
					...
					if ( in_array( $postKey, array( 'dbem_bookings_notify_admin', 'dbem_event_submitted_email_admin', 'dbem_js_limit_events_form', 'dbem_js_limit_search', 'dbem_js_limit_general', 'dbem_css_limit_include', 'dbem_css_limit_exclude', 'dbem_search_form_geo_distance_options' ) ) ) {
						$postValue = str_replace( ' ', '', $postValue ); } //clean up comma separated emails, no spaces needed
					if ( in_array( $postKey, $numeric_options ) && ! is_numeric( $postValue ) ) {
						...
						if ( is_array( $postValue ) ) {
							foreach ( $postValue as $postValue_key => $postValue_val ) {
								$postValue[ $postValue_key ] = wp_unslash( $postValue_val );
							}
						} else {
					...
					foreach ( $postValue as $postK => $postV ) {
						//TODO slashes being added?
						if ( is_array( $postV ) ) {
							foreach ( $postV as $postValue_key => $postValue_val ) {
								$postV[ $postValue_key ] = wp_unslash( $postValue_val );
							}
						} else {
	...
	if ( ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] == 'cleanup_event_orphans' && check_admin_referer( 'em_cleanup_event_orphans_' . get_current_user_id() . '_wpnonce' ) && em_wp_is_super_admin() ) {
		...
		//go back to plugin options page
		$EM_Notices->add_confirm( sprintf( __( 'Found %1$d orphaned events, deleted %2$d successfully', 'events-manager' ), count( $results ), $deleted_events ), true );
		wp_safe_redirect( em_wp_get_referer() );
		exit();
	}
			...
			if ( is_array( $settings ) ) {
				if ( is_multisite() && $_REQUEST['action'] == 'import_em_ms_settings' ) {
					...
					foreach ( $settings as $k => $v ) {
						if ( in_array( $k, $sitewide_options ) ) {
	...
	if ( ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] == 'export_em_settings' && check_admin_referer( 'export_em_settings' ) && em_wp_is_super_admin() ) {
		...
		$options          = array();
		$sitewide_options = $EM_MS_Globals->get_globals();
		foreach ( $sitewide_options as $option ) {
		...
		if ( $post_deletion !== false ) {
			...
			if ( $meta_deletion !== false ) {
				...
				if ( $update_events !== false ) {
					$wpdb->update( EM_EVENTS_TABLE, array( 'event_type' => 'recurring' ), array( 'event_type' => 'repeating' ) );
					$wpdb->update(
						$wpdb->postmeta,
						array( 'meta_value' => 'recurring' ),
						array(
							'meta_key'   => '_event_type',
							'meta_value' => 'repeating',
						)
					);
					$wpdb->update( $wpdb->posts, array( 'post_type' => EM_POST_TYPE_EVENT ), array( 'post_type' => 'event-recurring' ) );
	...
	if ( ! empty( $_REQUEST['action'] ) && $_REQUEST['action'] == 'reset_timezones' && check_admin_referer( 'reset_timezones' ) && em_wp_is_super_admin() ) {
		...
		if ( empty( $_REQUEST['timezone_reset_value'] ) ) {
			return;
		}
		...
		if ( is_multisite() ) {
			if ( ! empty( $_REQUEST['timezone_reset_blog'] ) && is_numeric( $_REQUEST['timezone_reset_blog'] ) ) {
				...
				foreach ( $blog_ids as $k => $blog_id ) {
					$result          = true;
					$plugin_basename = plugin_basename( dirname( __DIR__, 1 ) . '/events-manager.php' );
		...
		if ( $result !== true ) {
			...
			if ( is_multisite() ) {
				if ( $_REQUEST['timezone_reset_blog'] == 'all' || $_REQUEST['timezone_reset_blog'] == 'all-resume' ) {
					...
					$EM_Notices->add_confirm( sprintf( __( 'Event timezones for blog %1$s have been reset to %2$s.', 'events-manager' ), '<code>' . $blog_name . '</code>', '<code>' . $timezone . '</code>' ), true );
				}
			} else {
...
function em_options_save_kses_deep( $string ) {
	if ( ( defined( 'EM_UNFILTERED_HTML' ) && EM_UNFILTERED_HTML ) || current_user_can( 'unfiltered_html' ) ) {
		return $string;
	}
	if ( is_array( $string ) ) {
...
function em_admin_email_test_ajax() {
	if ( wp_verify_nonce( $_REQUEST['_check_email_nonce'], 'check_email' ) && current_user_can( 'activate_plugins' ) ) {
		...
		function pre_option_dbem_mail_sender_name() {
			return sanitize_text_field( $_REQUEST['dbem_mail_sender_name'] ); }
		...
		function pre_option_dbem_smtp_encryption() {
			return sanitize_text_field( $_REQUEST['dbem_smtp_encryption'] ); }
		add_filter( 'pre_option_dbem_smtp_encryption', 'pre_option_dbem_smtp_encryption' );
		function pre_option_dbem_smtp_autotls() {
			return sanitize_text_field( $_REQUEST['dbem_smtp_autotls'] ); }
		...
		<div class="wrap">		
			...
			<p>
				...
				echo esc_url(
					add_query_arg(
						array(
							'_wpnonce2' => wp_create_nonce( 'em_reset_' . get_current_user_id() . '_confirmed' ),
							'confirmed' => 1,
						)
					)
				);
				?>
							" class="button-primary"><?php _e( 'Reset Events Manager', 'events-manager' ); ?></a>
		...
		<div class="wrap">		
			...
			<p>
				...
				echo esc_url(
					add_query_arg(
						array(
							'_wpnonce2' => wp_create_nonce( 'em_uninstall_' . get_current_user_id() . '_confirmed' ),
							'confirmed' => 1,
						)
					)
				);
				?>
							" class="button-primary"><?php _e( 'Uninstall and Deactivate', 'events-manager' ); ?></a>
				<a href="<?php echo esc_url( em_wp_get_referer() ); ?>" class="button-secondary"><?php _e( 'Cancel', 'events-manager' ); ?></a>
			</p>
		</div>		
		<?php
...
function em_admin_options_page() {
	...
	$events_placeholder_tip     = ' ' . sprintf( __( 'This accepts %1$s and %2$s placeholders.', 'events-manager' ), $events_placeholders, $locations_placeholders );
	...
	$bookings_placeholder_tip   = ' ' . sprintf( __( 'This accepts %1$s, %2$s and %3$s placeholders.', 'events-manager' ), $bookings_placeholders, $events_placeholders, $locations_placeholders );
	...
	<div id="em-options-page" class="wrap 
	<?php
	if ( empty( $tabs_enabled ) ) {
		echo 'tabs-active';}
	?>
	" data-archetype="<?php echo $current_archetype; ?>" data-archetype-mode="<?php echo defined( 'EM_SETTINGS_ARCHETYPES_MERGED' ) && EM_SETTINGS_ARCHETYPES_MERGED ? 'merged' : 'single'; ?>">
		<h1 id="em-options-title"><?php _e( 'Event Manager Options', 'events-manager' ); ?></h1>
		<?php
		if ( ! empty( Archetypes::$types ) ) :
			$i = 0;
			?>
		...
		<form id="em-options-form" method="post" action="">
			...
			if ( ! empty( $tabs_enabled ) ) {
				if ( empty( $_REQUEST['em_tab'] ) || $_REQUEST['em_tab'] == 'general' ) {
					...
					if ( $_REQUEST['em_tab'] == 'pages' ) {
						include 'settings/tabs/pages.php';
					}
					if ( $_REQUEST['em_tab'] == 'formats' ) {
...
function em_admin_option_box_email() {
	...
	<div class="inside em-email-form">
		...
		<table class="form-table">
			...
			em_options_select(
				__( 'Mail sending method', 'events-manager' ),
				'dbem_rsvp_mail_send_method',
				array(
					'smtp'     => 'SMTP',
					'mail'     => __( 'PHP mail function', 'events-manager' ),
					'sendmail' => 'Sendmail',
					'qmail'    => 'Qmail',
					'wp_mail'  => 'WP Mail',
				),
				__( 'Select the method to send email notification.', 'events-manager' )
			);
			...
			<tbody class="em-email-settings-smtp">
				...
				em_options_select(
					__( 'SMTP Encryption', 'events-manager' ),
					'dbem_smtp_encryption',
					array(
						'0'   => __( 'None', 'events-manager' ),
						'ssl' => 'SSL',
						'tls' => 'TLS',
					),
					__( 'Encryption is always recommended if your SMTP server supports it. If your server supports TLS, this is also the most recommended method.', 'events-manager' )
				);
					...
					<thead>
						<tr>
							...
							foreach ( array_keys( $cap_docs ) as $capability_group ) {
								?>
								<th class="<?php echo ( ! is_int( $odd / 2 ) ) ? 'odd' : ''; ?>"><?php echo $capability_group; ?></th>
								<?php
								++$odd;
							}
							?>
			...
			<table class="form-table">
				...
				<tr><td colspan="2">
						...
						$convert_url   = esc_url(
							add_query_arg(
								array(
									'action' => 'convert_repeating_to_recurrence',
									'nonce'  => 'x',
								)
							)
						);
