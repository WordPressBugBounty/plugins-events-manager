...
class Dashboard {
	...
	public static function get_stats( $args ) {
		...
		if ( count( $scopes ) > 1 ) {
			...
			$title   = esc_html__( '%1$s  |  %2$s  vs  %3$s' );
			...
			if ( count( $selectors ) > 1 ) {
				// comparing units
				$title              = esc_html__( '%1$s  vs  %2$s  |  %3$s' );
	...
	public static function output( $view = 'dashboard', $item_id = null ) {
		...
		<div class="
		<?php
		em_template_classes( 'chart', 'bookings-chart' );
						...
						<div class="em-chart-filter-set dataset-1">
							...
							<div class="em-datepicker em-datepicker-range em-chart-dates-custom" data-until-id="em-chart-dates-compare-<?php echo $id; ?>" data-datepicker="<?php echo esc_js( json_encode( (object) array( 'allowInput', false ) ) ); ?>">
								...
								<div class="em-datepicker-data">
									<input type="date" name="range_dates[0][0]" value="
									<?php
									if ( ! empty( $args['range_dates'][0][0] ) ) {
										echo esc_attr( $args['range_dates'][0][0] );}
									?>
									" aria-label="<?php esc_html_e( 'From ', 'events-manager' ); ?>">
									<span class="separator"><?php esc_html_e( 'to', 'events-manager' ); ?></span>
									<input type="date" name="range_dates[0][1]" value="
									<?php
									if ( ! empty( $args['range_dates'][0][1] ) ) {
										echo esc_attr( $args['range_dates'][0][1] );}
									?>
									" aria-label="<?php esc_html_e( 'to', 'events-manager' ); ?>">
								</div>
							</div>
							<label>
						...
						<div class="em-chart-filter-set dataset-2">
							...
							<div>
								<div class="em-datepicker em-chart-dates-compare em-tooltip" id="em-chart-dates-compare-<?php echo $id; ?>" data-datepicker="<?php echo esc_js( json_encode( (object) array( 'allowInput', false ) ) ); ?>" aria-label="<?php esc_attr_e( 'Choose the start date for comparing equivalent date ranges. The latest comparison end date is day before main duration start date.', 'events-manager' ); ?>" data-tippy-placement="bottom" data-tippy-theme="dark">
									...
									<div class="em-datepicker-data">
										<input type="date" name="range_dates[1]" value="
										<?php
										if ( ! empty( $args['range_dates'][1] ) ) {
											echo esc_attr( $args['range_dates'][1] );}
										?>
										" aria-label="<?php esc_html_e( 'From ', 'events-manager' ); ?>">
									</div>
								</div>
				...
				<div class="em-modal <?php em_template_classes( 'chart-settings-modal' ); ?>" id="em-chart-settings-<?php echo $id; ?>" data-parent="em-chart-form-<?php echo $id; ?>" style="opacity:0;">
					<div class="em-modal-popup">
						...
						<div class="em-modal-content input">
							<p>
								<label><input type="checkbox" name="show_filters" value="1" 
								<?php
								if ( ! empty( $args['show_filters'] ) ) {
									echo 'checked';}
								?>
								> <?php esc_html_e( 'Show search filters on page load', 'events-manager' ); ?></label>
							</p>
							<section>
								...
								<div>
									...
									<p>
										<label><input type="checkbox" name="summaries[price]" value="1" 
										<?php
										if ( ! empty( $args['summaries']['price'] ) ) {
											echo 'checked';}
										?>
										> <?php esc_html_e( 'Total Sales', 'events-manager' ); ?></label>
										<label><input type="checkbox" name="summaries[spaces]" value="1" 
										<?php
										if ( ! empty( $args['summaries']['spaces'] ) ) {
											echo 'checked';}
										?>
										> <?php esc_html_e( 'Total Tickets', 'events-manager' ); ?></label>
										<label><input type="checkbox" name="summaries[bookings]" value="1" 
										<?php
										if ( ! empty( $args['summaries']['bookings'] ) ) {
											echo 'checked';}
										?>
										> <?php esc_html_e( 'Total Bookings', 'events-manager' ); ?></label>
										<label><input type="checkbox" name="summaries[price_avg]" value="1" 
										<?php
										if ( ! empty( $args['summaries']['price_avg'] ) ) {
											echo 'checked';}
										?>
										> <?php esc_html_e( 'Average Sale', 'events-manager' ); ?></label>
										<label><input type="checkbox" name="summaries[spaces_avg]" value="1" 
										<?php
										if ( ! empty( $args['summaries']['spaces_avg'] ) ) {
											echo 'checked';}
										?>
										> <?php esc_html_e( 'Average Ticket', 'events-manager' ); ?></label>
										<label><input type="checkbox" name="summaries[tickets_avg]" value="1" 
										<?php
										if ( ! empty( $args['summaries']['tickets_avg'] ) ) {
											echo 'checked';}
										?>
										> <?php esc_html_e( 'Tickets / Booking', 'events-manager' ); ?></label>
									</p>
								</div>
							</section>
						</div><!-- content -->
