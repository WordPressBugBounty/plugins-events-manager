...
function em_admin_menu() {
	...
	foreach ( Archetypes::get_cpts( array(), array( 'event', 'types' ) ) as $post_type ) {
		...
		if ( em_get_option( 'dbem_rsvp_enabled' ) ) {
			...
			if ( em_get_option( 'dbem_bookings_approval' ) == 1 ) {
				$bookings_pending_count += EM_Bookings::count(
					array(
						'status' => '0',
		...
		$events_num           = '';
		$events_pending_count = EM_Events::count(
			array(
				'status' => 0,
				'scope'  => 'all',
				'blog'   => get_current_blog_id(),
		...
		$events_recurring_pending_count = EM_Events::count(
			array(
				'status'    => 0,
				'recurring' => 1,
				'scope'     => 'all',
		...
		if ( EM_MS_GLOBAL && ! is_main_site() && get_site_option( 'dbem_ms_mainblog_locations' ) ) {
			include __DIR__ . '/em-ms-locations.php';
...
function em_admin_warnings() {
	...
	if ( current_user_can( 'activate_plugins' ) ) {
		...
		if ( ! empty( $_GET['em_dismiss_events_page'] ) && wp_verify_nonce( $_GET['em_dismiss_events_page'], 'em_dismiss_notice' ) ) {
			...
			if ( ! get_page( $events_page_id ) && ! em_get_option( 'dbem_dismiss_events_page' ) ) {
				...
				<div id="em_page_error" class="updated">
					<p><?php echo sprintf( __( 'Uh Oh! For some reason WordPress could not create an events page for you (or you just deleted it). Not to worry though, all you have to do is create an empty page, name it whatever you want, and select it as your events page in your <a href="%1$s">settings page</a>. Sorry for the extra step! If you know what you are doing, you may have done this on purpose, if so <a href="%2$s">ignore this message</a>', 'events-manager' ), EM_ADMIN_URL . '&amp;page=events-manager-options', esc_url( $_SERVER['REQUEST_URI'] . $dismiss_link_joiner . 'em_dismiss_events_page=' . wp_create_nonce( 'em_dismiss_notice' ) ) ); ?></p>
				</div>
				<?php
			}
		}
		...
		if ( is_multisite() && ! empty( $_REQUEST['page'] ) && $_REQUEST['page'] == 'events-manager-options' && em_wp_is_super_admin() && em_get_option( 'dbem_ms_update_nag' ) ) {
			if ( ! empty( $_GET['disable_dbem_ms_update_nag'] ) && wp_verify_nonce( $_GET['disable_dbem_ms_update_nag'], 'em_dismiss_notice' ) ) {
				...
				?>
				<div id="em_page_error" class="updated">
					<p><?php echo sprintf( __( 'MultiSite options have moved <a href="%1$s">here</a>. <a href="%2$s">Dismiss message</a>', 'events-manager' ), admin_url() . 'network/admin.php?page=events-manager-options', esc_url( $_SERVER['REQUEST_URI'] . '&amp;disable_dbem_ms_update_nag=' . wp_create_nonce( 'em_dismiss_notice' ) ) ); ?></p>
				</div>
				<?php
			}
		}
		...
		if ( class_exists( 'SitePress' ) && ! class_exists( 'EM_WPML' ) && ! get_site_option( 'disable_em_wpml_warning' ) ) {
			if ( ! empty( $_REQUEST['disable_em_wpml_warning'] ) && wp_verify_nonce( $_GET['disable_em_wpml_warning'], 'em_dismiss_notice' ) ) {
				...
				?>
				<div id="message" class="updated">
					<p><?php echo sprintf( __( 'It looks like you have WPML enabled on your site. We advise you also install our extra <a href="%1$s">Events Manager WPML Connector</a> plugin which helps the two work better together. <a href="%2$s">Dismiss message</a>', 'events-manager' ), 'http://wordpress.org/extend/plugins/events-manager-wpml/', esc_url( add_query_arg( array( 'disable_em_wpml_warning' => wp_create_nonce( 'em_dismiss_notice' ) ) ) ) ); ?></p>
				</div>
				<?php
			}
		}
	...
	if ( preg_match( '/(post|page).php/', $_SERVER ['SCRIPT_NAME'] ) && isset( $_GET ['action'] ) && $_GET ['action'] == 'edit' && isset( $_GET ['post'] ) && $_GET ['post'] == "$events_page_id" ) {
		$message = sprintf( __( "This page corresponds to the <strong>Events Manager</strong> %1\$s page. Its content will be overridden by Events Manager, although if you include the word CONTENTS (exactly in capitals) and surround it with other text, only CONTENTS will be overwritten. If you want to change the way your events look, go to the <a href='%2\$s'>settings</a> page. ", 'events-manager' ), __( 'Events', 'events-manager' ), EM_ADMIN_URL . '&amp;page=events-manager-options' );
...
add_filter(
	'em_admin_notice_v7-reconvert-recurrences_message',
	function () {
		ob_start();
		?>
	...
	<p>We recommend running the conversion one more time due to some issues that may have prevented the full conversion in versions 7.0 - 7.0.2.</p>
		...
		$convert_url   = esc_url(
			add_query_arg(
				...
				EM_ADMIN_URL . '&page=events-manager-options'
			)
		);
