...
function em_updates_check( $transient ) {
    ...
    //only bother if we're checking for dev or stable versions
    //these checks run for manual update requests only; during cron they would feed unattended auto-updates and defeat the WordPress.org staggered rollout
    $interactive = ! wp_doing_cron();
    $check_dev    = $interactive && ( em_get_option('em_check_dev_version') || em_get_option('dbem_pro_dev_updates') );
    $check_stable = $interactive && ( em_get_option('em_check_stable_version') || em_get_option('dbem_check_stable_version') );
    if( $check_dev || $check_stable ){
	    //check WP repo (trunk for dev, readme Stable tag for stable), other EM-related plugins on .org can hook here to make the best of our admin setting option
	    ...
	    foreach( $plugins as $org_slug => $plugin_info ) {
		    ...
		    if( $check_dev ){
			    ...
			    if( !is_wp_error($request) && preg_match('/Version: ([0-9a-z\.]+)/', $request['body'], $matches) ) $new_version = $matches[1];
		    }else{
			    ...
			    if( !is_wp_error($request) && preg_match('/Stable tag:\s*([0-9a-z\.\-]+)/i', $request['body'], $matches) && strtolower($matches[1]) !== 'trunk' ){
		    ...
		    if (!is_wp_error($request)) {
			    if (!empty($new_version)) {
				    ...
				    //flagged so em_prevent_manual_only_autoupdate() keeps bypassed versions out of unattended auto-updates
				    ...
				    if ( version_compare($transient->checked[$wp_slug], $new_version) < 0) {
...
add_filter('pre_set_site_transient_update_plugins', 'em_updates_check', 100); // Hook into the plugin update check and mod for dev/stable version

//Versions surfaced by em_updates_check() deliberately bypass the WordPress.org staggered rollout, so they must never drive unattended background auto-updates
function em_prevent_manual_only_autoupdate( $update, $item ){
	if( !empty($item->em_manual_only) ) return false;
	return $update;
}
