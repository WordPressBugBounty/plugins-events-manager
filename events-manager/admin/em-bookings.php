...
function em_bookings_page() {
	...
	if ( ! empty( $_REQUEST['_wpnonce'] ) ) {
		$_REQUEST['_wpnonce'] = $_GET['_wpnonce'] = $_POST['_wpnonce'] = esc_attr( $_REQUEST['_wpnonce'] ); } //XSS fix just in case here too
...
function em_bookings_event() {
	...
	?>
				</h1><?php endif; // inverted to help IDEs ?>
			...
			<div class="em-notice em-notice-icon" >
				...
				?>
				<span><?php echo sprintf( esc_html__( 'This is a recurring %1$s. You can find all %2$s on this page, or click on individual %3$s to view recurrence-specific bookings.', 'events-manager' ), $label_single, $recurrence_bookings_link, $event_recurrences_link ); ?></span>
			...
			<?php elseif ( $EM_Event->is_recurrence() || ( $EM_Event->is_timeslot() && $EM_Event->get_parent()->is_recurrence() ) ) : ?>
				<div class="em-notice em-notice-icon" >
					...
					?>
					<span><?php echo sprintf( esc_html__( 'This is a %1$s recurrence of %2$s.', 'events-manager' ), strtolower( $label_single ), $recurring_link ); ?></span>
					<?php if ( ! empty( $timeslots ) ) : ?>
						<span></span><?php echo $timeslots; ?>
					<?php endif; ?>
				</div>
			...
			<div class="metabox-holder">
				...
				<div class="postbox">
					...
					<div class="inside">
							...
							<form action="" method="post" class="em-booking-single-status-edit em-booking-single-rsvp-status-info">
								...
								<select name="booking_rsvp_status">
									<?php foreach ( $EM_Booking::get_rsvp_statuses() as $status => $status_type ) : ?>
										<option value="<?php echo esc_attr( $status ); ?>" 
										<?php
										if ( $status === $EM_Booking->get_rsvp_status() ) {
											echo 'selected="selected"'; }
										?>
										><?php echo esc_html( $status_type->label ); ?></option>
									<?php endforeach; ?>
								</select>
								<input type="submit" class="button-primary em-button em-booking-submit-status" id="em-booking-submit-status" value="<?php esc_attr_e( 'Submit Changes', 'events-manager' ); ?>" />
								<input type="button" class="button-secondary em-button em-booking-submit-status-cancel" id="em-booking-submit-status-cancel" value="<?php esc_attr_e( 'Cancel', 'events-manager' ); ?>" />
						...
						<?php endif; ?>
						<form action="" method="post" class="em-booking-form" id="em-booking-form-<?php echo $id; ?>" data-id="<?php echo $id; ?>" 
						<?php
						if ( ! em_get_option( 'dbem_uploads_ui' ) ) {
							echo ' enctype="multipart/form-data"';}
						?>
						data-noajax data-mode="view">
							<table class="em-tickets em-tickets-bookings-table" cellpadding="0" cellspacing="0">
								...
								foreach ( $EM_Event->get_bookings()->get_tickets()->tickets as $ticket_id => $EM_Ticket ) {
									if ( empty( $EM_Tickets_Bookings[ $ticket_id ] ) ) {
										$tickets_bookings[ $ticket_id ] = new EM_Ticket_Bookings(
											array(
												'ticket'  => $EM_Ticket,
												'booking' => $EM_Booking,
											)
								...
								<?php foreach ( $tickets_bookings as $ticket_id => $EM_Ticket_Bookings ) : /* @var $EM_Ticket_Bookings EM_Ticket_Bookings */ ?>
									...
									<tbody class="em-ticket-bookings 
									<?php
									if ( ! has_action( 'em_bookings_admin_ticket_booking_row' ) ) {
										echo 'hidden';}
									?>
									em-ticket-bookings-<?php echo $ticket_id; ?>" data-ticket-id="<?php echo $ticket_id; ?>">
										<?php
											// use the following
							...
							<table class="em-form-fields" cellspacing="0" cellpadding="0">
								...
								<tr>
									...
									do_action( 'em_bookings_single_custom', $EM_Booking ); //do your own thing, e.g. pro
									?>
								<?php endif; ?>
							</table>
