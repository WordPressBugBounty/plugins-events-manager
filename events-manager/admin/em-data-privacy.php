...
class EM_Data_Privacy {
	...
	public static function erase_bookings( $email_address, $page = 1 ) {
		...
		if ( $user !== false && em_get_option( 'dbem_data_privacy_erase_bookings' ) == 2 ) {
			...
			if ( $items_removed ) {
				add_action( 'em_data_privacy_bookings_deleted', $booking_ids );
			}
	...
	public static function erase_events( $email_address, $page = 1, $post_type = false ) {
		$user = get_user_by( 'email', $email_address ); //is user or no-user?
		if ( ! $post_type ) {
			$post_type = EM_POST_TYPE_EVENT; //default to event
		}
		$page          = (int) $page;
			...
			foreach ( $events as $post_id ) {
				...
				if ( $EM_Location->location_id && em_get_option( 'dbem_data_privacy_erase_locations', 2 ) ) {
					...
					if ( $user_probably_owns_location ) {
						...
						if ( em_get_option( 'dbem_data_privacy_erase_locations', 2 ) == 1 || EM_Events::count(
							array(
	...
	public static function export_bookings( $email_address, $page = 1 ) {
		...
		$user = get_user_by( 'email', $email_address ); //is user or no-user?
		if ( $user !== false && em_get_option( 'dbem_data_privacy_export_bookings' ) == 2 ) {
			return array(
				'data' => $export_items,
				'done' => true,
		...
		foreach ( $bookings as $booking_id ) {
			...
			$export_item['data']['status'] = array(
				'name'  => __( 'Status', 'events-manager' ),
				'value' => $EM_Booking->get_status(),
			);
			$export_item['data']['date']   = array(
				'name'  => __( 'Date', 'events-manager' ),
				'value' => $EM_Booking->date()->getDateTime() . ' ' . $EM_Booking->date()->getTimezone()->getName(),
			);
			$export_item['data']['event']  = array(
				'name'  => __( 'Event', 'events-manager' ),
				'value' => $EM_Booking->get_event()->output( '#_EVENTLINK - #_EVENTDATES @ #_EVENTTIMES' ),
			);
			...
			$export_item['data']['tickets']   = array(
				'name'  => __( 'Tickets', 'events-manager-pro' ),
				'value' => implode( '<br>', $booking_tickets ),
			);
			$export_item['data']['sub-total'] = array(
				'name'  => __( 'Sub Total', 'events-manager' ),
				'value' => $EM_Booking->get_price_base( true ),
			);
			...
			foreach ( $booking_price_adjustments as $adjustment_key => $adjustment_title ) {
				if ( count( $price_summary[ $adjustment_key ] ) > 0 ) {
					...
					$export_item['data'][ $adjustment_key ] = array(
						'name'  => $adjustment_title,
						'value' => implode( '<br>', $adjustments ),
					);
				}
			}
			if ( ! empty( $price_summary['taxes']['amount'] ) ) {
				$export_item['data']['taxes'] = array(
					'name'  => __( 'Taxes', 'events-manager' ),
					'value' => "({$price_summary['taxes']['rate']}) {$price_summary['taxes']['amount']}",
				);
			}
			$export_item['data']['total'] = array(
				'name'  => __( 'Total Price', 'events-manager' ),
				'value' => $price_summary['total'],
			);
			...
			if ( apply_filters( 'em_data_privacy_export_bookings_include_notes', true, $EM_Booking ) ) {
				...
				if ( ! empty( $booking_notes ) ) {
					...
					$export_item['data']['notes'] = array(
						'name'  => __( 'Booking Notes', 'events-manager' ),
						'value' => implode( '<br><br>', $booking_notes_data ),
					);
				}
			}
	...
	public static function export_events( $email_address, $page = 1, $post_type = false ) {
		...
		if ( $user !== false && em_get_option( 'dbem_data_privacy_export_events' ) == 2 ) {
			return array(
				'data' => $export_items,
				'done' => true,
		...
		if ( empty( $locations_exported ) ) {
			$locations_exported = array();
		}
		...
		foreach ( $events as $post_id ) {
			...
			$export_item['data'][]  = array(
				'name'  => __( 'Event Name', 'events-manager' ),
				'value' => $EM_Event->event_name,
			);
			$export_item['data'][]  = array(
				'name'  => sprintf( __( '%s Status', 'events-manager' ), __( 'Event', 'events-manager' ) ),
				'value' => $EM_Event->post_status,
			);
			if ( $post_type == EM_POST_TYPE_EVENT && $EM_Event->event_status == 1 ) {
				$export_item['data'][] = array(
					'name'  => sprintf( __( '%s URL', 'events-manager' ), __( 'Event', 'events-manager' ) ),
					'value' => $EM_Event->get_permalink(),
				);
			}
			if ( $post_type == 'event-recurring' ) {
				$export_item['data'][] = array(
					'name'  => __( 'When', 'events-manager' ),
					'value' => $EM_Event->output( '#_EVENTDATES @ #_EVENTTIMES' ) . '<br>' . $EM_Event->get_recurrence_description(),
				);
			} else {
				$export_item['data'][] = array(
					'name'  => __( 'When', 'events-manager' ),
					'value' => $EM_Event->output( '#_EVENTDATES @ #_EVENTTIMES' ),
				);
			}
			$export_item['data'][] = array(
				'name'  => __( 'Timezone', 'events-manager' ),
				'value' => $EM_Event->start()->getTimezone()->getName(),
			);
			if ( ! empty( $EM_Event->event_owner_name ) ) {
				$export_item['data'][] = array(
					'name'  => __( 'Name', 'events-manager' ),
					'value' => $EM_Event->event_owner_name,
				);
			}
			if ( $EM_Event->get_location()->location_id ) {
				...
				$export_item['data'][] = array(
					...
					'value' => $EM_Location->location_name . ', ' . $EM_Location->get_full_address() . ', ' . $EM_Location->location_country,
				);
				...
				if ( ! $already_exported && ( $user_owns_location || $user_submitted_location || $user_probably_owns_location ) ) {
					...
					$location_export_item['data'][]  = array(
						'name'  => __( 'Name', 'events-manager' ),
						'value' => $EM_Location->location_name,
					);
					$location_export_item['data'][]  = array(
						'name'  => __( 'Address', 'events-manager' ),
						'value' => $EM_Location->get_full_address() . ', ' . $EM_Location->location_country,
					);
					$location_export_item['data'][]  = array(
						'name'  => __( 'Coordinates', 'events-manager' ),
						'value' => $EM_Location->location_latitude . ', ' . $EM_Location->location_longitude,
					);
					$location_export_item['data'][]  = array(
						'name'  => sprintf( __( '%s Status', 'events-manager' ), __( 'Location', 'events-manager' ) ),
						'value' => $EM_Location->post_status,
					);
					if ( $EM_Location->post_status == 'publish' ) {
						$location_export_item['data'][] = array(
							'name'  => sprintf( __( '%s URL', 'events-manager' ), __( 'Location', 'events-manager' ) ),
							'value' => $EM_Location->get_permalink(),
						);
					}
			...
			if ( $EM_Event->event_rsvp ) {
				...
				foreach ( $EM_Event->get_tickets() as $EM_Ticket ) { /* @var EM_Ticket $EM_Ticket */
					...
					if ( empty( $EM_Ticket->description ) ) {
						unset( $ticket[1] );
					}
					$tickets[] = implode( ' - ', $ticket );
				}
				$export_item['data'][] = array(
					'name'  => __( 'Tickets', 'events-manager' ),
					'value' => implode( '<br>', $tickets ),
				);
			}
