	...
	<div class="inside">
		...
		<table class="form-table">
			...
			<tbody class="em-archetypes-settings">
				<?php
				em_options_select(
					__( 'Subsite Archetype Mode', 'events-manager' ),
					'dbem_ms_archetypes_mode',
					array(
						'custom'  => __( 'Allow subsites to create their own', 'events-manager' ),
						'choose'  => __( 'Subsites choose from network list', 'events-manager' ),
						'network' => __( 'Force network-defined list (no subsite choice)', 'events-manager' ),
					)
				);
				?>
		...
		<table class="form-table em-archetypes-settings" id="event-archetypes-container">
			...
			<tbody class="base-event-form" data-group="archetypes-base-event">
				...
				if ( Archetypes_Admin::can_rename_slugs() ) {
					em_options_input_text(
						__( 'Permalink Slug', 'events-manager' ),
						'dbem_cp_events_slug',
			...
			<tbody class="archetypes-select-ui em-subsection">
				...
				<tr>
					<td colspan="2">
						<?php
							$available = (array) get_site_option( 'em_ms_event_archetypes', array() );
							$selected  = (array) get_option( 'em_archetypes_selected', array() );
							$default   = get_option( 'em_archetype_default', '' );
							$list      = array_merge( array( Archetypes::$event['cpt'] => array( 'label' => Archetypes::$event['label'] ?: __( 'Events', 'events-manager' ) ) ), $available );
						?>
						<table id="em-archetypes-select">
							...
							<tbody>
							<?php
							foreach ( Archetypes::get_cpts( array(), array( 'event', 'types' ) ) as $cpt ) :
							...
							<?php foreach ( $custom_types as $key => $type ) : ?>
								...
								if ( $is_template ) {
									$archetype = array(
										...
										'cpt'          => '',
										'cpts'         => '',
										'taxonomies'   => array( 'event-tags', 'event-categories' ),
										'menu_icon'    => '',
									);
								} else {
									$archetype = $type;
								...
								if ( $is_template ) :
									?>
									<template id="archetype-template"><?php endif; ?>
