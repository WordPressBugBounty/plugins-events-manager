	...
	<div class="wrap">
		...
		<p>
			...
			echo esc_url(
				add_query_arg(
					array(
						'_wpnonce'  => wp_create_nonce( 'em_timezone_backcompat_confirmed' ),
						'confirmed' => 1,
					)
				)
			);
			?>
						" class="button-primary"><?php _e( 'Finalize Timezones Upgrade', 'events-manager' ); ?></a>
	...
	<div>
		...
		<p>
			...
			echo esc_url(
				add_query_arg(
					array(
						'action'   => 'timezone-backcompat',
						'_wpnonce' => wp_create_nonce( 'em_ms_finalize_timezone_upgrade' ),
					)
				)
			);
			?>
