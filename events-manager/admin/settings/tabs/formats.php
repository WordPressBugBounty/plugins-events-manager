<?php if ( ! function_exists( 'current_user_can' ) || ! current_user_can( 'manage_options' ) ) {
	return;} ?>
<!-- FORMAT OPTIONS -->
<div class="em-menu-formats em-menu-group"  
<?php
if ( ! defined( 'EM_SETTINGS_TABS' ) || ! EM_SETTINGS_TABS ) :
	...
	<div  class="postbox em-postbox-notice em-postbox-notice-info" id="em-opt-advanced-formatting" data-enabled-text="(<?php echo esc_attr( $enabled_text ); ?>)" data-disabled-text="(<?php echo esc_attr( $disabled_text ); ?>)">
		...
		<div class="inside">
			<div class="em-default-option em-advanced-formatting">
				...
				<?php
				function em_options_advanced_formatting_content( $current_status ) {
					?>
				<div>
					<?php
				...
				<div>
					...
					<p>
						<?php echo sprintf( esc_html__( "%1\$s allows you to modify some or all of the formats used by Events Manager to generate your content. You can choose to override default formats and customize them to your needs. You do not lose your previously defined custom formats should you enable/disable %2\$s or choose to not override a certain set of formats, they'll appear again once you enable %3\$s again for that content.", 'events-manager' ), $am, $am, $am ); ?>
					</p>
					<p>
						<?php echo sprintf( esc_html__( '%1$s overrides all formatting choices, which will be be taken from the settings defined on this page. If you want to override only certain formatting, choose the %2$s instead.', 'events-manager' ), $sam, $am ); ?>
					</p>
					<p>
	...
	<div class="inside">
		<table class="form-table">
			...
			<tbody>
				...
				<?php
				$grouby_modes = array(
					'yearly'  => __( 'Yearly', 'events-manager' ),
					'monthly' => __( 'Monthly', 'events-manager' ),
					'weekly'  => __( 'Weekly', 'events-manager' ),
					'daily'   => __( 'Daily', 'events-manager' ),
				);
			...
			<tbody class="am-af" data-group="format-event-single">
				<tr class="em-header">
					<td colspan="2">
						...
						<em><?php echo sprintf( __( 'These formats can be used on %1$s pages or on other areas of your site displaying an %2$s.', 'events-manager' ), __( 'event', 'events-manager' ), __( 'event', 'events-manager' ) ); ?></em>
					</td>
				</tr>
				<?php
	...
	<div class="inside">
		<?php
			...
			$screen_desc           = esc_html__( 'When the size of the %1$s is less than %2$s, this field can be hidden on the main search form to save viewing space. You can still enable this field to show in advanced search settings.', 'events-manager' );
		...
		<table class="form-table em-search-form-main">
					...
					<td>
						...
						<fieldset class="em-default-option option-checkboxes em-field-checkboxes">
							...
							<?php foreach ( $views as $view_name => $view_label ) : ?>
								<label>
									<input type="checkbox" name="dbem_search_form_views[]" value="<?php echo esc_attr( $view_name ); ?>" 
									<?php
									if ( in_array( $view_name, $view_types ) ) {
										echo 'checked';}
									?>
									>
						...
						<div class="em-archetype-options">
							<?php foreach ( $archetype_options['archetypes'] as $type => $atts ) : ?>
								<div class="em-archetype-option option-checkboxes" data-archetype="<?php echo $type; ?>"  
								<?php
								if ( empty( $atts['value'] ) ) {
									echo 'data-default';}
								?>
								>
									<?php foreach ( $views as $view_name => $view_label ) : ?>
										<label>
											<input type="checkbox" name="<?php echo esc_attr( $atts['name'] ); ?>[]" value="<?php echo esc_attr( $view_name ); ?>"
												...
												<?php
												if ( in_array( $view_name, $atts['value'] ) ) {
													echo 'checked';}
												?>
												>
											<?php echo esc_html( $view_label ); ?>
									...
									<div class="em-archetype-checkbox-buttons">
										<button type="button" class="button-secondary em-archetype-checkbox-default" 
										<?php
										if ( empty( $atts['value'] ) ) {
											echo 'disabled';}
										?>
										><?php esc_html_e( 'Use Defaults', 'events-manager' ); ?></button>
										<button type="button" class="button-secondary em-archetype-checkbox-override" 
										<?php
										if ( empty( $atts['value'] ) ) {
											echo 'disabled';}
										?>
										><?php esc_html_e( 'Override Defaults', 'events-manager' ); ?></button>
									</div>
								</div>
							<?php endforeach; ?>
						</div>
			...
			<tbody class="em-subsection em-search-form-main-option em-settings-geocoding" id="em-search-form-geo">
				...
				em_options_select(
					__( 'Default distance unit', 'events-manager' ),
					'dbem_search_form_geo_unit_default',
					array(
						'km' => 'km',
						'mi' => 'mi',
					),
		...
		<table class="form-table">
			...
			<tbody class="em-search-form-advanced">
				...
				em_options_select(
					__( 'Advanced search style', 'events-manager' ),
					'dbem_search_form_advanced_style',
					array(
						'accordion' => __( 'Accordion', 'events-manager' ),
						'headings'  => __( 'Headings', 'events-manager' ),
					),
					__( 'The advanced search can be styled in a section-based format, with expandable sections, or directly inline with static headings. If you want headings for some sections and not others, choose Headings and leave specific section heading labels blank in settings further down.', 'events-manager' ),
					''
				);

				$triggers = array( 'inline' => '#dbem_search_form_advanced_hidden_row, #dbem_search_form_advanced_hide_row' );
				em_options_select(
					__( 'Advanced search mode', 'events-manager' ),
					'dbem_search_form_advanced_mode',
					array(
						'inline' => __( 'Inline', 'events-manager' ),
						'modal'  => __( 'Modal', 'events-manager' ),
					),
					__( 'You can choose to show a popup modal or inline under the main search bar triggered by an icon on the main search bar.', 'events-manager' ),
					'',
					$triggers
				);
				?>
	...
	<div class="inside">
		<table class="form-table">
			<?php
			em_options_select(
				__( 'Default calendar size', 'events-manager' ),
				'dbem_calendar_size',
				array(
					'auto'   => esc_html__( 'Responsive', 'events-manager' ),
					'large'  => esc_html__( 'Large', 'events-manager' ),
					'medium' => esc_html__( 'Medium', 'events-manager' ),
					'small'  => esc_html__( 'Small', 'events-manager' ),
				),
				sprintf( __( 'Unless speficied with the %s parameter, the calendar will by default remain the chosen size regardless of calendar width. Responsive calendar size will adjust from small to large according to the calendar width on screen.', 'events-manager' ), '<code>calendar_size</code>' )
			);
			em_options_radio_binary( __( 'Split by timeslots?', 'events-manager' ), 'dbem_calendar_timeslots', sprintf( __( 'Unless overriden with the %s parameter in shortcode or PHP arguments, the calendar will by default split events by timeslot if on the same day or show one event for the day which includes the whole range of timeslots.', 'events-manager' ), '<code>timeslots</code>' ) );
			...
			em_options_input_text(
				__( 'Event format', 'events-manager' ),
				'dbem_calendar_large_pill_format',
				__( 'The format of each event when displayed in the full calendar.', 'events-manager' ) . $events_placeholder_tip . ' <br>' .
			...
			<?php
			em_options_select(
				__( 'Event Preview Mode', 'events-manager' ),
				'dbem_calendar_preview_mode',
				array(
					'tooltips' => esc_html__( 'Tooltip', 'events-manager' ),
					'modal'    => esc_html__( 'Modal Popup', 'events-manager' ),
					'none'     => esc_html__( 'No Preview', 'events-manager' ),
				),
				__( 'Choose how to show a information about an event when clicking or hovering over a single event on the calendar.', 'events-manager' )
			);
			em_options_select(
				__( 'Date Preview Mode', 'events-manager' ),
				'dbem_calendar_preview_mode_date',
				array(
					'modal' => esc_html__( 'Modal Popup', 'events-manager' ),
					'none'  => esc_html__( 'Direct Link', 'events-manager' ),
				),
				__( 'Choose whether to show a preview of the upcoming events for the date clicked on, or directly link to the calendar day.', 'events-manager' )
			);
			?>
					...
					<select name="dbem_display_calendar_orderby" >
						<?php
							$orderby_options = apply_filters(
								'dbem_display_calendar_orderby_ddm',
								array(
			...
			<tbody class="am-af">
				<tr class="em-header">
					<td colspan="2">
						...
						<em><?php echo sprintf( __( 'These formats can be used on %1$s pages or on other areas of your site displaying an %2$s.', 'events-manager' ), __( 'location', 'events-manager' ), __( 'location', 'events-manager' ) ); ?></em>
					</td>
				</tr>
			...
			<tbody class="am-af">
				<tr class="em-header">
					<td colspan="2">
						...
						<em><?php echo sprintf( __( 'These formats can be used when WordPress automatically displays %1$s excerpts on your site and %2$s is enabled in your %3$s settings tab.', 'events-manager' ), __( 'location', 'events-manager' ), '<strong>' . __( 'Override Excerpts with Formats?', 'events-manager' ) . '</strong>', '<a href="#formats" class="nav-tab-link" rel="#em-menu-pages">' . __( 'Pages', 'events-manager' ) . '  &gt; ' . sprintf( __( '%s List/Archives', 'events-manager' ), __( 'Location', 'events-manager' ) ) . '</a>' ); ?></em>
					</td>
				</tr>
	...
	<div class="inside">
		...
		<table class='form-table'>
			<tr valign="top">
				<?php em_options_input_text( __( 'Default map width', 'events-manager' ), 'dbem_map_default_width', sprintf( __( 'Can be in form of pixels or a percentage such as %1$s or %2$s.', 'events-manager' ), '<code>100%</code>', '<code>100px</code>' ) ); ?>
				<?php em_options_input_text( __( 'Default map height', 'events-manager' ), 'dbem_map_default_height', sprintf( __( 'Can be in form of pixels or a percentage such as %1$s or %2$s.', 'events-manager' ), '<code>100%</code>', '<code>100px</code>' ) ); ?>
			</tr>
			...
			<tbody class="am-af am-af-maps">
				<tr class="em-header"><td colspan="2">
					...
					<p><?php echo sprintf( __( 'If you use the %1$s <a href="%2$s">shortcode</a>, you can display a map of all your locations and events, the settings below will be used.', 'events-manager' ), '<code>[locations_map]</code>', 'http://wp-events-plugin.com/documentation/shortcodes/' ); ?></p>
				</td></tr>
				<?php
