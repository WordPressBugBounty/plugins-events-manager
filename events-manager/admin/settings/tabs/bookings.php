...
if ( ! defined( 'EM_SETTINGS_TABS' ) || ! EM_SETTINGS_TABS ) :
	...
	<div class="inside">
		<table class='form-table'>
			...
			$gateway_approval = '<br><strong>' . __( 'This setting will not apply to bookings using payment gateways, see individual gateways for approval settings.', 'events-manager' ) . '</strong>';
			em_options_radio_binary( __( 'Approval Required?', 'events-manager' ), 'dbem_bookings_approval', __( 'Bookings will not be confirmed until the event administrator approves it.', 'events-manager' ) . ' ' . $gateway_approval );
			em_options_radio_binary( __( 'Reserved unconfirmed spaces?', 'events-manager' ), 'dbem_bookings_approval_reserved', __( 'By default, event spaces become unavailable once there are enough CONFIRMED bookings. To reserve spaces even if unapproved, choose yes.', 'events-manager' ) . ' ' . $gateway_approval );
			...
			$cancellation_hours_desc_2 = __( '%1$s are also accepted, for example %2$s equals 1 month and 12 hours before the event starts.', 'events-manager' );
	...
	<div class="inside">
		...
		<table class='form-table'>
			...
			em_options_input_text( __( 'No spaces booked', 'events-manager' ), 'dbem_booking_feedback_min_space', __( 'If the user tries to make a booking without requesting any spaces.', 'events-manager' ) );
			$notice_full = __( 'Sold Out', 'events-manager' );
	...
	<div class="inside">
		<table class='form-table'>
			...
			em_options_select(
				__( 'Recurring Events Picker', 'events-manager' ),
				'dbem_recurrence_picker',
				array(
					'select'   => __( 'Dropdown Selection', 'events-manager' ),
					'calendar' => __( 'Calendar Selection', 'events-manager' ),
				),
				__( 'Recurring events have a picker in order to load the right booking form, choose your picker style for your booking forms.', 'events-manager' )
			);
			em_options_input_text( __( 'Submit button text', 'events-manager' ), 'dbem_bookings_submit_button', sprintf( __( 'The text used by the submit button. To use an image instead, enter the full url starting with %1$s or %2$s.', 'events-manager' ), '<code>http://</code>', '<code>https://</code>' ) ) . ' ' . __( 'This button text would be shown for a free booking', 'events-manager' );
	...
	<div class="inside">
		<table class='form-table'>
			...
			$ticket_orders = apply_filters(
				'em_tickets_orderby_options',
				array(
