<?php if ( ! function_exists( 'current_user_can' ) || ! current_user_can( 'manage_options' ) ) {
	return;} ?>
<!-- PAGE OPTIONS -->
<div class="em-menu-pages em-menu-group"  
<?php
if ( ! defined( 'EM_SETTINGS_TABS' ) || ! EM_SETTINGS_TABS ) :
	...
	style="display:none;"<?php endif; ?>>			
		...
		$page_templates      = array(
			'posts'                                   => __( 'Posts' ),
			'page'                                    => __( 'Pages' ),
			__( 'Theme Templates', 'events-manager' ) => array_flip( get_page_templates() ),
		);
		?>
		...
		<div class="inside">
			<table class="form-table">
				...
				em_options_select_page( __( 'Events page', 'events-manager' ), 'dbem_events_page', array( 'none' => sprintf( __( '[No %s Page]', 'events-manager' ), __( 'Events', 'events-manager' ) ) ), $events_page_desc );
				?>
			...
			<tr class="em-header">
				<td colspan="2">
					...
					<p><?php echo sprintf( __( '%1$s custom post types can have archives, just like normal WordPress posts. If enabled, should you visit your base slug url %2$s and you will see an post-formatted archive of previous %3$s', 'events-manager' ), __( 'Event', 'events-manager' ), '<code>' . home_url() . '/' . esc_html( get_option( 'dbem_cp_events_slug', EM_POST_TYPE_EVENT_SLUG ) ) . '/</code>', __( 'events', 'events-manager' ) ); ?></p>
					<p><?php echo sprintf( __( 'Note that assigning a %1$s page above will override this archive if the URLs collide (which is the default setting, and is recommended for maximum plugin compatibility). You can have both at the same time, but you must ensure that your page and %2$s slugs are different.', 'events-manager' ), __( 'events', 'events-manager' ), __( 'event', 'events-manager' ) ); ?></p>
				</td>
			</tr>
			...
			<tbody class="em-event-archive-options em-event-archive-sub-options">
				<tr valign="top">
						...
						<select name="dbem_events_default_archive_orderby" >
							<?php
								$event_archive_orderby_options = apply_filters(
									'em_settings_events_default_archive_orderby_ddm',
									array(
						...
						<select name="dbem_events_default_archive_order" >
							...
							$event_archive_order_options = apply_filters(
								'em_settings_events_default_archive_order_ddm',
								array(
			...
			<tr valign="top" id='dbem_events_default_orderby_row'>
					...
					<select name="dbem_events_default_orderby" >
						<?php
							$event_list_orderby_options = apply_filters(
					...
					<select name="dbem_events_default_order" >
						...
						$ascending                = __( 'Ascending', 'events-manager' );
						$descending               = __( 'Descending', 'events-manager' );
						$event_list_order_options = apply_filters(
		...
		<div class="inside">
			...
			<tr>
				...
				<td>
					...
					wp_dropdown_pages(
						array(
							'name'             => 'dbem_locations_page',
							'selected'         => get_option( 'dbem_locations_page' ),
							'show_option_none' => sprintf( __( '[No %s Page]', 'events-manager' ), __( 'Locations', 'events-manager' ) ),
						)
					);
					?>
					<br />
					<em><?php echo sprintf( __( 'This option allows you to select which page to use as the %1$s page. If you do not select a %2$s page, to display lists you can enable archives or use the appropriate shortcodes and/or template tags.', 'events-manager' ), __( 'locations', 'events-manager' ), __( 'locations', 'events-manager' ) ); ?></em>
				</td>
			</tr>
			<?php
			...
			<tr class="em-header">
				<td colspan="2">
					...
					<p><?php echo sprintf( __( '%1$s custom post types can have archives, just like normal WordPress posts. If enabled, should you visit your base slug url %2$s and you will see an post-formatted archive of previous %3$s', 'events-manager' ), __( 'Location', 'events-manager' ), '<code>' . home_url() . '/' . esc_html( get_option( 'dbem_cp_locations_slug', EM_POST_TYPE_LOCATION_SLUG ) ) . '/</code>', __( 'locations', 'events-manager' ) ); ?></p>
					<p><?php echo sprintf( __( 'Note that assigning a %1$s page above will override this archive if the URLs collide (which is the default settings, and is recommended for maximum plugin compatibility). You can have both at the same time, but you must ensure that your page and %2$s slugs are different.', 'events-manager' ), __( 'locations', 'events-manager' ), __( 'location', 'events-manager' ) ); ?></p>
				</td>
			</tr>
			...
			<tbody class="em-location-archive-options em-location-archive-sub-options">
				<tr valign="top">
						...
						<select name="dbem_locations_default_archive_orderby" >
							<?php
								$locations_list_orderby_options = apply_filters(
									'em_settings_locations_default_archive_orderby_ddm',
									array(
						...
						<select name="dbem_locations_default_archive_order" >
							...
							$ascending                    = __( 'Ascending', 'events-manager' );
							$descending                   = __( 'Descending', 'events-manager' );
							$locations_list_order_options = apply_filters(
								'em_settings_locations_default_archive_order_ddm',
								array(
									'ASC'  => __( 'Ascending', 'events-manager' ),
									'DESC' => __( 'Descending', 'events-manager' ),
								)
			...
			<tr valign="top" id='dbem_locations_default_orderby_row'>
					...
					<select name="dbem_locations_default_orderby" >
						<?php
							$locations_list_orderby_options = apply_filters(
								'em_settings_locations_default_orderby_ddm',
								array(
					...
					<select name="dbem_locations_default_order" >
						...
						$descending                   = __( 'Descending', 'events-manager' );
						$locations_list_order_options = apply_filters(
							'em_settings_locations_default_order_ddm',
							array(
								'ASC'  => __( 'Ascending', 'events-manager' ),
								'DESC' => __( 'Descending', 'events-manager' ),
							)
		...
		<div class="inside">
			<div class="em-boxheader">
				<p>
					<?php echo sprintf( __( '%1$s are a <a href="%2$s" target="_blank">WordPress custom taxonomy</a>.', 'events-manager' ), __( 'Event Categories', 'events-manager' ), 'http://codex.wordpress.org/Taxonomies' ); ?>
					<?php echo sprintf( __( '%1$s can be displayed just like normal WordPress custom taxonomies in an archive-style format, however Events Manager by default allows you to completely change the standard look of these archives and use our own <a href="%2$s">custom formatting</a> methods.', 'events-manager' ), __( 'Event Categories', 'events-manager' ), EM_ADMIN_URL . '&amp;page=events-manager-help#event-placeholders' ); ?>
				</p>
				<p>
					...
					<?php sprintf( __( '<a href="%1$s">See some more information</a> on how %2$s work when overriding with formats.', 'events-manager' ), '#', __( 'categories', 'events-manager' ) ); //not ready yet, but make translatable ?>
				</p>
			</div>
			<table class="form-table">
			<tr>
				...
				<td>
					...
					wp_dropdown_pages(
						array(
							'name'             => 'dbem_categories_page',
							'selected'         => get_option( 'dbem_categories_page' ),
							'show_option_none' => sprintf( __( '[No %s Page]', 'events-manager' ), __( 'Categories', 'events-manager' ) ),
						)
					);
					?>
					<br />
			...
			<tr valign="top" id='dbem_categories_default_orderby_row'>
					...
					<select name="dbem_categories_default_orderby" >
						<?php
							$categories_list_orderby_options = apply_filters(
								'em_settings_categories_default_orderby_ddm',
								array(
					...
					<select name="dbem_categories_default_order" >
						...
						$ascending                     = __( 'Ascending', 'events-manager' );
						$descending                    = __( 'Descending', 'events-manager' );
						$categories_list_order_options = apply_filters(
							'em_settings_categories_default_order_ddm',
							array(
								'ASC'  => __( 'Ascending', 'events-manager' ),
								'DESC' => __( 'Descending', 'events-manager' ),
							)
		...
		<div class="inside">
			<div class="em-boxheader">
				<p>
					<?php echo sprintf( __( '%1$s are a <a href="%2$s" target="_blank">WordPress custom taxonomy</a>.', 'events-manager' ), __( 'Event Tags', 'events-manager' ), 'http://codex.wordpress.org/Taxonomies' ); ?>
					<?php echo sprintf( __( '%1$s can be displayed just like normal WordPress custom taxonomies in an archive-style format, however Events Manager by default allows you to completely change the standard look of these archives and use our own <a href="%2$s">custom formatting</a> methods.', 'events-manager' ), __( 'Event Tags', 'events-manager' ), EM_ADMIN_URL . '&amp;page=events-manager-help#event-placeholders' ); ?>
				</p>
				<p>
					...
					<?php sprintf( __( '<a href="%1$s">See some more information</a> on how %2$s work when overriding with formats.', 'events-manager' ), '#', __( 'tags', 'events-manager' ) ); //not ready yet, but make translatable ?>
				</p>
			</div>
			<table class="form-table">
				<tr>
					...
					<td>
						...
						wp_dropdown_pages(
							array(
								'name'             => 'dbem_tags_page',
								'selected'         => get_option( 'dbem_tags_page' ),
								'show_option_none' => sprintf( __( '[No %s Page]', 'events-manager' ), __( 'Tags', 'events-manager' ) ),
							)
						);
						?>
						<br />
				...
				<tr valign="top" id='dbem_tags_default_orderby_row'>
						...
						<select name="dbem_tags_default_orderby" >
							<?php
								$tags_list_orderby_options = apply_filters(
									'em_settings_tags_default_orderby_ddm',
									array(
										'id'         => sprintf( __( 'Order by %s', 'events-manager' ), __( 'ID', 'events-manager' ) ),
										'count'      => sprintf( __( 'Order by %s', 'events-manager' ), __( 'Count', 'events-manager' ) ),
										'name'       => sprintf( __( 'Order by %s', 'events-manager' ), __( 'Name', 'events-manager' ) ),
						...
						<select name="dbem_tags_default_order" >
							...
							$ascending               = __( 'Ascending', 'events-manager' );
							$descending              = __( 'Descending', 'events-manager' );
							$tags_list_order_options = apply_filters(
								'em_settings_tags_default_order_ddm',
								array(
									'ASC'  => __( 'Ascending', 'events-manager' ),
									'DESC' => __( 'Descending', 'events-manager' ),
								)
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							wp_dropdown_pages(
								array(
									'name'             => 'dbem_my_bookings_page',
									'selected'         => get_option( 'dbem_my_bookings_page' ),
									'show_option_none' => '[' . __( 'None', 'events-manager' ) . ']',
								)
							);
							?>
							<br />
						...
						<td>
							<select name="dbem_bookings_default_orderby" >
								<?php
									$orderby_options = apply_filters(
										'em_settings_bookings_default_orderby_ddm',
										array(
							...
							<select name="dbem_bookings_default_order" >
								...
								$order_options = apply_filters(
									'em_settings_bookings_default_order_ddm',
									array(
										'ASC'  => __( 'Ascending', 'events-manager' ),
										'DESC' => __( 'Descending', 'events-manager' ),
									)
