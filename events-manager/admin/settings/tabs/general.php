...
<div class="em-menu-general em-menu-group em">
				...
				if ( ! ( EM_MS_GLOBAL && ! is_main_site() ) ) {
					...
					if ( get_option( 'dbem_categories_enabled' ) ) {
						...
						wp_dropdown_categories(
							array(
								'hide_empty'       => 0,
								'name'             => 'dbem_default_category',
								'hierarchical'     => true,
								'taxonomy'         => EM_TAXONOMY_CATEGORY,
								'selected'         => get_option( 'dbem_default_category' ),
				...
				<tr valign="top" id='dbem_location_types_row'>
					...
					<td>
						...
						<div class="em-default-option option-checkboxes">
							<label>
								<input type="checkbox" name="dbem_location_types[location]" value="1" 
								<?php
								if ( ! empty( $location_types['location'] ) ) {
									echo 'checked';}
								?>
							...
							<?php foreach ( EM_Event_Locations\Event_Locations::get_types() as $event_location_type => $EM_Event_Location_Class ) : /* @var EM_Event_Locations\Event_Location $EM_Event_Location_Class */ ?>
								<br>
								<label>
									<input type="checkbox" name="dbem_location_types[<?php echo esc_attr( $event_location_type ); ?>]" value="1" 
									<?php
									if ( ! empty( $location_types[ $event_location_type ] ) ) {
										echo 'checked';}
									?>
						...
						<div class="em-archetype-options">
							...
							>
								<label>
									<input type="checkbox" name="<?php echo esc_attr( $atts['name'] ); ?>[location]" value="1" 
									<?php
									if ( ! empty( $atts['value']['location'] ) ) {
										echo 'checked';}
									?>
								...
								<?php foreach ( EM_Event_Locations\Event_Locations::get_types() as $event_location_type => $EM_Event_Location_Class ) : /* @var EM_Event_Locations\Event_Location $EM_Event_Location_Class */ ?>
									<br>
									<label>
										<input type="checkbox" name="<?php echo esc_attr( $atts['name'] ); ?>[<?php echo esc_attr( $event_location_type ); ?>]" value="1" 
										<?php
										if ( ! empty( $atts['value'][ $event_location_type ] ) ) {
											echo 'checked';}
										?>
	...
	<?php
	if ( ! is_multisite() || ( em_wp_is_super_admin() && ! get_site_option( 'dbem_ms_global_caps' ) ) ) {
		...
		<div class="inside">
			...
			<table class="form-table">
				...
				<tbody class="form-table em-google-maps-enabled">
					...
					$google_map_options      = apply_filters(
					...
					$embed_options = array(
						'place'       => __( 'Location name and address', 'events-manager' ),
						'address'     => __( 'Address only', 'events-manager' ),
						'coordinates' => __( 'Location coordinates', 'events-manager' ),
					);
		...
		<div class="inside">
			<table class='form-table'>
				...
				<tbody class="em-phone-options" id="em-phone-settings">
					<?php
						$phone_countries = array_merge( array( '' => esc_html( 'None Selected' ) ), em_get_countries() );
						...
						em_options_select(
							esc_html__( 'Include Countries', 'events-manager' ),
							'dbem_phone_countries_include',
							$phone_countries,
							esc_html__( 'Only the selected countries will be included in the country selection list. This takes precedence over excluded countries.', 'events-manager' ),
							'',
							array(),
							array(
								'selectize' => true,
						...
						em_options_select(
							esc_html__( 'Exclude Countries', 'events-manager' ),
							'dbem_phone_countries_exclude',
							$phone_countries,
							esc_html__( 'These countries will be excluded from the country selection list.', 'events-manager' ),
							'',
							array(),
							array(
								'selectize' => true,
		...
		<table class="form-table">
			...
			<tbody class="theme-css">
				<tr><td colspan="2">
					...
					<p>
					<?php echo sprintf( esc_html__( 'Our theme has multiple CSS variables that can be overriden easily. The options below could be overriden just as easily via one line of CSS in your %1$s settings area, for example %2$s' ), '<em>' . __( 'Customizer' ) . ' > ' . __( 'Additional CSS' ) . '</em>', '<code>body .em.pixelbones{ --font-family:arial, --font-size:14px; --font-weight: normal; --line-height:16px; }</code>' ); ?>
					</p>
				</td></tr>
				<?php
				$settings = array(
					0 => esc_html__( 'Default plugin setting', 'events-manager' ),
					1 => esc_html__( 'Inherit your theme settings', 'events-manager' ),
				);
			...
			<tbody class="all-css">
				...
				<?php
					$select = array(
						1 => __( 'Enabled', 'events-manager' ),
						0 => __( 'Disabled', 'events-manager' ),
						2 => __( 'Basic Only', 'events-manager' ),
					);
