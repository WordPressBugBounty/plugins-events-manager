...
function wp_events_plugin_init() {
	...
	if ( get_option( 'dbem_tags_enabled', true ) ) {
		register_taxonomy(
			EM_TAXONOMY_TAG,
			array( EM_POST_TYPE_EVENT, 'event-recurring' ),
			apply_filters(
				'em_ct_tags',
				array(
					'hierarchical'   => false,
	...
	if ( get_option( 'dbem_repeating_enabled' ) ) {
		$event_recurring_post_type = apply_filters(
			'em_cpt_event_recurring',
			array(
	...
	if ( get_option( 'dbem_locations_enabled', true ) ) {
		$location_post_type = apply_filters(
			'em_cpt_location',
			array(
				'public'              => true,
				'hierarchical'        => false,
				...
				'publicly_queryable'  => true,
				'rewrite'             => array(
					'slug'       => EM_POST_TYPE_LOCATION_SLUG,
					'with_front' => false,
				),
				'query_var'           => true,
	...
	if ( defined( 'EM_GUTENBERG' ) && EM_GUTENBERG ) {
		...
		if ( get_option( 'dbem_repeating_enabled' ) ) {
			$event_recurring_post_type['show_in_rest'] = true;
		}
		if ( get_option( 'dbem_locations_enabled', true ) ) {
			$location_post_type['show_in_rest'] = true;
		}
		function em_gutenberg_support( $can_edit, $post_type ) {
			...
			if ( $post_type == EM_POST_TYPE_EVENT || $recurrences || $locations ) {
				$can_edit = true;
			}
			return $can_edit;
		}
...
function em_map_meta_cap( $caps, $cap, $user_id, $args ) {
	if ( ! empty( $args[0] ) ) {
		...
		if ( 'edit_event' == $cap || 'delete_event' == $cap || 'read_event' == $cap ) {
			...
			if ( empty( $post->post_type ) || ! in_array( $post->post_type, array( EM_POST_TYPE_EVENT, 'event-recurring' ) ) ) {
				return $caps;
			}
		...
		if ( 'edit_recurring_event' == $cap || 'delete_recurring_event' == $cap || 'read_recurring_event' == $cap ) {
			...
			if ( empty( $post->post_type ) || $post->post_type != 'event-recurring' ) {
				return $caps;
			}
			//continue with getting post type and assigning caps
			$EM_Event  = em_get_event( $post );
		...
		if ( 'edit_location' == $cap || 'delete_location' == $cap || 'read_location' == $cap ) {
			...
			if ( empty( $post->post_type ) || $post->post_type != EM_POST_TYPE_LOCATION ) {
				return $caps;
			}
			//continue with getting post type and assigning caps
