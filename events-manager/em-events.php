...
function em_content( $page_content ) {
	...
	if ( empty( $post ) || empty( $post->ID ) || ! $in_the_loop ) {
		return $page_content; //fix for any other plugins calling the_content outside the loop
	}
	...
	if ( ! post_password_required() && in_array( $post->ID, array( $locations_page_id, $categories_page_id, $edit_locations_page_id, $my_bookings_page_id, $tags_page_id, ...array_values( $edit_bookings_page_ids ), ...array_values( $edit_events_page_ids ), ...array_values( $event_page_ids ) ) ) ) {
		get_post();
		...
		if ( empty( $content ) ) {
			...
			if ( in_array( $post->ID, $event_page_ids ) && $post->ID != 0 ) {
				...
				if ( ! empty( $args['ajax'] ) ) {
					echo '<div class="em-search-ajax">'; } //AJAX wrapper open
				em_locate_template( 'templates/categories-list.php', true, array( 'args' => $args ) );
				if ( ! empty( $args['ajax'] ) ) {
					echo '</div>'; //AJAX wrapper close
				}
...
function em_content_page_title( $original_content, $id = null ) {
	...
	$in_the_loop = ! ( defined( 'EM_CHECK_THE_LOOP' ) && EM_CHECK_THE_LOOP ) || in_the_loop(); // check if we're in the loop... since this may break some themes, we're introducing it as a constant for themes that cause issues vs. breaking themes that already work.
	if ( empty( $post ) || empty( $post->ID ) || ! $in_the_loop ) {
		return $original_content; //fix for any other plugins calling the_content outside the loop
	}
	if ( $id && $id !== $post->ID ) {
	...
	$edit_locations_page_id = get_option( 'dbem_edit_locations_page' );
	$edit_bookings_page_ids = Archetypes::get_option_values( 'dbem_edit_bookings_page' );
	if ( ! empty( $post->ID ) && in_array( $post->ID, array( $locations_page_id, $edit_locations_page_id, ...array_values( $edit_bookings_page_ids ), ...array_values( $edit_events_page_ids ), ...array_values( $event_page_ids ) ) ) ) {
...
function em_content_wp_title( $title, $sep = '', $seplocation = '' ) {
	...
	$in_the_loop = ! ( defined( 'EM_CHECK_THE_LOOP' ) && EM_CHECK_THE_LOOP ) || in_the_loop(); // check if we're in the loop... since this may break some themes, we're introducing it as a constant for themes that cause issues vs. breaking themes that already work.
	if ( empty( $post ) || empty( $post->ID ) || ! $in_the_loop ) {
		return $title; //fix for any other plugins calling the_content outside the loop
	}
	...
	if ( ! empty( $post->ID ) && ! in_array( $post->ID, $pages ) ) {
		return $title; }
	...
	if ( $new_title != $title ) {
		...
		foreach ( $title_array as $title_item_key => $title_item ) {
			if ( trim( $title_item ) == '' ) {
				unset( $title_array[ $title_item_key ] ); }
		} //remove blanks
...
function em_wp_the_title( $data, $id = null ) {
	...
	$in_the_loop = ! ( defined( 'EM_CHECK_THE_LOOP' ) && EM_CHECK_THE_LOOP ) || in_the_loop(); // check if we're in the loop... since this may break some themes, we're introducing it as a constant for themes that cause issues vs. breaking themes that already work.
	if ( empty( $post ) || empty( $post->ID ) || ! $in_the_loop ) {
		return $data; //fix for any other plugins calling the_content outside the loop
	}
