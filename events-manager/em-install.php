...
function em_install() {
	...
	if ( $old_version != '' && $old_version < 4.1 ) {
		function em_update_required_notification() {
			global $EM_Booking;
			?><div class="error"><p><strong>Events Manager upgrade not complete, please upgrade to the version 4.300 or higher first from <a href="http://wordpress.org/extend/plugins/events-manager/download/">here</a> before upgrading to this version.</strong></p></div>
			<?php
		}
...
function em_check_utf8mb4_tables() {
	...
	if ( $em_check_utf8mb4_tables || $em_check_utf8mb4_tables === false ) {
...
function em_create_timeslots_table() {
	...
	// Add additional keys
	em_sort_out_table_nu_keys( $table_name, array( 'timerange_group_id', 'timerange_start' ) );
}
...
function em_add_options() {
	...
	if ( ! $already_installed ) {
		$advice          = sprintf( __( "<p>Events Manager is ready to go! It is highly recommended you read the <a href='%1\$s'>Getting Started</a> guide on our site, as well as checking out the <a href='%2\$s'>Settings Page</a></p>", 'events-manager' ), 'https://wp-events-plugin.com/documentation/getting-started-guide/?utm_source=em&utm_medium=plugin&utm_content=installationlink&utm_campaign=plugin_links', EM_ADMIN_URL . '&amp;page=events-manager-options' );
		$EM_Admin_Notice = new EM_Admin_Notice(
			array(
				'name'    => 'hello',
				'who'     => 'admin',
				'where'   => 'all',
	...
	$dbem_options = array(
		...
		*/
		...
		'dbem_phone_detect'                              => 1,
		'dbem_phone_countries_include'                   => array(),
		'dbem_phone_countries_exclude'                   => array(),
	);
...
function em_upgrade_current_installation() {
	...
	if ( empty( $current_version ) || ! isset( $data['admin-modals'] ) ) { // if admin-modals isn't set, it was never added before
		if ( empty( $data['admin-modals'] ) ) {
			$data['admin-modals'] = array();
		}
		if ( ! is_array( $data['admin-modals'] ) ) {
			$data['admin-modals'] = array();
		}
	...
	if ( ! empty( $current_version ) ) {
		...
		if ( $current_version != '' && $current_version < 5.66 ) {
			if ( get_option( 'dbem_ical_description_format' ) == '#_EVENTNAME - #_LOCATIONNAME - #_EVENTDATES - #_EVENTTIMES' ) {
				update_option( 'dbem_ical_description_format', '#_EVENTNAME' );
			}
			if ( get_option( 'dbem_ical_location_format' ) == '#_LOCATION' ) {
				update_option( 'dbem_ical_location_format', '#_LOCATIONNAME, #_LOCATIONFULLLINE, #_LOCATIONCOUNTRY' );
			}
			$old_values = array(
		...
		if ( $current_version != '' && $current_version < 5.821 ) {
			...
			if ( empty( $admin_data['datetime_backcompat'] ) ) {
				...
				if ( $opt == '_start_ts' ) {
					update_option( 'dbem_tags_default_archive_orderby', '_event_start' );
				}
				$opt = get_option( 'dbem_categories_default_archive_orderby' );
				if ( $opt == '_start_ts' ) {
					update_option( 'dbem_categories_default_archive_orderby', '_event_start' );
				}
				$opt = get_option( 'dbem_events_default_archive_orderby' );
				if ( $opt == '_start_ts' ) {
					update_option( 'dbem_events_default_archive_orderby', '_event_start' );
				}
			} else {
			...
			if ( empty( $admin_data['updates'] ) ) {
				$admin_data['updates'] = array();
			}
		...
		if ( $current_version != '' && $current_version < 5.95 ) {
			...
			$EM_Admin_Notice = new EM_Admin_Notice(
				array(
					...
					'who'     => 'admin',
					'where'   => 'all',
					'message' => "<p>$message</p><p>$message2</p>",
				)
			);
			EM_Admin_Notices::add( $EM_Admin_Notice, is_multisite() );
		}
		...
		if ( $current_version != '' && version_compare( $current_version, '6.0', '<' ) ) {
			...
			if ( ! preg_match( "/$non_convertables/", $dateformat ) ) {
				$placeholder_convertables = array(
					'ATOM'      => 'yy-mm-dd',
					'COOKIE'    => 'D, dd M yy',
					'ISO_8601'  => 'yy-mm-dd',
					'RFC_822'   => 'D, d M y',
		...
		if ( $current_version != '' && version_compare( $current_version, '6.1.0.1', '<' ) ) {
			...
			if ( is_array( $cols ) && ! array_key_exists( 'booking_meta_migrated', $cols ) ) {
				...
				if ( $result === false ) {
					...
					$EM_Admin_Notice = new EM_Admin_Notice(
						array(
							'name'    => 'v6.1-booking-atomic-meta-error',
							'who'     => 'admin',
							'where'   => 'all',
							'message' => $message,
							'what'    => 'warning',
					...
					if ( $copy_result ) {
						...
						if ( $copy_count !== null && $original_count !== null && $copy_count === $original_count ) {
							...
							if ( $deletion_result !== false ) {
								...
								EM_Admin_Notices::add(
									new EM_Admin_Notice(
										array(
											'name'    => 'v6.1.2-duplicate-update',
											'who'     => 'admin',
											'where'   => 'settings',
											'message' => $message . $message2,
										)
		...
		if ( version_compare( $current_version, '6.4.10', '<' ) ) {
			...
			EM_Admin_Notices::add(
				new EM_Admin_Notice(
					array(
						'name'    => 'shortcode-security-update-6-4-10',
						'who'     => 'admin',
						'what'    => 'warning',
						'where'   => 'all',
						'message' => $message,
		...
		if ( version_compare( $current_version, '6.6', '<' ) ) { // 6.6. update
			...
			foreach ( array( 'registration', 'attendees', 'coupon', 'booking', 'zoom', 'test', 'discounts', 'surcharges' ) as $field ) {
			...
			foreach ( array( 'dbem_location_default_country', 'dbem_search_form_default_country' ) as $opt ) {
				if ( get_option( $opt ) == 'KV' ) {
					update_option( $opt, 'XK' );
				}
			}
			// change any reference of KV to XK in location and postmeta tables
			$wpdb->update( EM_LOCATIONS_TABLE, array( 'location_country' => 'XK' ), array( 'location_country' => 'KV' ) );
			$wpdb->update(
				$wpdb->postmeta,
				array( 'meta_value' => 'XK' ),
				array(
					'meta_key'   => '_location_country',
					'meta_value' => 'KV',
				)
			);
		}
		...
		if ( version_compare( $current_version, '7.0', '<' ) ) {
			...
			if ( empty( $done_already ) ) {
				...
				$wpdb->query(
					'UPDATE ' . EM_EVENTS_TABLE . " SET event_type = 
		                CASE 
		...
		if ( version_compare( $current_version, '7.2.3', '<' ) ) {
			...
			foreach ( array( 'dbem_map_text_format', 'dbem_location_baloon_format' ) as $opt ) {
				$map_balloon = get_option( $opt );
				$map_balloon = str_replace( array( '<strong>#_LOCATIONNAME</strong><br />', '<strong>#_LOCATIONNAME</strong>' ), '', $map_balloon );
...
function em_migrate_datetime_timezones( $reset_new_fields = true, $migrate_date_fields = true, $timezone = false ) {
	...
	if ( $reset_new_fields || $migrate_date_fields ) {
		$wpdb->query( 'UPDATE ' . $db . 'em_events' . ' SET event_start = NULL, event_end = NULL, event_timezone = NULL' . $blog_id_where );
	}
	if ( ! $migrate_date_fields ) {
		return true;
	}
	...
	if ( $migration_result === false ) {
		$migration_errors[] = array( 'Local datetime allday event times modification errors', $wpdb->last_error );
	}
	...
	if ( ! preg_match( '/^UTC/', $timezone ) ) {
		...
		foreach ( $query_data as $offset => $statements ) {
			...
			if ( $migration_result === false ) {
				$migration_errors[] = array( 'Event start UTC transition', $wpdb->last_error );
			}
			$migration_result = $wpdb->query( $statements['end']['sql'] . ' WHERE event_end IS NULL AND (' . implode( ' OR ', $statements['end']['where'] ) . ')' . $blog_id_and );
			if ( $migration_result === false ) {
				$migration_errors[] = array( 'Event end UTC transation', $wpdb->last_error );
			}
		}
	} else {
		...
		if ( $migration_result === false ) {
			$migration_errors[] = array( 'Event start/end UTC offset', $wpdb->last_error );
		}
	}
	...
	if ( $migration_result === false ) {
		$migration_errors[] = array( 'Event timezone setting', $wpdb->last_error );
	}
	...
	if ( empty( $migration_errors ) ) {
		...
		if ( $migration_result === false ) {
			$migration_errors[] = array( 'Previous meta deletion', $wpdb->last_error );
		}
		foreach ( array( 'event_start', 'event_end', 'event_timezone', 'start', 'end' ) as $field ) {
			...
			$migration_result = $wpdb->query( $sql );
			if ( $migration_result === false ) {
				$migration_errors[] = array( 'Adding new meta data key <em>_' . $field . '</em>', $wpdb->last_error );
			}
		}
	}
...
function em_migrate_get_tz_transitions( $timezone, $blog_id_where = '' ) {
	...
	if ( count( $DTZ_Transitions ) == 1 ) {
		...
		do {
			...
			if ( $current_transition['ts'] < $start ) {
				continue;
			}
			$dst_offset = $current_transition['isdst'] ? 60 * 60 : 0;
			if ( $transition_key == 0 ) {
